#!/usr/bin/env bash
# Tests for scripts/parse-pr-command.sh — the strict allowlist parser for
# /aster-code-review PR-comment commands. Model-free.
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
. "$HERE/lib.sh"
P="$HERE/../scripts/parse-pr-command.sh"
...
test_reject_nonproblems_var()  { run "/aster-code-review benchmark KEEP=1";        assert_eq "rejected" "$RC" 2; }
