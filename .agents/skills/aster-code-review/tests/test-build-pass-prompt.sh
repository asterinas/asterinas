#!/usr/bin/env bash
# Test cases for scripts/build-pass-prompt.sh.
# Run via `make -C tests test-build-pass-prompt`.
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
. "$HERE/lib.sh"
BP="$HERE/../scripts/build-pass-prompt.sh"
...
test_honours_acr_guideline_root() {
    # The benchmark overrides the guideline root; the inlined page must come from it.
    local g="$TMP/groot"
