...
from acr.benchmark.grader import GraderError, calculate_recall, grade
...
class GradeTests(unittest.TestCase):
    ...
    def test_grader_retries_invalid_output(self):
        expected = "# Expected defects\n\n1. location: foo.rs\n   MATCH IF: first\n"
        attempts = 0
        ...
        def response(_request):
            ...
            if attempts == 1:
                return {"results": []}
            return {"results": [{"defect": 1, "status": "caught", "reason": "found"}]}

        backend = FakeBackend({"grader": response})
        result = asyncio.run(
            grade(expected, "review", backend, RunConfig(backend="fake", retries=1))
        )
        self.assertEqual(result.results[0].status, "caught")
