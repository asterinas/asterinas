...
for persona in "$@"; do
    ...
    if [[ "$DISCLOSURE" == progressive ]]; then
        printf 'Do not open guideline files directly. Use print_guidelines.py so the pass loads only the requested rule sections and honors the guideline root selected for the current or benchmark workflow.\n\n'
        python3 "$PRINT_GUIDELINES" "$persona" --catalog
        printf '\n'
    else
