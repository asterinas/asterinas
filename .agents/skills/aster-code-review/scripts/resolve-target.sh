#!/usr/bin/env bash
#
# resolve-target.sh — parse the skill's raw argument string (self-tokenizing)
...
#
# The skill is triggered from a prompt,
# not a shell, so the raw argument string arrives with its quotes intact;
# this script owns the tokenization rather than leaning on a shell to do it.
# Pass the WHOLE argument string as ONE argument:
#
#   resolve-target.sh        '<raw arg string>'   # the canonical review input
#   resolve-target.sh --meta '<raw arg string>'   # mode=/base=|files=/head=/branch=/output=/overwrite=
#
# Raw arg string grammar:
#   diff   <base>              <output> [--overwrite]
#   files  <path[:lines] ...>  <output> [--overwrite]
#
# diff  = the commit series merge-base(base,HEAD)..HEAD — each commit's message and
#         its diff, so the review covers per-commit intent and commit hygiene, not
#         just the net change. Commit first; uncommitted edits are not reviewed here.
...
def fail(msg):
    sys.stderr.write("resolve-target.sh: " + msg + "\n")
    sys.exit(2)
...
# Head token: HEAD short SHA. files mode reviews the working tree,
...
if [[ "$mode" == "diff" ]]; then
    ...
    if [[ -z "$(git rev-list "$mb..HEAD" 2>/dev/null)" ]]; then
        echo "resolve-target.sh: no commits in ${base}..HEAD — commit your changes first" >&2
    fi
    ...
    for i in "${!tpath[@]}"; do
        ...
        if [[ ! -f "$f" ]]; then
            echo "resolve-target.sh: no such file in the working tree: $f" >&2
            exit 2
        fi
        if [[ -z "$r" ]]; then
            printf '===== %s =====\n' "$f"
