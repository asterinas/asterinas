...
the root-pinned `print_guidelines.py <persona> <short-name>...` command printed
in the persona block once for that batch.
...
The print command resolves the same current or benchmark-bundled corpus used to build the catalog,
...
- **Deterministic primitives are scripts, not prose.**
  ...
  guideline access (`print_guidelines.py` — persona catalogs and exact rule chunks),
