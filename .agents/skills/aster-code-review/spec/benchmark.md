...
(The harness itself is agent-agnostic — any agent can be run via `ACR_AGENT_PROFILE`; see [*Agent profiles*](#agent-profiles) — so this is a choice of reference, not a hard-coding.)
...
```
benchmark/
├── problems.yaml             # the suite (schema below); diff problems cite a commit SHA
├── validate-problem-yaml.sh  # model-free schema check (run by run.sh and the tests)
├── overlay-skill.sh          # drops the current skill into a scratch worktree, minus the answers
└── run.sh                    # the harness
```
...
```yaml
- problem_id: <slug>              # REQUIRED, unique. number + kebab slug, e.g. 0004-semop-dead-timer-retain
  base_commit: <rev>              # REQUIRED in files mode ONLY (a SHA or expr, e.g. f4e29d67c^);
                                  #   OMITTED in diff mode, where the base is the commit's parent.
  source: >                       # REQUIRED. freeform provenance + why the problem is leak-free
    ...
  review_mode:                    # REQUIRED. EXACTLY ONE of `diff` / `files`.
    diff:                         #   diff mode: the change under review IS this commit (its diff vs
      commit: <40-char SHA>       #     its parent), fetched by full SHA — no local patch; base = commit^.
      remote: <fetch URL>         #     OPTIONAL; defaults to https://github.com/asterinas/asterinas.
    files:                        #   files mode: targets reviewed at base_commit (whole-file is the norm)
      - <path[:lines]>
  defects:                        # REQUIRED, one or more — the ground truth
    - target:                     #   REQUIRED
      ...
      grounding: <name|bug>       #   REQUIRED: a guideline short-name, or the literal `bug`
...
`validate-problem-yaml.sh` enforces the invariants (model-free, run by `make check` and as a [`tests/`](../tests/) case):
`problem_id` unique; exactly one of `diff`/`files`;
`diff` carries a full-40-char `commit` SHA,
an *optional* `remote` (default upstream),
and *no* `base_commit`; `files` is non-empty and has a `base_commit`;
...
1. **Fix-anchored (the strongest signal).**
   For a known fixed bug, set `base_commit = fixed_by^` (the fix's parent) and review the **whole** file(s) the fix touched.
...
2. **Manual.**
   A `base_commit` plus named files known to contain a defect,
...
| 0004 | semop-dead-timer-retain | diff | timeout timer dropped at scope end (never fires) **and** `retain` removes *all* of a PID's ops — **two** defects | Development · `raii` + `bug` |
| 0005 | virtio-blk-flush-desc-count | diff | flush enqueues 2 descriptors but accounts for 1 → virtqueue exhaustion (off-by-one) | Development · `bug` |
...
| 0001 | mprotect-merge-unwrap | diff | merge absorbs a not-yet-processed mapping → reachable `remove().unwrap()` panic, triggerable from userspace via an `mprotect` `MAY_*` bit (DoS) | Development · `bug` |
...
the shared launcher [`scripts/run-agent.sh`](../scripts/run-agent.sh) is the one thing that turns a profile into a running agent,
and the headless CLI ([`aster-code-review.sh`](../aster-code-review.sh)) and the PR-review CI use the very same profiles and launcher (see [`interface.md`](interface.md)).
...
and **`codex_workflow`** (Codex, `gpt-5.3-codex`, high effort, API-key auth — what the PR-review CI runs).
...
└── codex_workflow/           # the PR-review CI profile
    ...
    └── config.toml           # gpt-5.3-codex, high, danger-full-access
```
...
The launcher `run-agent.sh` makes the private `{workdir}`,
...
0. **Validate** `problems.yaml` against the schema (`validate-problem-yaml.sh`) and **fail closed**
...
2. **Reconstruct the snapshot**
   — for a `diff` problem, fetch the commit by full SHA from its `remote` if it is not already present,
   then check it out in a detached scratch worktree (reviewed as `diff <commit>^`);
   for a `files` problem, a scratch worktree at `base_commit`.
...
5. **Aggregate** — per-problem recall and the suite total;
   ...
   The per-problem combined-vs-fan-out outcome is the labelled data a future `--per-persona-context=auto` heuristic is trained on (see [`execution-model.md`](execution-model.md)).
...
and `overlay-skill.sh` — which drops the current skill into the worktree so it reviews with up-to-date guidelines
— **excludes `benchmark/`**,
