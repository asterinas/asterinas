#!/usr/bin/env bash
#
# overlay-skill.sh — copy TODAY's skill package into a scratch worktree,
# so a historical snapshot is reviewed by the *current* skill (the snapshot's own .agents are stale or absent).
...
# The harness instead points the skill at current guidelines via ACR_GUIDELINE_ROOT (a guidelines-only tree).
#
...
set -euo pipefail
wt="${1:?usage: overlay-skill.sh <worktree>}"
