...
# — see agent_profiles/. The launcher (../scripts/run-agent.sh) and the skill CLI
# (../aster-code-review.sh) do the launching, so this harness names no agent
...
# --- agent launcher ---------------------------------------------------------
# ACR_AGENT_PROFILE selects the agent; the shared launcher (scripts/run-agent.sh)
# resolves and runs it, so this harness names no agent.
# Reviews run through the skill CLI (aster-code-review.sh); grader calls go
# straight to the launcher (grading is not a skill invocation).
SKILL="$(cd "$HERE/.." && pwd)"
ACR_CLI="$SKILL/aster-code-review.sh"
RUN_AGENT="$SKILL/scripts/run-agent.sh"
[[ -n "${ACR_AGENT_PROFILE:-}" ]] || {
    echo "run.sh: ACR_AGENT_PROFILE is required (e.g. ACR_AGENT_PROFILE=codex); run-agent.sh lists available profiles" >&2; exit 2; }
