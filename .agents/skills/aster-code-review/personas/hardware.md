...
— subsections: `assembly-conventions.md`,
`cpu-architecture-specific/x86-64.md`.
...
2. **Per-architecture ABI invariants**
   ...
   a change to a field, padding,
   or size can break an alignment or offset the hardware/ABI depends on,
   even if no assembly is in the diff.

You activate only when the change touches assembly,
an architecture directory, or `asm!`/`global_asm!`.
