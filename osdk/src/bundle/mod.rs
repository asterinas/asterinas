...
use crate::{
    ...
    daemon::{self, DaemonSupervisor, SignalGuard},
    error::Errno,
    error_msg,
...
impl Bundle {
    ...
    pub fn can_run_with_config(&self, config: &Config, action: ActionChoice) -> Result<(), String> {
        ...
        match config_action.boot.method {
            BootMethod::VmmDirect => {
                ...
                // switching boot methods (for example, from a Grub ISO to `vmm-direct`),
    ...
    pub(crate) fn run_qemu_and_wait(&self, config: &Config, action: ActionChoice) -> ExitStatus {
        ...
        let Ok(signal_guard) = SignalGuard::install() else {
        ...
        let Ok(mut daemon_supervisor) =
            DaemonSupervisor::start(&action.qemu.daemons, &config.work_dir, signal)
        else {
            process::exit(Errno::ExecuteCommand as _);
        };
            ...
            && let Some(qemu_log_file) = &action.qemu.log_file
        {
            ...
            let Ok(mut qemu_monitor_stream) = UnixStream::connect(&qemu_monitor_socket_path) else {
                daemon_supervisor.stop_qemu_and_daemons(&mut qemu_child);
                process::exit(Errno::ExecuteCommand as _);
            };
            ...
            let Ok(exit_status) = daemon_supervisor.wait_for_qemu(&mut qemu_child) else {
                daemon_supervisor.stop_qemu_and_daemons(&mut qemu_child);
                process::exit(Errno::ExecuteCommand as _);
            };
            exit_status
        ...
        fn wait_until_guest_kernel_shutdown(
            ...
            qemu_monitor_stream
                ...
                .unwrap_or_else(|err| {
                    ...
                    daemon_supervisor.stop_qemu_and_daemons(qemu_child);
                    process::exit(Errno::ExecuteCommand as _);
                });
            ...
            while monitor_reader.get_mut().write_all(b"info status\n").is_ok() {
                if daemon::signal_value(signal).is_some() {
                    daemon_supervisor.stop_qemu_and_daemons(qemu_child);
                    process::exit(Errno::Interrupted as _);
                }
                if let Err(errno) = daemon_supervisor.check_children() {
                    daemon_supervisor.stop_qemu_and_daemons(qemu_child);
