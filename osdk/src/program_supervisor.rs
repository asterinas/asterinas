...
use crate::{
    error::Errno,
    error_msg,
    program::{POLL_INTERVAL, RunningDaemon, SHUTDOWN_TIMEOUT},
    signal::signal_value,
    warn_msg,
};
...
pub(crate) struct DaemonSupervisor<'a> {
    ...
    running_daemons: Vec<RunningDaemon>,
...
impl<'a> DaemonSupervisor<'a> {
    ...
    pub(crate) fn start(
        ...
        for executable in executables {
            ...
            if let Err(errno) = supervisor.start_daemon(executable) {
                supervisor.stop_all();
                return Err(errno);
            }
    ...
    fn start_daemon(&mut self, executable: &Path) -> Result<(), Errno> {
        info!("Starting managed QEMU daemon `{}`", executable.display());

        let running = RunningDaemon::new(executable, self.work_dir)?;
