...
pub fn dump_coverage_from_qemu(cov_file: &PathBuf, monitor_socket: &mut UnixStream) {
    let cmd = "pmemsave 0x10000000 0x1000000 coverage.mem\n".to_string();
...
fn read_coverage_data_from_file(file_path: &PathBuf) -> Result<Vec<u8>> {
    ...
    info!("Reading {} bytes of coverage data from file", data_length);
