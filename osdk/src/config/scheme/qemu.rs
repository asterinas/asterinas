...
pub struct QemuScheme {
    ...
    /// Daemon executables and their arguments to keep alive for the duration of QEMU.
    pub daemons: Option<Vec<Daemon>>,
}
...
impl QemuScheme {
    pub fn inherit(&mut self, from: &Self) {
        ...
        if self.daemons.is_none() {
            self.daemons.clone_from(&from.daemons);
        }
    }
    ...
    pub fn finalize(self, arch: Arch) -> Qemu {
        Qemu {
            ...
            daemons: self.daemons.unwrap_or_default(),
        }
    }
}
