...
// Add `target/osdk/base` and `target/osdk/base-test` to `exclude` array of the workspace manifest
fn exclude_osdk_base(metadata: &serde_json::Value) {
    let osdk_run_base_path = "target/osdk/base";
    let osdk_test_base_path = "target/osdk/base-test";
    ...
    if let Some(workspace) = manifest_toml.get_mut("workspace") {
        ...
        let exclude =
            toml::Table::from_str(r#"exclude = ["target/osdk/base", "target/osdk/base-test"]"#)
                .unwrap();
