...
///
/// A maximum buddy order of 32 supports up to 4KiBs*2^31 = 8 TiBs of chunks.
...
///
/// A maximum local buddy order of 18 supports up to 4KiBs*2^17 = 512 MiBs of
/// chunks.
...
impl GlobalFrameAllocator for FrameAllocator {
    ...
    fn dealloc(&self, mut addr: Paddr, mut size: usize) {
        let irq_guard = trap::disable_local();
        let local_pool_cell = LOCAL_POOL.get_with(&irq_guard);
        let mut local_pool = local_pool_cell.borrow_mut();
...
pub mod balancing {
    ...
    ///
    /// The minimal will be the expected size multiplied by this constant.
