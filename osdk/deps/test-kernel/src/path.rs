...
impl KtestPath {
    ...
    pub(crate) fn push_back(&mut self, s: &str) {
        self.path.push_back(PathElement::from(s));
    }

    #[cfg_attr(not(ktest), expect(dead_code))]
    pub(crate) fn pop_back(&mut self) -> Option<PathElement> {
        self.path.pop_back()
    }

    #[expect(dead_code)]
    pub(crate) fn push_front(&mut self, s: &str) {
    ...
    pub(crate) fn iter(&self) -> KtestPathIter<'_> {
        self.path.iter()
    }
}
...
impl SuffixTrie {
    ...
    pub(super) fn insert<I, P>(&mut self, path: I)
    where
    ...
    #[cfg_attr(not(ktest), expect(dead_code))]
    pub(super) fn matches<I, P>(&self, path: I) -> bool
    where
        I: DoubleEndedIterator<Item = P>,
        P: Deref<Target = PathElement>,
    {
