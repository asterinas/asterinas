...
impl<const ORDER: usize> BuddyFreeList<ORDER> {
    ...
    pub(crate) fn insert_to_free_list(&mut self, class: usize, frame: usize) {
        ...
        if self.block_cnt[class] == 0 {
            ...
            let mut freepage = FreePage::try_lock(self.free_lists[class] * PAGE_SIZE)
                .expect("Failed to lock FreePage while adding frame");
            freepage.set_value(class + 1);
    ...
    pub(crate) fn add_free_pages(&mut self, range: Range<usize>) -> usize {
        ...
        while current_start + segment_length < end {
            if FreePage::try_lock(current_start * PAGE_SIZE).is_none() {
                if segment_length == 0 {
                    current_start += 1;
                    continue;
                }
...
impl<const ORDER: usize> BuddyFrameAllocator<ORDER> {
    ...
    fn decrease_local_cache(
        ...
        let mut central_free_list = self.central_free_lists.lock();
    ...
    pub fn rescue_local_cache(
        ...
        assert!(class < ORDER);
        let new_frame_head: usize =
            if let Some(frame_idx) = self.central_free_lists.lock().alloc(CPU_LOCAL_PAGE_COUNT) {
                frame_idx
            } else {
    ...
    pub fn alloc(&self, count: usize) -> Option<usize> {
        ...
        if class >= THRESHOLD {
            ...
            let frame = self.central_free_lists.lock().alloc(count)?;
    ...
    pub fn dealloc(&self, start_frame: usize, count: usize) {
        ...
        if class >= THRESHOLD {
            self.central_free_lists.lock().dealloc(start_frame, count);
