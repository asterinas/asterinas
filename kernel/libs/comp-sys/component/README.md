...
2. **Kthread**: The kernel thread stage, initialized after SMP is enabled and the first kernel thread is spawned. This stage runs in the context of the first kernel thread on the BSP and initializes subsystems that require kernel thread support, such as work queues, network stack, and filesystem components.
3. **Process**: The process stage, initialized after the first user process is created. This stage runs in the context of the first user process and initializes components that require full process functionality, and prepares the system for user-space execution.
