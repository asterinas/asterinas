...
use crate::{
    ext::Ext,
    inet_hashtable::{ConnectionKey, ListenerKey, SocketTable},
...
impl<E: Ext> PollContext<'_, E> {
    ...
    fn process_tcp(
        ...
        // Process connection request at first
        if tcp_repr.control == TcpControl::Syn && tcp_repr.ack_number.is_none() {
            let listener_key = ListenerKey::from((ip_repr.dst_addr(), tcp_repr.dst_port));
        ...
        // Process normal connection
        let connection_key = ConnectionKey::from((
            ip_repr.dst_addr(),
            tcp_repr.dst_port,
            ip_repr.src_addr(),
            tcp_repr.src_port,
