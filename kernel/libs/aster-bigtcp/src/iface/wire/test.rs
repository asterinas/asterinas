...
use crate::packet::{
    ApplicationLayer, FreshTxPacket, Layer, NetworkLayer, RxPacket, TransportLayer, TxPacket,
};

fn tx_packet<L: Layer>(payload: &[u8]) -> TxPacket<L> {
    let allocated = FreshTxPacket::<L>::alloc(payload.len()).unwrap();
    let mut builder = allocated.to_builder();
    let written = builder.append_writer().write(&mut VmReader::from(payload));
...
fn ethernet_round_trip() {
    ...
    let packet = ether::emit(tx_packet::<NetworkLayer>(&payload), &repr)
        .to_rx_packet()
        .unwrap();
...
fn arp_round_trip() {
    ...
    let packet = arp::emit(tx_packet::<TransportLayer>(&[]), &repr)
        .to_rx_packet()
        .unwrap();
...
fn udp_ipv4_round_trip() {
    ...
    let packet = ip::emit(packet, &ip_repr, true).to_rx_packet().unwrap();
