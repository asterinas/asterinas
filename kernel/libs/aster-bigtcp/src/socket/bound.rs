...
use crate::iface::{Ext, Iface};

pub struct BoundSocket<T: AnySocket<E>, E>(Arc<BoundSocketInner<T, E>>);
...
impl<T: AnySocket<E>, E> Drop for BoundSocket<T, E> {
    fn drop(&mut self) {
        T::on_drop(&self.0);
    }
}
...
impl<T: AnySocket<E>, E> BoundSocket<T, E> {
    pub(crate) fn new(
        iface: Arc<dyn Iface<E>>,
        port: u16,
