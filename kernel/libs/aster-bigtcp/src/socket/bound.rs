...
use crate::iface::{BindPortConfig, BoundPort, Ext, Iface};
...
impl<E: Ext> RawTcpSocketExt<E> {
    fn on_new_state(&mut self, this: &KeyableArc<TcpConnectionBg<E>>) -> SocketEvents {
        ...
        TcpConnectionInner::update_dead(this, self);
...
impl<E: Ext> TcpConnectionInner<E> {
    ...
    fn update_dead(this: &KeyableArc<TcpConnectionBg<E>>, socket: &RawTcpSocketExt<E>) {
        ...
        if socket.state() == smoltcp::socket::tcp::State::Listen {
            this.inner.is_dead.store(true, Ordering::Relaxed);

            if let Some(ref listener) = socket.listener {
                let mut backlog = listener.inner.lock();
...
impl<E: Ext> Inner<E> for TcpConnectionInner<E> {
    ...
    fn on_drop(this: &KeyableArc<SocketBg<Self, E>>) {
        ...
        TcpConnectionInner::update_dead(this, &socket);
    }
}
...
impl<E: Ext> TcpConnection<E> {
    ...
    pub fn new_connect(
        ...
        let this = Self::new(bound, inner);
        this.0.update_next_poll_at_ms(PollAt::Now);
        this.init_observer(observer);
        this.iface()
            .common()
            .register_tcp_connection(this.inner().clone());
    ...
    pub fn into_bound(mut self) -> Option<BoundPort<E>> {
...
impl<E: Ext> TcpListener<E> {
    ...
    pub fn new_listen(
        ...
        this.iface()
            .common()
            .register_tcp_listener(this.inner().clone());
...
impl<E: Ext> UdpSocket<E> {
    ...
    pub fn new_bind(
        ...
        this.iface()
            .common()
            .register_udp_socket(this.inner().clone());
...
impl<E: Ext> TcpConnectionBg<E> {
    /// Returns whether the TCP connection is dead.
    ///
    /// A TCP connection is considered dead if and only if it is closed.
