...
pub fn sys_fcntl(fd: FileDesc, cmd: i32, arg: u64) -> Result<SyscallReturn> {
    ...
    match fcntl_cmd {
        ...
        FcntlCmd::F_GETOWN => handle_getown(fd),
        FcntlCmd::F_SETOWN => handle_setown(fd, arg),
    }
}
...
fn handle_getlk(fd: FileDesc, arg: u64) -> Result<SyscallReturn> {
    ...
    write_val_to_user(lock_mut_ptr, &lock_mut_c)?;
    Ok(SyscallReturn::Return(0))
}
...
fn handle_getown(fd: FileDesc) -> Result<SyscallReturn> {
    let current = current!();
    let file_table = current.file_table().lock();
...
fn handle_setown(fd: FileDesc, arg: u64) -> Result<SyscallReturn> {
    let current = current!();
    let file_table = current.file_table().lock();
    let file_entry = file_table.get_entry(fd)?;
