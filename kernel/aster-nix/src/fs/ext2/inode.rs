...
impl Inner {
    ...
    pub fn read_direct_at(&self, offset: usize, buf: &mut [u8]) -> Result<usize> {
        ...
        self.inode_impl.read_blocks_sync(start_bid, &segment)?;
    ...
    pub fn write_direct_at(&mut self, offset: usize, buf: &[u8]) -> Result<()> {
        ...
        self.inode_impl.write_blocks_sync(start_bid, &segment)
    }
...
impl InodeImpl_ {
    ...
    pub fn read_blocks_sync(&self, bid: Ext2Bid, blocks: &Segment) -> Result<()> {
    ...
    pub fn write_blocks_sync(&self, bid: Ext2Bid, blocks: &Segment) -> Result<()> {
...
impl InodeImpl {
    ...
    pub fn read_blocks_sync(&self, bid: Ext2Bid, blocks: &Segment) -> Result<()> {
        self.0.read().read_blocks_sync(bid, blocks)
    }
    ...
    pub fn write_blocks_sync(&self, bid: Ext2Bid, blocks: &Segment) -> Result<()> {
        self.0.read().write_blocks_sync(bid, blocks)
    }
    ...
    pub fn sync_data_holes(&self) -> Result<()> {
        ...
        for (start_bid, batch_len) in data_hole_batches {
            inner.write_blocks_sync(start_bid, &zeroed_segment.range(0..batch_len))?;
        }
        Ok(())
    }
