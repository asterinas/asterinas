    ...
    fn set_device_range(&mut self, start_bid: u32, device_range: Range<u32>) -> Result<()> {
        match BidPath::from(start_bid) {
            ...
            BidPath::Indirect(idx) => {
                ...
                let indirect_block = self.indirect_blocks.find_mut(indirect_bid)?;
                for (i, bid) in device_range.enumerate() {
                ...
                BidPath::DbIndirect(lvl1_idx, lvl2_idx) => {
                    ...
                    if self.desc.block_ptrs.db_indirect() == 0 {
                        ...
                        let db_indirect_block = self
                            .indirect_blocks
                            .find_mut(self.desc.block_ptrs.db_indirect())?;
                    ...
                    if self.desc.block_ptrs.tb_indirect() == 0 {
                        ...
                        let lvl1_indirect_bid = {
                            let tb_indirect_block = self
                                .indirect_blocks
                                .find(self.desc.block_ptrs.tb_indirect())?;
                            tb_indirect_block.read_bid(lvl1_idx)?
                        };
                        ...
                        if lvl1_indirect_bid == 0 {
                            let tb_indirect_block = self
                                .indirect_blocks
                                .find_mut(self.desc.block_ptrs.tb_indirect())?;
...
impl<'a> DeviceRangeReader<'a> {
    ...
    pub fn new(inode: &'a mut InodeImpl_, range: Range<u32>) -> Result<Self> {
    ...
    fn update_indirect_block(&mut self) -> Result<()> {
        ...
        match bid_path {
            ...
            BidPath::Indirect(_) => {
                ...
                let indirect_block = self.inode.indirect_blocks.find(indirect_bid)?;
            ...
            BidPath::DbIndirect(lvl1_idx, _) => {
                ...
                let lvl1_indirect_block = self.inode.indirect_blocks.find(lvl1_indirect_bid)?;
            ...
            BidPath::TbIndirect(lvl1_idx, lvl2_idx, _) => {
                ...
                let lvl2_indirect_block = self.inode.indirect_blocks.find(lvl2_indirect_bid)?;
...
impl InodeImpl {
    ...
    pub fn set_device_id(&self, device_id: u64) {
        self.0.write().desc.block_ptrs.as_payload_mut()[..core::mem::size_of::<u64>()]
    ...
    pub fn device_id(&self) -> u64 {
        ...
        device_id.as_bytes_mut().copy_from_slice(
            &self.0.read().desc.block_ptrs.as_payload()[..core::mem::size_of::<u64>()],
        );
        device_id
    }
    ...
    pub fn write_link(&self, target: &str) -> Result<()> {
        let mut inner = self.0.write();
        inner.desc.block_ptrs.as_payload_mut()[..target.len()].copy_from_slice(target.as_bytes());
    ...
    pub fn read_link(&self) -> Result<String> {
        ...
        symlink.copy_from_slice(&inner.desc.block_ptrs.as_payload()[..inner.desc.size]);
        Ok(String::from_utf8(symlink)?)
    }
