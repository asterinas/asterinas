...
///  * **Mapping support.** Given a VMO, one can create a child with `MapRange` right
///    from it. The child VMO can be used for mapping and can only access the range indicated
///    by the `MapRange` of the parent's memory. This child VMO only have rights to duplicate
///    and get the committed frame from the VMO, which is a good thing for the perspective of
///    access control.
...
///
/// VMOs are implemented with three flavors of capabilities:
/// the dynamic one (`Vmo<Rights>`) and the static one (`Vmo<R: TRights>),
/// and an additional capability for child mapped VMO (`Vmo<MapRange>`).
///
/// # Examples
///
...
/// Should only be used as the generic parameter in `Vmo`.
pub type MapRange = Range<usize>;

impl Vmo<MapRange> {
    /// Returns whether a page is already committed.
    pub fn is_page_committed(&self, page_idx: usize) -> bool {
        debug_assert!(self.1.contains(&(page_idx * PAGE_SIZE)));

        self.0.is_page_committed(page_idx)
    }

    /// Gets the committed frame at the input `page_idx` in the VMO.
    ///
    ...
    pub fn get_committed_frame(&self, page_idx: usize) -> Result<Frame> {
        ...
        self.0.commit_page(page_idx * PAGE_SIZE)
    }
...
impl<R> Vmo<R> {
    ...
    pub(super) fn alloc_child_for_mapping(&self, range: Range<usize>) -> Result<Vmo<MapRange>> {
        Ok(Vmo(self.0.clone(), MapRange::from(range)))
    }
}
