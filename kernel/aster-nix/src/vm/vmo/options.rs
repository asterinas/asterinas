...
use super::{Pager, Vmo, VmoFlags, VmoMark, VmoRightsOp};
...
fn alloc_child_vmo_(
    ...
    let child_pages = parent_vmo_.clone_pages_for_child(child_type, &range)?;
    let new_vmo = Vmo_ {
        pager: parent_vmo_.pager.clone(),
        flags: child_flags,
        pages: child_pages,
        page_idx_offset: parent_page_idx_offset + parent_vmo_.page_idx_offset(),
        size: AtomicUsize::new(range.len()),
    };
    Ok(new_vmo)
}
