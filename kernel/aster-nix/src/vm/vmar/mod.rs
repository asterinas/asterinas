...
impl Vmar_ {
    ...
    fn new_cow(&self, parent: Option<&Arc<Vmar_>>) -> Result<Arc<Self>> {
        let new_vmar_ = {
            ...
            // If this is not a root vmar, we clone the vm space from parent.
            //
            // If this is a root vmar, we leverage Copy-On-Write (COW) mechanism to clone the `VmSpace` to the child.
            // We set all the page table entries in current `VmSpace` to be read-only, then clone the `VmSpace` to the child.
            // In this way, initially, the child shares the same page table contents as the current Vmar.
            // Later on, whether the current Vmar or the child Vmar needs to perform a write operation, the COW mechanism will be triggered, creating a new page for writing.
