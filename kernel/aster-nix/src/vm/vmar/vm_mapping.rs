...
use ostd::mm::{
    vm_space::VmQueryResult, CachePolicy, Frame, FrameAllocOptions, PageFlags, PageProperty,
    VmSpace,
};
...
/// mapping is a filebacked mapping. Yet there are also some situations where specific pages
...
impl VmMapping {
    pub fn try_clone(&self) -> Result<Self> {
        ...
        let vmo = self.vmo.as_ref().map(|vmo| vmo.dup());
        Ok(Self {
...
impl VmMapping {
    pub fn build_mapping<R1, R2>(option: VmarMapOptions<R1, R2>) -> Result<Self> {
        ...
        let (vmo, vmo_offset) = {
            if let Some(vmo) = vmo {
                (
                    Some(vmo.alloc_child_for_mapping(vmo_offset..vmo_limit)?),
    ...
    pub fn vmo(&self) -> Option<&Vmo<MapRange>> {
        self.vmo.as_ref()
    }
    ...
    pub fn handle_page_fault(
        ...
        if !self.is_shared && write && !not_present {
            ...
            if let VmQueryResult::Mapped {
                va: _,
                frame,
                mut prop,
            } = cursor.query().unwrap()
            {
                let new_frame = Frame::new_from(&frame)?;
    ...
    pub(super) fn new_fork(&self, new_parent: &Arc<Vmar_>) -> Result<VmMapping> {
        ...
        Ok(VmMapping {
            ...
            vmo: self.vmo.as_ref().map(|vmo| vmo.dup()),
            is_shared: self.is_shared,
        })
    }
