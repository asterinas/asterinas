...
impl VmMapping {
    ...
    pub fn write_bytes(&self, offset: usize, buf: &[u8]) -> Result<()> {
        ...
        for page_idx in page_idx_range {
            ...
            // When the VmMapping have the write permission but the corresponding PTE is present and is read-only,
            // It means this PTE is set to read-only due to the COW mechanism. In this situation we need to trigger a
            // page fault before writing at the Vmo to guarantee the consistency between Vmo and the PageTable.
