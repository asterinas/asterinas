...
//! and lets [`ClassInterface`]s subscribe to membership changes.
...
use crate::{
    Error, Result, SysStr,
    attr::Attr,
    device::{AnyDevice, ClassDevice, DevNode},
...
///
/// This is for code that must act on *every* member of a class without being the code that creates them.
/// Without it, each such consumer has to be called by hand from every producer,
/// which is why each subsystem in Asterinas keeps a device list of its own
/// (`aster_block::DEVICE_REGISTRY`, `aster_console::console_device_table`, `EVDEV_DEVICES`).
/// An interface inverts that: the class is the one place that knows its members,
/// and a consumer subscribes to it.
///
/// One subsystem has already built this privately:
/// `aster-input`'s `InputHandlerClass` is a class interface under another name, replayed in both directions,
/// and it is how evdev gets an `/dev/input/eventN` for every input device
/// without virtio-input calling it.
/// The two that have not:
///
/// - **A layer built on top of a class.**
///   When a disk joins the `block` class,
///   something has to read its partition table and register a device per partition.
///   That code belongs to the partition layer, not to virtio-blk or NVMe,
///   and it must run for disks from every driver.
/// - **A chooser.**
///   The console picks among the members of the `tty` class by walking a list;
///   it would rather be told as they appear.
///
/// [`ClassHandle::register_interface`] replays `add_dev` for the members that already exist,
/// and [`ClassHandle::unregister_interface`] replays `remove_dev` for those still present.
/// That is what makes initialization order free:
/// the partition layer sees the same set of disks whether it starts before or after the disk drivers,
/// and it sees each disk exactly once.
///
/// Callbacks hold the class's membership lock.
/// To avoid deadlocks, they must not change this class's devices or interfaces,
...
///
/// Linux's counterpart is `class_interface_register`,
/// which the SCSI core uses this way through `scsi_register_interface`, under the same per-class lock.
...
pub struct ClassHandle<C: Class> {
    ...
    /// Serializes membership changes with interface registration,
    /// so that an interface sees each member exactly once.
    /// Interface callbacks run under it; see `ClassInterface` for restrictions.
    membership: Mutex<()>,
}
...
impl<C: Class> ClassHandle<C> {
    ...
    ///
    /// The callback restrictions in [`ClassInterface`] apply during replay.
    pub fn register_interface(&self, interface: Arc<dyn ClassInterface<C>>) -> Result<()> {
        let _guard = self.membership.lock();
        {
            let mut interfaces = self.interfaces.lock();
            if interfaces.iter().any(|i| Arc::ptr_eq(i, &interface)) {
