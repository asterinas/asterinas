...
//!
//! An attribute is declared once, statically, against the concrete device
//! type it belongs to ([`Attr<D>`]). Several layers contribute attributes to
//! one device at different times: the device model itself (`uevent`, `dev`),
//! the bus or class, the device type, the device, and, after binding, the
//! driver. The device keeps them in an [`AttrTable`], where each entry has
//! been erased to operate on `&dyn AnyDevice` so that the table need not know
//! the concrete type. The erasure is done by the layer that declared the
//! attribute, which does know the type, so the downcast inside can only fail
//! if a table is fed attributes of the wrong device type, which the typed
//! entry points make impossible.
...
impl AttrTable {
    ...
    pub(crate) fn store(
        ...
        store(dev, &text).map_err(aster_systree::Error::from)?;
        Ok(len)
    }
}
