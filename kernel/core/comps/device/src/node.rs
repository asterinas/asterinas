...
use aster_systree::{
    ...
    SysNodeId, SysNodeType, SysObj, SysPerms, inherit_sys_symlink_node, relative_path,
};
...
impl Dir {
    ...
    /// built. Only a device's attributes come and go, and a device keeps them
    /// in an [`AttrTable`](crate::attr::AttrTable) rather than here.
    ///
    /// The callbacks that serve the files are installed separately, by
    /// [`Self::set_ops`], because they usually belong to an object that needs
    /// this directory to exist first. Until they are, reading or writing one
    /// of the files fails with `NotFound`.
    ...
    ///
    /// Has no effect after the first call. Must happen before the directory is
...
impl SysNode for Dir {
    ...
    fn write_attr(&self, name: &str, reader: &mut VmReader) -> aster_systree::Result<usize> {
        ...
        ops.store(name, &text).map_err(aster_systree::Error::from)?;
        Ok(len)
    }
    ...
    fn read_attr_at(
        ...
        ops.show(name, &mut printer)
            .map_err(aster_systree::Error::from)?;
