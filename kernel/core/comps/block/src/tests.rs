...
impl Drop for TestRegistrations {
    fn drop(&mut self) {
        // Remove remaining entries before releasing the major ID, including on unwind.
        for &id in &self.ids {
            // The test may have already unregistered this device explicitly.
            let _ = unregister(id);
        }
    }
}
...
fn request_handlers_follow_device_registration() {
    ...
    let mut registrations = TestRegistrations {
        major,
        ids: Vec::with_capacity(devices.len()),
    };
    register_with_request_handler(devices[2].clone()).unwrap();
    registrations.ids.push(devices[2].id);
    register(devices[1].clone()).unwrap();
    registrations.ids.push(devices[1].id);
    register_with_request_handler(devices[0].clone()).unwrap();
    registrations.ids.push(devices[0].id);
    register_with_request_handler(devices[3].clone()).unwrap();
    registrations.ids.push(devices[3].id);
    ...
    let handlers: Vec<_> = collect_request_handlers()
        .into_iter()
        .filter(|handler| handler.id().major() == registrations.major.get())
        .collect();
    ...
    assert!(
        collect_request_handlers()
            .iter()
            .all(|handler| handler.id().major() != registrations.major.get())
    );
