...
impl SysAttrSet {
    /// The size of the attribute ID space, including reserved ID 0.
    pub const ID_CAPACITY: usize = 1 << u8::BITS;
...
impl SysAttrSetBuilder {
    ...
    pub fn new() -> Self {
        let mut ids = IdAlloc::with_capacity(SysAttrSet::ID_CAPACITY);
        // Reserve ID 0 to avoid sharing an inode number with the containing directory.
        const DOT_ID: usize = 0;
        ids.alloc_specific(DOT_ID).unwrap();
