...
impl Mount {
    ...
    pub(super) fn attach_mount_tree_with_propagation(
        ...
        for (receiver, copy) in copies {
            // Every receiver mounts the same filesystem, and the reachability checks above
            // ensure that this dentry is within its root. It can therefore be paired with the
            // receiver to form the corresponding path.
