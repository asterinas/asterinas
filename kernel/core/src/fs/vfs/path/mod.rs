...
impl Path {
    ...
    /// Use [`Self::new_fs_symlink_child`] to create a symbolic link, because a
    ...
    pub(crate) fn new_fs_symlink_child(
        &self,
        name: &str,
        target: &str,
        mode: InodeMode,
        ...
        let dir_dentry = self.dentry.as_dir_dentry_or_err()?;
        self.check_dir_entry_mutation()?;
        let new_child_dentry = dir_dentry.symlink(name, target, mode)?;
        Ok(Self::new(self.mount.clone(), new_child_dentry))
    }
