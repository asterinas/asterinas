...
impl RealObject {
    /// This real object's mount-level layer index; an upper's is the constant `UPPER_LAYER_INDEX`.
    ...
    /// Status flags: a lower read carries `O_NOATIME`, so a read never updates a lower's atime.
...
pub(in overlayfs) fn read_all_dirents(
