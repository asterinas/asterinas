...
pub(super) struct MountOptions {
    ...
    pub(super) upper_workdir: Option<UpperWorkdirOptions>,
...
#[derive(Debug)]
pub(super) struct UpperWorkdirOptions {
...
impl MountOptions {
    pub(super) fn parse(args: Option<&str>, fs_flags: FsFlags) -> Result<Self> {
        ...
        let mut upper_dir: Option<String> = None;
        ...
        for entry in args.split(',') {
            ...
            match entry.split_once('=') {
                Some(("upperdir", value)) => Self::set_layer_root_option(
                    &mut upper_dir,
                    value,
                    "duplicate overlay mount option `upperdir`",
                    "the `upperdir` mount option requires a non-empty value",
                )?,
                Some(("workdir", value)) => Self::set_layer_root_option(
                    &mut workdir,
                    value,
                    "duplicate overlay mount option `workdir`",
                    "the `workdir` mount option requires a non-empty value",
                )?,
                Some((key, value)) => options.set_key_value(
                    key,
                    value,
                    &mut is_lowerdir_seen,
                    &mut is_lowerdir_plus_seen,
                    &mut is_nooverride_creds_seen,
                )?,
                None => options.set_entry(entry, &mut is_nooverride_creds_seen)?,
            }
        }
        ...
        options.upper_workdir = match (upper_dir, workdir) {
            (Some(upper_dir), Some(workdir)) => Some(UpperWorkdirOptions { upper_dir, workdir }),
            (None, None) => None,
            _ => {
                return_errno_with_message!(
                    Errno::EINVAL,
    ...
    fn set_key_value(
        ...
        key: &str,
        value: &str,
        is_lowerdir_seen: &mut bool,
        is_lowerdir_plus_seen: &mut bool,
        is_nooverride_creds_seen: &mut bool,
    ) -> Result<()> {
        match key {
            "lowerdir" => {
                if *is_lowerdir_seen {
            ...
            "uuid" => {
                if self.uuid_mode.is_some() {
                    return_errno_with_message!(
                        Errno::EINVAL,
                        "duplicate overlay mount option `uuid`"
                    );
                }
                self.uuid_mode = Some(match value {
                    "off" => UuidMode::Off,
                    "null" => UuidMode::Null,
                    "on" => UuidMode::On,
                    "auto" => UuidMode::Auto,
            ...
            "xino" => {
                if self.xino_mode.is_some() {
                    return_errno_with_message!(
                        Errno::EINVAL,
                        "duplicate overlay mount option `xino`"
                    );
                }
                self.xino_mode = Some(match value {
                    "off" => XinoMode::Off,
                    "auto" => XinoMode::Auto,
                    "on" => XinoMode::On,
                    _ => {
            ...
            "redirect_dir" => {
                if self.redirect_dir.is_some() {
                    return_errno_with_message!(
                        Errno::EINVAL,
                        "duplicate overlay mount option `redirect_dir`"
                    );
                }
                self.redirect_dir = Some(match value {
            ...
            "nfs_export" => {
                if self.nfs_export.is_some() {
                    return_errno_with_message!(
                        Errno::EINVAL,
                        "duplicate overlay mount option `nfs_export`"
                    );
                }
                self.nfs_export = Some(match value {
                    "on" => true,
                    "off" => false,
                    _ => {
                        return_errno_with_message!(
            ...
            "metacopy" => {
                if self.metacopy.is_some() {
                    return_errno_with_message!(
                        Errno::EINVAL,
                        "duplicate overlay mount option `metacopy`"
                    );
                }
                self.metacopy = Some(match value {
                    "on" => true,
                    "off" => false,
                    _ => {
                        return_errno_with_message!(
            ...
            "fsync" => {
                // Only `auto` is honored: copy-up already syncs data before publication.
                if self.fsync_mode.is_some() {
                    return_errno_with_message!(
                        Errno::EINVAL,
                        "duplicate overlay mount option `fsync`"
                    );
                }
                self.fsync_mode = Some(match value {
            ...
            "datadir+" => {
                // `datadir+` is rejected: data-only layers are unsupported.
                return_errno_with_message!(
                    Errno::EINVAL,
    ...
    /// Records one paired raw option, rejecting duplicates and empty values.
    fn set_layer_root_option(
        slot: &mut Option<String>,
        value: &str,
        duplicate_msg: &'static str,
        empty_value_msg: &'static str,
    ) -> Result<()> {
        if slot.is_some() {
            return_errno_with_message!(Errno::EINVAL, duplicate_msg);
        }
        Self::require_non_empty_value(value, empty_value_msg)?;
        *slot = Some(value.to_string());
        Ok(())
    }

    fn set_entry(&mut self, entry: &str, is_nooverride_creds_seen: &mut bool) -> Result<()> {
        match entry {
            "lowerdir" | "lowerdir+" | "upperdir" | "workdir" | "uuid" | "xino"
            | "redirect_dir" | "index" | "nfs_export" | "metacopy" | "verity" | "fsync" => {
                return_errno_with_message!(
                    Errno::EINVAL,
                    "the `{entry}` mount option requires a value"
                );
            }
            "default_permissions" => {
                if self.is_default_permissions {
                ...
                self.is_default_permissions = true;
            }
            "userxattr" => {
                if self.is_userxattr {
                    return_errno_with_message!(
            ...
            "volatile" => {
                ...
                self.fsync_mode = Some(FsyncMode::Volatile);
            }
            "nooverride_creds" => {
...
mod test {
    ...
    fn parse_upperdir_workdir_pairing() {
        ...
        let pair = options.upper_workdir.as_ref().unwrap();
        assert_eq!(pair.upper_dir, "u");
        assert_eq!(pair.workdir, "w");
    }
