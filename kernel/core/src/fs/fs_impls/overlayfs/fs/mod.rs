...
//! ## References
//!
//! - <https://elixir.bootlin.com/linux/v7.0/source/fs/overlayfs/super.c>
//!   (Linux `ovl_sync_fs` syncs only the upper filesystem and returns early when there is nothing to sync)
//! - <https://elixir.bootlin.com/linux/v7.0/source/fs/overlayfs/util.c>
//!   (Linux `ovl_sync_status` reports a mount without an upper as having nothing to sync)
...
impl OverlayFs {
    /// The claim this mount was built on; only a mount given an upper and workdir has one.
...
impl FileSystem for OverlayFs {
    ...
    fn sync(&self) -> Result<()> {
        match self.layer_stack.upper.as_ref() {
