...
//! effectively read-only, the `xino`/UUID modes, the selected private-xattr
//! prefix, and the effective overlay UUID. The upper-filesystem capabilities
//! are measured during mount construction and published here as fixed state;
//! the policy performs no probing itself.
...
impl MountPolicy {
    ...
    /// It is part of the persistent-identity condition that `xino=auto` requires.
