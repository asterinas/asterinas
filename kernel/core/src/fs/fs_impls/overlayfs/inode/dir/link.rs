...
impl OverlayInode {
    pub(super) fn link_source(
        ...
        Ok(upper.dentry().clone())
    }

    pub(super) fn link_over_whiteout(&self, name: &str, source: &Arc<Dentry>) -> Result<()> {
        let fs = self.fs_arc();
        let upper_parent = self.upper_parent_dentry()?;
        let temp = fs.create_workdir_link_temp(name, source)?;
        if let Err(err) = fs.publish_temp(&temp, upper_parent, name, RenameMode::Replace) {
