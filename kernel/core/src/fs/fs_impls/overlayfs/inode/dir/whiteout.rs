...
use crate::{
    fs::{
        ...
        fs_impls::overlayfs::{
            ...
            inode::{
                CreateOp, copyup::workdir::WorkdirTemp, is_whiteout_inode, xattr::OverlayXattrType,
            },
...
impl WhiteoutCache {
    ...
    pub(in overlayfs) fn with_shared_whiteout(
        ...
        let shared = match whiteout_representation(capability) {
            Ok(representation) => {
                create_whiteout_handle(upper_workdir, representation, namespace).ok()
            }
            Err(_) => None,
        };
        Self {
...
fn create_whiteout_handle(
    ...
    match representation {
        WhiteoutRepresentation::CharDevice => {
            let temp = upper_workdir.create_workdir_temp(
...
impl WorkdirTemp {
    ...
    fn into_whiteout(self) -> WhiteoutHandle {
        WhiteoutHandle {
...
fn is_whiteout_child(
    ...
    is_whiteout_inode(&child, namespace)
}
