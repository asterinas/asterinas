...
impl OverlayInode {
    pub(super) fn set_mode_impl(&self, self_dentry: &Dentry, mode: InodeMode) -> Result<()> {
        ...
        self.check_mutating_permission(self_dentry, Permission::empty())?;
    ...
    pub(super) fn set_group_impl(&self, self_dentry: &Dentry, gid: Gid) -> Result<()> {
        ...
        if gid != metadata.gid {
            ...
            // Owner-chgrp exemption: the owner may change the group to a supplementary group.
...
impl OverlayInode {
    fn best_effort_time_set(
        ...
        let has_cap = current_task_has_capability(CapSet::FOWNER);
        if self
            .check_mutating_permission(self_dentry, Permission::MAY_WRITE)
            .is_err()
        {
            if !is_owner && !has_cap {
                return;
            ...
            if self
                .check_mutating_permission(self_dentry, Permission::empty())
                .is_err()
            {
                return;
            }
