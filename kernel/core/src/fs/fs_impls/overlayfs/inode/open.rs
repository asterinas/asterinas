...
use crate::{
    ...
    fs::{
        file::{
            AccessMode, InodeType, Mappable, PerOpenFileOps, SettableStatusFlags, StatusFlags,
            SyncMode,
        },
        utils::DirentVisitor,
        vfs::{
...
pub(super) struct OverlayDirOpenHandle {
    /// The directory this handle iterates; also the owner of the snapshot's transaction lock.
    inode: Arc<OverlayInode>,
    /// This open file's frozen view; `None` means "no snapshot yet", not "an empty snapshot".
    readdir_cache_snapshot: Mutex<Option<Arc<ReaddirCache>>>,
    /// The `..` identity captured at open: the parent's inode, or this inode at the mount root.
    parent: Arc<dyn Inode>,
}

impl OverlayDirOpenHandle {
    pub(super) fn new(inode: Arc<OverlayInode>, self_dentry: &Dentry) -> Self {
        let parent: Arc<dyn Inode> = match self_dentry.parent() {
            Some(parent) => parent.inode().clone(),
    ...
    fn emit_entries(&self, offset: usize, visitor: &mut dyn DirentVisitor) -> Result<usize> {
        ...
        if offset < 2 {
            ...
            emit_item("..", Some(self.parent.ino()), InodeType::Dir, 2);
        }
