...
impl FileOps for VirtioFsInode {
    ...
    fn readdir_at(
        ...
        // This transient handle is opened by the kernel and has no owning file
        // description, so the request carries whatever `status_flags` the
        // caller passed. The VFS readdir path goes through `VirtioFsDir`
        // instead, which holds a real per-open handle and its live flags.
