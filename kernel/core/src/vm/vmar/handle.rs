...
pub(crate) struct VmarHandle(Arc<Vmar>);
...
impl VmarHandle {
    ...
    pub(crate) fn new(process_vm: ProcessVm) -> Result<Self> {
    ...
    pub(crate) fn clone_arc(&self) -> Arc<Vmar> {
        self.0.clone()
    }

    /// Clones the VMAR object with a weak reference count.
    pub(crate) fn clone_weak(&self) -> Weak<Vmar> {
