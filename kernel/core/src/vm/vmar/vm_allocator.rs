...
impl VirtualAddressAllocator {
    ...
    /// Creates an allocator with empty free-range hints for a forked VMAR.
    ///
    /// The hints are only a cache. Starting empty avoids taking the parent's
    /// allocator locks under its PT cursor (the inverse of mmap's lock order).
    /// The first allocation searches the copied PT and repopulates the cache.
    pub fn new_for_fork() -> Result<Self> {
        let must_be_free = alloc_cpu_local(|_| SpinLock::new(0..0))?;
        Ok(Self { must_be_free })
    }
    ...
    pub fn alloc_and_lock<'a>(
        ...
        if let Ok((va, alloc_guard)) = self.alloc_from_cpu_must_free(cur_cpu, size, align) {
            let mut cursor = vmspace.cursor_mut(guard, &(va..va + size))?;
            if cursor.find_next_mapped(va + size).is_none() {
                return Ok((va, alloc_guard, cursor));
            }
            // The per-CPU range is only a cache. Keep its conservative
            // truncation and retry using authoritative page-table metadata.
            drop(cursor);
            drop(alloc_guard);
        }
    ...
    /// Allocates and locks a virtual address range within the given range.
    #[cfg(target_arch = "x86_64")]
    pub fn alloc_and_lock_in_range<'a>(
        &self,
        guard: &'a DisabledPreemptGuard,
        vmspace: &'a VmarSpace,
        range: &Range<Vaddr>,
        ...
        self.try_lock_and_alloc_from(guard, vmspace, range, size, align)
    }
    ...
    pub fn alloc_and_lock_covering_another<'a>(
            ...
            )
        {
            ...
            let mut cursor = vmspace.cursor_mut(guard, &lock_range)?;
            cursor.jump(va).unwrap();
            if cursor.find_next_mapped(va + size).is_none() {
                cursor.jump(lock_range.start).unwrap();
                return Ok((va, alloc_guard, cursor));
            }
            drop(cursor);
            drop(alloc_guard);
...
fn find_fit_range_in_cursor(
    ...
    while let Some(candidate) = search_end.checked_sub(size) {
        if candidate.align_down(align) < guard_range.start {
            break;
        }
...
mod tests {
    ...
    #[ktest]
    fn alloc_from_free_range_accepts_exact_fit() {
        assert_eq!(
            alloc_from_free_range(0x4000..0x8000, 0x4000, PAGE_SIZE).unwrap(),
            0x4000
        );
    }

    #[ktest]
    fn alloc_from_free_range_rejects_too_small_range() {
        assert!(alloc_from_free_range(0x4000..0x7000, 0x4000, PAGE_SIZE).is_err());
    }
}
