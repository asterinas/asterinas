...
use ostd::mm::{
    CachePolicy, HIGHEST_PAGING_LEVEL, PageFlags, PagingLevel, page_size_at, tlb::TlbFlushOp,
    vm_space::CursorMut,
};

use super::{
    RmapEntry, RssType, Vmar,
    interval_set::Interval,
    rmap_locks::RmapObject,
    util::is_intersected,
...
impl VmMapping {
    ...
    pub(super) fn add_frames_mapped(&mut self, num_frames: usize) {
        if let Some(frames_mapped) = self.frames_mapped.as_mut() {
            *frames_mapped += num_frames;
            debug_assert!(*frames_mapped <= self.map_size.get() / PAGE_SIZE);
        }
    }
    ...
    pub(super) fn clone_range_for_remap_at(&self, range: Range<Vaddr>, va: Vaddr) -> VmMapping {
        ...
        let frames_mapped = match self.frames_mapped {
            Some(0) => Some(0),
            Some(frames_mapped) if frames_mapped == self.map_size() / PAGE_SIZE => {
                Some(range_size / PAGE_SIZE)
            }
            // The exact count for a partially populated subrange is not known.
            _ => None,
        };
        VmMapping {
            map_size: NonZeroUsize::new(range_size).unwrap(),
            map_to_addr: va,
    ...
    /// Returns the reverse-mapping entry for the part intersecting `range`.
    pub(super) fn rmap_entry_in(&self, range: &Range<Vaddr>) -> Option<(RmapObject, RmapEntry)> {
        let object = self.mapped_mem.rmap_object(self.is_shared)?;
        let offset = match &self.mapped_mem {
            MappedMemory::Vmo(vmo) => vmo.offset(),
            MappedMemory::Device(dmo) => dmo.offset,
            MappedMemory::Anonymous => unreachable!(),
        };
        let start = self.map_to_addr.max(range.start);
        let end = self.map_end().min(range.end);
        if start >= end {
            return None;
        }
        ...
        Some((
            ...
            RmapEntry {
                vaddr: start,
                offset: offset + start - self.map_to_addr,
                size: end - start,
            },
        ))
    }
...
impl VmMapping {
    ...
    pub(crate) fn split(self, at: Vaddr) -> Result<(Self, Self)> {
        debug_assert!(self.map_to_addr < at && at < self.map_end());
        debug_assert!(at.is_multiple_of(PAGE_SIZE));

        let r_mapped_mem = self.mapped_mem.dup_at_offset(at - self.map_to_addr);
    ...
    pub(super) fn protect(self, cursor: &mut CursorMut<PerPtMeta>, perms: VmPerms) -> Self {
        ...
        let new_flags = if matches!(self.mapped_mem, MappedMemory::Device(_)) {
            // Device pages are driver-owned, not COW/page-cache pages. There
            // is no write-fault callback to restore their write permission.
        ...
        let set_flags_fn = |flags: &mut PageFlags, _cache: &mut CachePolicy| *flags = new_flags;
...
pub(super) enum MappedMemory {
    ...
    /// offset identifies the start of this mapping within the DMO.
    Device(MappedDmo),
}

/// A device object and the offset corresponding to this VMA's start.
#[derive(Clone, Debug)]
pub(super) struct MappedDmo {
    dmo: Arc<Dmo>,
    offset: usize,
}

impl MappedDmo {
    pub(super) fn new(dmo: Arc<Dmo>, offset: usize) -> Self {
        debug_assert!(offset.is_multiple_of(PAGE_SIZE));
        Self { dmo, offset }
    }

    pub(super) fn dmo(&self) -> &Arc<Dmo> {
        &self.dmo
    }
...
impl MappedMemory {
    pub(super) fn rmap_object(&self, is_shared: bool) -> Option<RmapObject> {
        match self {
            // Preserve the existing policy for private file mappings. Device
            // mappings, including MAP_PRIVATE, must always be invalidatable.
            Self::Vmo(vmo) if is_shared => Some(RmapObject::Vmo(vmo.vmo().clone())),
            Self::Device(dmo) => Some(RmapObject::Dmo(dmo.dmo().clone())),
            _ => None,
        }
    }
    ...
    pub(super) fn dup(&self) -> Self {
        match self {
            ...
            MappedMemory::Device(dmo) => MappedMemory::Device(dmo.clone()),
        }
    }
    ...
    fn dup_at_offset(&self, offset: usize) -> Self {
        ...
        match self {
            ...
            MappedMemory::Device(dmo) => MappedMemory::Device(MappedDmo {
                dmo: dmo.dmo.clone(),
                offset: dmo
                    .offset
                    .checked_add(offset)
                    .expect("mapped DMO offset should not overflow"),
            }),
        }
    }
}
...
impl MappedVmo {
    ...
    pub(super) fn dup_commit(&self, page_idx: usize, map_mode: VmoMapMode) -> VmoCommitHandle {
        VmoCommitHandle {
            vmo: self.vmo.clone(),
            page_idx,
            map_mode,
    ...
    pub(super) fn is_same_vmo(&self, other: &MappedVmo) -> bool {
...
impl VmoCommitHandle {
    pub(super) fn commit(self) -> Result<()> {
        self.vmo.commit_on(self.page_idx, self.map_mode)
    }
}
...
fn can_merge(left: &VmMapping, right: &VmMapping) -> bool {
    ...
    match (&left.mapped_mem, &right.mapped_mem) {
        ...
        (MappedMemory::Device(left_dmo), MappedMemory::Device(right_dmo)) => {
            Arc::ptr_eq(&left_dmo.dmo, &right_dmo.dmo)
                && left_dmo.offset + left.map_size() == right_dmo.offset
        }
        _ => false,
    }
}
...
fn try_merge(left: &VmMapping, right: &VmMapping) -> Option<VmMapping> {
    ...
    let mapped_mem = match (&left.mapped_mem, &right.mapped_mem) {
        ...
        (MappedMemory::Device(left_dmo), MappedMemory::Device(_)) => {
            MappedMemory::Device(left_dmo.clone())
        }
        _ => return None,
    };
