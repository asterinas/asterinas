...
impl VmMapping {
    ...
    pub(super) fn rmap_entry_in(&self, range: &Range<Vaddr>) -> Option<(Arc<Vmo>, RmapEntry)> {
        let vmo = self.vmo_for_rmap()?.clone();
        ...
        Some((
            ...
            RmapEntry {
                vaddr: start,
                offset: self.vmo().unwrap().offset() + start - self.map_to_addr,
                size: end - start,
            },
        ))
    }
