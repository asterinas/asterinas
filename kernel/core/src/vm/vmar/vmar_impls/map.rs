...
use super::{MappedMemory, MappedVmo, RmapTargets, RsAsDelta, VmMapping, Vmar};
use crate::{
    fs::{
    ...
    prelude::*,
    vm::{
        dmo::{MapOperation, PendingDmoMapping},
...
impl Vmar {
    ...
    ///     vm::{page_cache::VmoOptions, perms::VmPerms, vmar::OffsetType},
    /// };
    ///
    ...
    ///     .new_map(NonZeroUsize::new(PAGE_SIZE * 4).unwrap(), VmPerms::READ)
    ///     // Provide an optional offset for the mapping inside the VMAR
    ///     .offset(target_vaddr, OffsetType::FixedNoReplace)
    ...
    pub(crate) fn new_map(&self, size: NonZeroUsize, perms: VmPerms) -> VmarMapOptions<'_> {
...
impl<'a> VmarMapOptions<'a> {
    ...
    pub(crate) fn build(mut self) -> Result<Vaddr> {
        ...
        let (mapped_mem, pending) = self.prepare_mapped_memory()?;
        ...
        loop {
            ...
            let mut targets = RmapTargets::new(&old_rmap_entries);
            if let Some(object) = mapped_mem.rmap_object(is_shared) {
                targets.insert(object);
            }
            let mut rmap_locks = match targets.try_lock() {
                Ok(locks) => locks,
                Err(busy) => {
                    drop(cursor);
                    drop(alloc_guard);
            ...
            if let Some(pending) = pending {
                let operations = rmap_locks.take_pending(pending);
                let rss_type = vm_mapping.rss_type();
                let frames_mapped = map_device(&mut cursor, vm_mapping, operations);
    ...
    fn prepare_mapped_memory(&mut self) -> Result<(MappedMemory, Option<PendingDmoMapping>)> {
        ...
        let mem = match self.mappable.take() {
            ...
            Some(Mappable::Dmo(device)) => {
                if let Some(file) = &self.file
                    && !file.access_mode().is_writable()
                {
                    if self.perms.contains(VmPerms::WRITE) {
                        return_errno_with_message!(
                            Errno::EACCES,
                            "the device file is not opened writable"
                        );
                    }
                    new_may_perms.remove(VmPerms::MAY_WRITE);
                }
                // Driver-owned pages have no private-copy fault path. Do not
                // grant write access to the backing device through MAP_PRIVATE,
                // including a later mprotect on a read-only private mapping.
                if !self.is_shared {
                    if self.perms.contains(VmPerms::WRITE) {
                        return_errno_with_message!(
                            Errno::EINVAL,
                            "private writable device mappings are not supported"
                        );
                    }
                    new_may_perms.remove(VmPerms::MAY_WRITE);
                }
                let end = self
                    ...
                    .ok_or(Errno::EINVAL)?;
                let range = self.vmo_offset..end;
                let prepared = device.prepare_mapping(range.clone())?;
                if prepared.range() != &range {
                    return_errno_with_message!(
                        Errno::EINVAL,
                        "device prepared the wrong mapping range"
                    );
                }
                let mapping = MappedDmo::new(prepared.dmo().clone(), self.vmo_offset);
                pending = Some(prepared);
                MappedMemory::Device(mapping)
            }
...
fn allocate_range<'a>(
    ...
    let (map_to_addr, alloc_guard, cursor) = match offset {
        ...
        Some((offset, OffsetType::Map32Bit)) => {
            ...
            if allocation_range.contains(&offset) && map_size_bytes <= allocation_range.end - offset
            {
                let range = offset..offset + map_size_bytes;
...
pub(super) fn map_populate(cursor: &mut VmarCursorMut<'_>, vm_mapping: VmMapping) -> usize {
    ...
    for (mut mapping, level) in vm_mapping.split_for_pt(1) {
        ...
        match mapping.mapped_mem() {
            ...
            MappedMemory::Device(_) => unreachable!("device mappings use pending operations"),
        }
...
fn map_device(
    ...
    let MappedMemory::Device(dmo) = mapping.mapped_mem() else {
        unreachable!("pending operations require a device mapping");
    };
    let object_offset = dmo.offset();
    let map_addr = mapping.map_to_addr();
    let flags = PageFlags::from(mapping.perms()) | PageFlags::ACCESSED;
    // Create metadata before populating so subsequent PT splits propagate it.
    map_to_page_table(cursor, mapping);

    let mut count = 0;
    for operation in operations {
        let range = operation.range();
        let va = map_addr + range.start - object_offset;
        match operation {
            MapOperation::Frame(frame, _) => {
                cursor.jump(va).unwrap();
                cursor.adjust_level(1);
                let prop = PageProperty::new_user(flags, CachePolicy::Writeback);
                cursor.map(frame, prop);
                let Some(PteRangeMeta::VmMapping(mapping)) =
                    cursor.aux_meta_mut().inner.find_one_mut(&va)
                else {
                    unreachable!("device mapping metadata disappeared");
                };
                mapping.add_frames_mapped(1);
                count += 1;
            }
            MapOperation::IoMem(memory, _) => {
                let prop = PageProperty::new_user(flags, memory.cache_policy());
                for offset in (0..range.len()).step_by(PAGE_SIZE) {
                    let page = va + offset;
                    cursor.jump(page).unwrap();
                    cursor.adjust_level(1);
                    cursor.map_iomem(memory.clone(), prop, PAGE_SIZE, offset);
                    let Some(PteRangeMeta::VmMapping(mapping)) =
                        cursor.aux_meta_mut().inner.find_one_mut(&page)
                    else {
                        unreachable!("device mapping metadata disappeared");
                    };
                    mapping.add_frames_mapped(1);
