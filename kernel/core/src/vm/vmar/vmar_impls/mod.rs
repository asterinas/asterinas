...
use super::{
    ...
    rmap_locks::{RmapLocks, RmapObject, RmapTargets},
...
impl Vmar {
    ...
    fn snapshot_rmap_entries(
        ...
        for range in ranges {
            ...
            while let Some(mapping) = cursor.find_next_mapped(range.end) {
                ...
                if let Some((object, entry)) = mapping.rmap_entry_in(range)
                    && seen.insert((object.key(), entry.vaddr, entry.offset, entry.size))
                {
    ...
    ///
    /// All affected backing objects must have been try-locked before changing
    /// the page tables. Publication must not acquire any additional locks.
    fn refresh_rmap_entries(
        &self,
        cursor: &mut VmarCursorMut<'_>,
        locks: &mut RmapLocks<'_>,
        ranges: &[Range<Vaddr>],
    ) {
        let new_entries = Self::snapshot_rmap_entries(cursor, ranges);
        locks.refresh(self.weak_self.clone(), new_entries, ranges);
    }
}
