...
use super::{PteRangeMeta, RmapTargets, RsAsDelta, Vmar, VmarCursorMut};
use crate::{
    prelude::*,
    vm::vmar::{
        cursor::{
        ...
        vmar_impls::map::map_to_page_table,
    },
};
...
impl Vmar {
    ...
    pub fn resize_mapping(
        ...
        if old_size == 0 || new_size == 0 {
            return_errno_with_message!(Errno::EINVAL, "resize sizes must be nonzero");
        }
        if !is_userspace_vaddr_range(map_addr, old_size) {
            return_errno_with_message!(Errno::EFAULT, "resize source range is outside userspace");
        }
        if !is_userspace_vaddr_range(map_addr, new_size) {
            return_errno_with_message!(Errno::ENOMEM, "resize destination exceeds userspace");
        }
        ...
        loop {
            ...
            let (alloc_guard, mut cursor) = self.allocator.alloc_specific_and_lock(
                &preempt_guard,
                self.vm_space(),
                &range_to_lock,
            );
            ...
            let targets = RmapTargets::new(&old_rmap_entries);
            ...
            let last_mapping_addr = last_mapping.map_to_addr().max(map_addr);
            ...
            let vm_mapping = if vm_mapping.map_to_addr() < map_addr {
                let suffix = map_addr..vm_mapping.map_end();
                split_and_insert_rest(&mut cursor, vm_mapping, suffix)
            } else {
                vm_mapping
            };
    ...
    pub fn remap(
        ...
        if old_size == 0 || new_size == 0 {
            return_errno_with_message!(Errno::EINVAL, "remap sizes must be nonzero");
        }
        if !is_userspace_vaddr_range(old_addr, old_size) {
            return_errno_with_message!(Errno::EFAULT, "remap source range is outside userspace");
        }
        ...
        loop {
            ...
            let operation_result: Result<Vaddr> = (|| {
                ...
                if new_size > old_size && !last_mapping.can_expand() {
                    return_errno_with_message!(
                        Errno::EINVAL,
                        "remap: the mapping cannot be expanded"
                    );
                }

                drop(rs_as_delta);
                if let Some(range) = remap_op.new_mapping_range() {
                    // A move transfers the source charge; only a kept source
                    // requires charging the entire destination again.
                    let transferred = remap_op
                        .move_op
                        .as_ref()
                        .filter(|op| !op.keep_source)
                        .map_or(0, |op| op.size);
                    self.add_mapping_size(&preempt_guard, range.len() - transferred)?;
                }
            ...
            self.refresh_rmap_entries(&mut cursor, &mut rmap_locks, &affected_ranges);
    ...
    while let Some(va) = cursor.find_next(end) {
        ...
        match cursor.query() {
            ...
            VmQueriedItem::MappedIoMem { paddr, prop, level } => {
                let (memory, offset) = cursor.find_iomem_by_paddr(paddr).unwrap();
                cursor.jump(to_va).unwrap();
                cursor.map_iomem(memory, prop, page_size_at(level), offset);
            }
