...
use super::{MappedMemory, PerPtMeta, RmapTargets, Vmar};
use crate::{
    prelude::*,
    process::ProcessVm,
    vm::vmar::{
...
impl Vmar {
    ...
    pub(crate) fn fork_from(vmar: &Self) -> Result<VmarHandle> {
        ...
        loop {
            ...
            let entries =
                Self::snapshot_rmap_entries(&mut cur_cursor, core::slice::from_ref(&RANGE));
            let targets = RmapTargets::new(&entries);
            ...
            let allocator = VirtualAddressAllocator::new_for_fork()?;
            ...
            return Ok(VmarHandle::from_arc(new_vmar));
        }
    }
}
...
fn cow_copy_pt(src: &mut CursorMut<'_, PerPtMeta>, dst: &mut CursorMut<'_, PerPtMeta>) {
    while let Some(src_vm_mapping) = src.find_next_mapped(VMAR_CAP_ADDR) {
        ...
        let mode = if matches!(src_vm_mapping.mapped_mem(), MappedMemory::Device(_)) {
            CopyMode::Device
        } else {
        ...
        copy_mappings(src, dst, vm_mapping_range.end, mode);
...
/// The fork policy for page-table entries.
#[derive(Clone, Copy)]
enum CopyMode {
    CopyOnWrite,
    Device,
...
/// Copies PTEs, write-protecting COW pages while preserving device permissions.
fn copy_mappings(
    ...
    while let Some(mapped_va) = src.find_next(end) {
        let mapped_size = match src.query() {
            VmQueriedItem::MappedRam { frame, mut prop } => {
                ...
                if matches!(mode, CopyMode::CopyOnWrite) {
                    src.protect(op);
