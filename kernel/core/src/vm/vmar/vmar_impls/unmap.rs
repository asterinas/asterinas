...
use crate::{
    ...
    vm::vmar::{
        VMAR_CAP_ADDR,
        cursor::{CursorExt, propagate_if_needed, take_next_unmappable, unmap_count_rs_as},
...
impl Vmar {
    ...
    pub(super) fn clear(&self) {
        ...
        loop {
            ...
            let old_rmap_entries =
                Self::snapshot_rmap_entries(&mut cursor, core::slice::from_ref(&full_range));
            ...
            // Child page tables are reclaimed after an RCU grace period.
            // Extract their mappings now so file close notifications do not
            // depend on reclamation. Drop them outside the page-table locks.
            let mut mappings = Vec::new();
            while let Some(mapping) = cursor.find_next_mapped(full_range.end) {
                let start = mapping.map_to_addr();
                mappings.push(cursor.aux_meta_mut().inner.take_one(&start).unwrap());
            }

            cursor.jump(full_range.start).unwrap();
            cursor.adjust_level(cursor.guard_level());
    ...
    pub(crate) fn remove_mapping(&self, range: Range<usize>) {
        ...
        loop {
            ...
            let old_rmap_entries =
                Self::snapshot_rmap_entries(&mut cursor, core::slice::from_ref(&range));
            let targets = RmapTargets::new(&old_rmap_entries);
            let mut rmap_locks = match targets.try_lock() {
                Ok(locks) => locks,
                Err(busy) => {
            ...
            drop(rs_as_delta);
            self.refresh_rmap_entries(&mut cursor, &mut rmap_locks, core::slice::from_ref(&range));
    ...
    pub fn discard_pages(&self, range: Range<usize>) -> Result<()> {
        ...
        while let Some(vm_mapping) = cursor.find_next_mapped(range.end) {
            ...
            while let Some(va) = cursor.find_next(intersected_range.end) {
                propagate_if_needed(&mut cursor, intersected_range.end - va);
                let page_size = page_size_at(cursor.level());
