...
pub use entry::{RouteAddressFamily, RouteEntry, RouteMetric, RouteTableId, RouteType};
...
#[derive(Clone, Debug)]
pub struct RouteLookupKey<A: RouteAddressFamily> {
    dst: A,
...
fn iface_ipv6_routes(iface: &Arc<Iface>) -> Vec<RouteEntry<Ipv6Address>> {
    ...
    // for the IPv6 loopback address (`::1`), though I'm not entirely sure about this.
...
pub fn lookup_iface(dst: IpAddress) -> Result<Arc<Iface>> {
    let iface = match dst {
...
pub fn lookup_local_iface(address: IpAddress) -> Result<Arc<Iface>> {
