...
#[derive(Clone)]
pub struct RouteEntry<A: RouteAddressFamily> {
    dst: A::Cidr,
    #[expect(dead_code)]
    gateway: Option<A>,
...
impl<A: RouteAddressFamily> RouteEntry<A> {
    ...
    pub const fn dst(&self) -> &A::Cidr {
        &self.dst
    }

    /// Returns the next-hop gateway.
    #[expect(dead_code)]
    pub const fn gateway(&self) -> Option<A> {
        self.gateway
    }

    /// Returns the output interface.
    pub const fn output_iface(&self) -> &Arc<Iface> {
        &self.output_iface
    }

    /// Returns the ID of the table containing the route.
    pub const fn table_id(&self) -> RouteTableId {
        self.table_id
    }

    /// Returns the route metric.
    pub const fn metric(&self) -> RouteMetric {
        self.metric
    }

    /// Returns the route type.
    pub const fn type_(&self) -> RouteType {
        self.type_
    }
...
pub trait RouteAddressFamily: Copy + Eq + Ord {
