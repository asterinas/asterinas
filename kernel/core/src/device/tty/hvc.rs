...
impl TtyDriver for HvcDriver {
    ...
    fn devtmpfs_meta(&self, index: u32) -> Option<DevtmpfsInodeMeta> {
        Some(DevtmpfsInodeMeta::new(format!("hvc{}", index)).unwrap())
    }
...
pub(super) fn hvc0_device() -> Option<&'static Arc<Tty<HvcDriver>>> {
    HVC0.get()
}
