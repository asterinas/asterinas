...
use aster_drm::device::DrmFeatures;

use crate::{
    device::drm::{file::DrmFile, has_current_sys_admin, minor::DrmMinorType},
    prelude::*,
};
...
pub(super) fn check_drm_ioctl_flags<T: DrmIoctlFlagsInfo>(file: &DrmFile) -> Result<()> {
    ...
    if required_flags.contains(DrmIoctlFlags::MASTER) && !file.is_current_master() {
        return_errno!(Errno::EACCES);
    }
