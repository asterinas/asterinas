...
//!
//! See the `README.md` in this directory for backend implementation examples.

#![short_vis_path::add(misc)]
...
use aster_virtio::virtio_ring::{self, AvailRing, UsedRing};
...
pub(in misc) use self::virtqueue::VhostVirtQueue;

const VHOST_MAX_VRING_NUM: u32 = 32768;
const VHOST_MAX_MEMORY_REGIONS: usize = 64;
const VHOST_MAX_IOV: usize = 1024;

pub(in misc) const VIRTIO_F_VERSION_1: u64 = 1 << 32;
pub(in misc) const VIRTIO_RING_F_INDIRECT_DESC: u64 = 1 << 28;

#[repr(C)]
#[derive(Clone, Copy, Debug, Default, Pod)]
pub(in misc) struct VhostMemory {
    pub nregions: u32,
    pub padding: u32,
}
...
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, Pod)]
pub(in misc) struct VhostMemoryRegion {
...
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, Pod)]
pub(in misc) struct VhostVringState {
    pub index: u32,
    pub num: u32,
}
...
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, Pod)]
pub(in misc) struct VhostVringFile {
    pub index: u32,
    pub fd: i32,
}
...
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, Pod)]
pub(in misc) struct VhostVringAddr {
    pub index: u32,
    pub flags: u32,
...
pub(in misc) mod ioctl_defs {
    ...
    pub(in misc) type GetFeatures        = ioc!(VHOST_GET_FEATURES,         0xaf, 0x00, OutData<u64>);
    pub(in misc) type SetFeatures        = ioc!(VHOST_SET_FEATURES,         0xaf, 0x00, InData<u64>);
    pub(in misc) type SetOwner           = ioc!(VHOST_SET_OWNER,            0xaf, 0x01, NoData);
    pub(in misc) type ResetOwner         = ioc!(VHOST_RESET_OWNER,          0xaf, 0x02, NoData);
    pub(in misc) type SetMemTable        = ioc!(VHOST_SET_MEM_TABLE,        0xaf, 0x03, InData<VhostMemory>);
    pub(in misc) type SetVringNum        = ioc!(VHOST_SET_VRING_NUM,        0xaf, 0x10, InData<VhostVringState>);
    pub(in misc) type SetVringAddr       = ioc!(VHOST_SET_VRING_ADDR,       0xaf, 0x11, InData<VhostVringAddr>);
    pub(in misc) type SetVringBase       = ioc!(VHOST_SET_VRING_BASE,       0xaf, 0x12, InData<VhostVringState>);
    pub(in misc) type GetVringBase       = ioc!(VHOST_GET_VRING_BASE,       0xaf, 0x12, InOutData<VhostVringState>);
    pub(in misc) type SetVringKick       = ioc!(VHOST_SET_VRING_KICK,       0xaf, 0x20, InData<VhostVringFile>);
    pub(in misc) type SetVringCall       = ioc!(VHOST_SET_VRING_CALL,       0xaf, 0x21, InData<VhostVringFile>);
    pub(in misc) type SetVringErr        = ioc!(VHOST_SET_VRING_ERR,        0xaf, 0x22, InData<VhostVringFile>);
    pub(in misc) type SetBackendFeatures = ioc!(VHOST_SET_BACKEND_FEATURES, 0xaf, 0x25, InData<u64>);
    pub(in misc) type GetBackendFeatures = ioc!(VHOST_GET_BACKEND_FEATURES, 0xaf, 0x26, OutData<u64>);
}

/// Accesses userspace addresses in the process that owns this vhost session.
///
/// The queue and GPA translation logic depend only on this boundary.
trait OwnerMemory: Send + Sync {
    fn read(&self, addr: usize, dst: &mut [u8]) -> Result<()>;
    fn write(&self, addr: usize, src: &[u8]) -> Result<()>;
}

/// Copies through the owner's active address space.
///
/// Workers must be bound to this VMAR with `ThreadOptions::vmar`.
/// The `Arc` keeps the VMAR alive, but its mappings can still be cleared when
/// the owner exits. All copies remain fallible and may resolve page faults.
struct VmarOwnerMemory(Arc<Vmar>);

impl OwnerMemory for VmarOwnerMemory {
    fn read(&self, addr: usize, dst: &mut [u8]) -> Result<()> {
        let mut reader = self.0.vm_space().reader(addr, dst.len())?;
    ...
    fn write(&self, addr: usize, src: &[u8]) -> Result<()> {
        let mut writer = self.0.vm_space().writer(addr, src.len())?;
...
pub(in misc) struct VhostDeviceConfig {
...
pub(in misc) struct VhostDeviceState<const NUM_QUEUES: usize> {
...
impl<const NUM_QUEUES: usize> VhostDeviceState<NUM_QUEUES> {
    pub(in misc) fn new(config: VhostDeviceConfig) -> Self {
        assert!(NUM_QUEUES > 0);
        Self {
            config,
            owner_vmar: None,
    ...
    pub(in misc) fn build_runtime(&self) -> Result<VhostRuntime<NUM_QUEUES>> {
        self.check_owner()?;
        ...
        let memory = VhostMemorySpace::new(
            Arc::new(VmarOwnerMemory(owner.clone())),
    ...
    pub(in misc) fn reset_owner_after_quiesce(&mut self) {
    ...
    pub(in misc) fn check_owner(&self) -> Result<()> {
...
pub(in misc) struct VhostRuntime<const NUM_QUEUES: usize> {
    vmar: Arc<Vmar>,
    generation: u64,
...
impl<const NUM_QUEUES: usize> VhostRuntime<NUM_QUEUES> {
    ...
    pub(in misc) fn vmar(&self) -> &Arc<Vmar> {
        &self.vmar
    }

    pub(in misc) fn is_current(&self) -> bool {
        self.current_generation.load(Ordering::Acquire) == self.generation
    }

    pub(in misc) fn queue_mut(&mut self, index: usize) -> Result<&mut VhostVirtQueue> {
        if !self.is_current() {
...
fn validate_vring_addr(addr: &VhostVringAddr, num: u32) -> Result<()> {
    ...
    let desc_len = virtio_ring::descriptor_offset(num).ok_or_else(|| {
        Error::with_message(Errno::EINVAL, "vhost descriptor table size overflow")
    })?;
    let avail_len = virtio_ring::avail_entry_offset(num)
        .ok_or_else(|| Error::with_message(Errno::EINVAL, "vhost available ring size overflow"))?;
    let used_len = virtio_ring::used_entry_offset(num)
