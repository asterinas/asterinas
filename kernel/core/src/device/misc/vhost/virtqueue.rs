...
use aster_virtio::virtio_ring::{self, AvailRing, Descriptor, UsedElem, UsedRing};

use super::{MemorySegment, MemorySegments, VhostMemorySpace, VhostQueueState, VhostVringAddr};
...
pub(in misc) struct VhostVirtQueue {
...
impl VhostVirtQueue {
    ...
    pub(in misc) fn kick_event(&self) -> Option<Arc<KernelEventFile>> {
        self.kick.clone()
    }

    pub(in misc) fn consume_kick(&self) -> Option<u64> {
    ...
    pub(in misc) fn try_pop(&mut self) -> Result<Option<VhostDescriptorChain>> {
        let avail = self
            .memory
        ...
        let head_addr = checked_addr(
            ...
            virtio_ring::avail_entry_offset(slot).ok_or_else(|| {
    ...
    pub(in misc) fn add_used(&mut self, chain: &VhostDescriptorChain, len: u32) -> Result<()> {
        let slot = usize::from(self.last_used) % self.num;
        let element_addr = checked_addr(
            self.addr.used_user_addr,
            virtio_ring::used_entry_offset(slot).ok_or_else(|| {
    ...
    pub(in misc) fn notify(&self) -> Result<()> {
        ...
        if avail.flags() & virtio_ring::AVAIL_F_NO_INTERRUPT == 0 {
            call.signal();
        }
        Ok(())
    }
    ...
    pub(in misc) fn disable_kick_notifications(&mut self) -> Result<()> {
    ...
    pub(in misc) fn enable_kick_notifications(&mut self) -> Result<bool> {
        if self.used_flags & virtio_ring::USED_F_NO_NOTIFY == 0 {
            return Ok(false);
        }
        let flags = self.used_flags & !virtio_ring::USED_F_NO_NOTIFY;
    ...
    pub(in misc) fn signal_error(&self) {
    ...
    fn read_chain(&self, head: usize) -> Result<VhostDescriptorChain> {
        ...
        if first.flags() & virtio_ring::DESC_F_INDIRECT != 0 {
            ...
            if first.flags() & (virtio_ring::DESC_F_NEXT | virtio_ring::DESC_F_WRITE) != 0
                || first.buffer_len() == 0
            {
    ...
    fn walk_direct_chain(
        ...
        for _ in 0..self.num {
            ...
            if descriptor.flags() & virtio_ring::DESC_F_NEXT == 0 {
                return Ok(());
            }
    ...
    fn walk_chain(
        ...
        for _ in 0..table_num {
            ...
            let offset = virtio_ring::descriptor_offset(index).ok_or_else(|| {
    ...
    fn append_descriptor(
        ...
        if descriptor.flags() & virtio_ring::DESC_F_INDIRECT != 0 {
            return_errno_with_message!(
                Errno::EINVAL,
                "nested indirect descriptors are unsupported"
            );
        }
        if descriptor.flags() & virtio_ring::DESC_F_WRITE != 0 {
            *has_writable = true;
        ...
        if readable
            ...
            .is_none_or(|count| count > super::VHOST_MAX_IOV)
        {
            return_errno_with_message!(
                Errno::ENOBUFS,
    ...
    fn read_descriptor(&self, index: usize) -> Result<Descriptor> {
        let addr = checked_addr(
            ...
            virtio_ring::descriptor_offset(index).ok_or_else(|| {
...
pub(in misc) struct VhostDescriptorChain {
    memory: VhostMemorySpace,
    head: usize,
...
impl VhostDescriptorChain {
    ...
    pub(in misc) fn readable_len(&self) -> usize {
        self.readable.iter().map(|segment| segment.len).sum()
    }

    pub(in misc) fn writable_len(&self) -> usize {
        self.writable.iter().map(|segment| segment.len).sum()
    }

    pub(in misc) fn reader(&self) -> VhostChainReader<'_> {
        VhostChainReader {
            memory: &self.memory,
    ...
    pub(in misc) fn writer(&self) -> VhostChainWriter<'_> {
        VhostChainWriter {
            memory: &self.memory,
...
pub(in misc) struct VhostChainReader<'a> {
...
impl VhostChainReader<'_> {
    pub(in misc) fn remaining(&self) -> usize {
        self.segments
            .iter()
            .skip(self.index)
    ...
    pub(in misc) fn read_exact(&mut self, mut dst: &mut [u8]) -> Result<()> {
...
pub(in misc) struct VhostChainWriter<'a> {
    memory: &'a VhostMemorySpace,
    segments: &'a [MemorySegment],
    index: usize,
    offset: usize,
...
impl VhostChainWriter<'_> {
    ...
    pub(in misc) fn write_all(&mut self, mut src: &[u8]) -> Result<()> {
    ...
    pub(in misc) fn bytes_written(&self) -> usize {
        self.written
    }
}
...
fn checked_addr(base: u64, offset: usize, message: &'static str) -> Result<usize> {
    ...
    super::validate_owner_range(address as u64, 0)
}
