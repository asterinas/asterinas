...
impl VhostMemorySpace {
    /// Creates an address space with an empty memory table.
    ...
    /// Replaces the table only if every region is valid and non-overlapping.
    ...
    /// byte is unmapped, or `EINVAL` on overflow, without exposing partial results.
    /// A zero-length interval produces no segments.
