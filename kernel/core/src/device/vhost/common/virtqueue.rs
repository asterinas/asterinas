...
///
/// The queue must be disabled before changing its size or base.
/// Disabling preserves pending requests and configuration.
...
impl VhostVirtQueue {
    ...
    pub(super) fn enable(&mut self, memory: &VhostMemorySpace) -> Result<()> {
        ...
        memory.write_owner_val(used_addr + offset_of!(UsedRing, flags), &self.used_flags)?;
        self.last_used = used.idx();
        self.is_enabled = true;
        Ok(())
    }

    /// Takes the next available descriptor chain.
    ///
    /// Errors leave the request available for another attempt.
    /// A returned chain is consumed even if the backend drops it unfinished.
    ...
    pub(in vhost) fn notify(&self, memory: &VhostMemorySpace) -> Result<()> {
        ...
        let flags = memory.read_owner_val::<AvailFlags>(
            self.addr()?.avail_user_addr as usize + offset_of!(AvailRing, flags),
        )?;
    ...
    /// Suppresses guest kicks.
    ///
    /// A backend typically uses this while actively draining the queue, when
    /// kicks would only cause redundant wakeups.
    pub(in vhost) fn disable_kick_notifications(
        ...
        memory.write_owner_val(
            self.addr()?.used_user_addr as usize + offset_of!(UsedRing, flags),
            &flags,
        )?;
    ...
    /// Enables guest kicks and checks for pending work.
    ///
    /// A backend typically uses this before waiting. If it returns `true`, the
    /// backend must continue processing instead of sleeping.
    ...
    fn walk_chain(
        ...
        let mut index = head;
        // A valid direct chain cannot contain more descriptors than the queue.
        // Bounding the walk by the queue size also detects cycles.
        for _ in 0..self.num {
            if index >= self.num {
            ...
            let descriptor: Descriptor = memory.read_owner_val(addr)?;
    ...
    fn walk_indirect_chain(
        ...
        for _ in 0..table_num {
            ...
            let segment_index = table
                ...
                .unwrap();
            let mut reader = VhostDescriptorChainReader {
                memory,
                segments: &table,
                segment_index,
                offset_in_segment: offset,
            };
            let descriptor: Descriptor = reader.read_val()?;
            Self::append_descriptor(memory, descriptor, readable, writable, has_writable)?;
            if !descriptor.flags().contains(DescFlags::NEXT) {
                return Ok(());
            }
    ...
    fn read_avail_idx(&self, memory: &VhostMemorySpace) -> Result<u16> {
        memory.read_owner_val::<u16>(
            self.addr()?.avail_user_addr as usize + offset_of!(AvailRing, idx),
        )
    }
}
...
impl VhostDescriptorChain<'_> {
    ...
    /// Guest notification is a separate queue operation.
    pub(in vhost) fn complete(self, len: u32) -> Result<()> {
        ...
        // The guest must see the completed entry before its published index.
        atomic::fence(Ordering::Release);

        let next_used = self.queue.last_used.wrapping_add(1);
        self.memory.write_owner_val(
            self.queue.addr()?.used_user_addr as usize + offset_of!(UsedRing, idx),
            &next_used,
        )?;
    ...
    pub(in vhost) fn reader(&self) -> VhostDescriptorChainReader<'_> {
        VhostDescriptorChainReader {
            memory: self.memory,
    ...
    pub(in vhost) fn writer(&self) -> VhostDescriptorChainWriter<'_> {
        VhostDescriptorChainWriter {
            memory: self.memory,
...
pub(in vhost) struct VhostDescriptorChainReader<'a> {
    memory: &'a VhostMemorySpace,
...
impl VhostDescriptorChainReader<'_> {
    ...
    /// Reads a plain-old-data value across the chain's memory regions.
    pub(in vhost) fn read_val<T: Pod>(&mut self) -> Result<T> {
        let mut value = T::new_zeroed();
        self.read_exact(value.as_mut_bytes())?;
        Ok(value)
    }

    /// Reads exactly enough bytes to fill the destination.
    ///
    ...
    pub(in vhost) fn read_exact(&mut self, mut dst: &mut [u8]) -> Result<()> {
        ...
        while !dst.is_empty() {
            ...
            let available = segment.len - self.offset_in_segment;
            let count = available.min(dst.len());
            let addr = segment.host_virt_addr + self.offset_in_segment;
            self.memory.read_owner_bytes(addr, &mut dst[..count])?;
            self.offset_in_segment += count;
            dst = &mut dst[count..];
...
pub(in vhost) struct VhostDescriptorChainWriter<'a> {
    memory: &'a VhostMemorySpace,
    segments: &'a [TranslatedMemoryRegion],
    segment_index: usize,
    offset_in_segment: usize,
...
impl VhostDescriptorChainWriter<'_> {
    pub(in vhost) fn avail(&self) -> usize {
        self.segments
            .iter()
            .skip(self.segment_index)
            .map(|segment| segment.len)
