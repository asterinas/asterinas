...
use super::common::device::{
    VhostDeviceConfig, VhostFileCommon, VhostSharedState, ioctl_defs::SetOwner,
};
use crate::{
...
mod ioctl_defs {
    use crate::{ioc, util::ioctl::InData};
...
// Lock order: common -> runtime -> socket/pending locks. The worker
// never takes the common mutex; socket callbacks take neither sleeping mutex.
...
impl Backend {
    fn new() -> Arc<Self> {
        let common = VhostFileCommon::new(VhostDeviceConfig {
            device_features: (Feature::VERSION_1 | Feature::RING_INDIRECT_DESC).bits(),
            backend_features: 0,
            max_queue_size: 32768,
        });
        let shared = Arc::new(VhostSharedState::new());
        Arc::new(Self {
    ...
    fn ioctl(self: &Arc<Self>, raw: RawIoctl) -> Result<i32> {
        ...
        dispatch_ioctl!(match raw {
            SetOwner => {
                ...
                common.set_owner(&self.shared, vmar, move |shared| backend.process(shared))?;
                Ok(0)
            }
            cmd @ SetGuestCid => {
            ...
            cmd @ SetRunning => {
                ...
                self.shared.runtime().lock().check_owner()?;
                if running != 0 {
                    vsock::ensure_vhost_backend()?;
                    self.start()?;
                } else {
                    self.shared.disable_queues();
                }
                Ok(0)
            }
            _ => common.handle_ioctl(raw, &self.shared),
        })
    }

    fn set_guest_cid(self: &Arc<Self>, cid: u64) -> Result<()> {
        let cid = validate_guest_cid(cid)?;
        let _common = self.shared.runtime().lock();
    ...
    fn start(&self) -> Result<()> {
        ...
        loop {
            self.shared.worker_pollee().poll_with(
                IoEvents::IN,
            ...
            if !pending.needs_reset {
                ...
                drop(pending);
                runtime.enable_queues()?;
                drop(runtime);
                self.shared.worker_pollee().notify(IoEvents::IN);
                return Ok(());
            }
            // The worker keeps needs_reset set until old socket cleanup finishes.
            // It never takes the common lock held by this ioctl.
            drop(pending);
            drop(runtime);
            poller.wait()?;
        }
    ...
    fn shutdown(&self) {
        ...
        common.reset_owner(&self.shared);
        let cid = self.cid();
        if cid != 0 {
            // Keep the CID reserved until old sockets are reset, so a new
            // file cannot inherit a reset intended for this file.
