...
sockets. It consumes the [common vhost layer](../vhost/mod.rs), including
...
CID reservation does not require `VHOST_SET_OWNER`: a process may reserve a CID
and pass the device file to another process, which establishes ownership before
configuring memory and queues. Starting or stopping the device requires its owner.
