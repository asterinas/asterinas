...
impl Packet {
    pub(super) fn new(header: VirtioVsockHdr, payload: &[u8]) -> Result<Arc<Self>> {
    ...
    pub(super) fn header_for_fragment(
        &self,
        offset: usize,
        capacity: usize,
...
pub(super) struct PendingPackets {
    pub is_active: bool,
    pub generation: u64,
    // Stays set until the worker has finished resetting the failed endpoint.
...
impl PendingPackets {
    ...
    pub(super) fn reserve(&mut self, len: usize) -> bool {
    ...
    pub(super) fn release(&mut self, len: usize) {
        self.reservations -= 1;
        self.bytes -= HEADER_LEN + len;
    }

    pub(super) fn push_reserved(&mut self, packet: Arc<Packet>) {
        self.reservations -= 1;
        self.packets.push_back((packet, 0));
    }

    pub(super) fn push(&mut self, packet: Arc<Packet>) -> bool {
    ...
    pub(super) fn front(&self) -> Option<(Arc<Packet>, usize)> {
        self.packets
            .front()
            .map(|(packet, offset)| (packet.clone(), *offset))
    }

    pub(super) fn complete_fragment(&mut self, len: usize) {
    ...
    pub(super) fn discard(&mut self) {
        // In-flight send reservations can outlive an endpoint failure. Their Drop
