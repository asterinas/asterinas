...
impl VhostWork for Arc<Backend> {
    fn process(&mut self) -> VhostWorkStatus {
        self.reset_failed_endpoint();
        let mut common = self.shared.lock();
...
impl Backend {
    ...
    pub(super) fn reset_failed_endpoint(&self) {
        let cid = {
            let mut common = self.shared.lock();
    ...
    fn process_queues(&self, common: &mut VhostRuntimeData<NUM_QUEUES>) -> VhostWorkStatus {
