...
fn map_segment_vmos(
    ...
    let map_range = if elf.is_shared_object() {
        ...
        vmar.new_map(map_size, VmPerms::empty())
            .offset(elf_va_range_aligned.start, OffsetType::FixedNoReplace)
            .build()?;
...
fn map_segment_vmo(
    ...
    if anonymous_map_size > 0 {
        let anonymous_map_options = vmar
            .new_map(NonZeroUsize::new(anonymous_map_size).unwrap(), perms)
            .offset(offset + segment_size, OffsetType::Fixed);
