...
pub(super) fn sys_rt_sigprocmask(
    ...
    do_rt_sigprocmask(mask_op, set_ptr, oldset_ptr, sigset_size, ctx)?;
...
fn do_rt_sigprocmask(
    ...
    let (set_ptr, oldset_ptr) = {
        let size_policy = RequireFullSize::new(sigset_size)?;
        (
            UserSigSetPtr::new(&user_space, set_ptr, size_policy),
            UserSigSetPtr::new(&user_space, oldset_ptr, size_policy),
        )
    };
...
/// A user-space pointer to a [`SigSet`] bound to a size policy,
...
impl<'a, 'b> UserSigSetPtr<'a, 'b, AllowTruncSize> {
    pub(super) fn write_val(&self, sigset: &SigSet) -> Result<()> {
        ...
        self.user_space
            .write_bytes(self.addr, &bytes[..self.size_policy.requested_size])?;
        Ok(())
    }
}
