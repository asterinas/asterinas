...
///
/// Kernel subsystems obtain this object from an [`EventFile`] after validating
/// the generic file reference. Its fields remain private; callers can only use
/// the eventfd operations exposed below.
...
impl EventFile {
    ...
    /// Returns the kernel-facing eventfd state.
    pub fn kernel_event_file(&self) -> Arc<KernelEventFile> {
        self.kernel_event_file.clone()
    }
...
impl Pollable for EventFile {
    fn poll(&self, mask: IoEvents, poller: Option<&mut PollHandle>) -> IoEvents {
        self.kernel_event_file
            .pollee
            .poll_with(mask, poller, || self.kernel_event_file.check_io_events())
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::events::{EpollFile, KernelEventFile as ExportedKernelEventFile};

    fn event_file(init_val: u64, is_semaphore: bool) -> EventFile {
        // `EventFile::new` creates an anonymous-inode path whose metadata uses
        // the coarse realtime clock. Kernel tests do not run the normal boot
        // clock initialization, so initialize the ktest clock fixture first.
        crate::time::clocks::init_for_ktest();
        let flags = if is_semaphore {
            EventFileFlags::EFD_SEMAPHORE
        } else {
            EventFileFlags::empty()
        };
    ...
    fn kernel_event_file_rejects_non_eventfd() {
        ...
        assert!(KernelEventFile::from_file(file).is_err());
    }

    #[ktest]
    fn kernel_event_file_owns_state_after_file_drop() {
        let file = Arc::new(event_file(0, false)) as Arc<dyn FileLike>;
        let kernel_event_file = ExportedKernelEventFile::from_file(file).unwrap();
    ...
    fn event_file_signal_consume_and_poll() {
        let event_file = event_file(0, false);
    ...
    fn event_file_respects_semaphore_mode() {
        let event_file = event_file(0, true);
    ...
    fn event_file_reports_kernel_signal_overflow() {
        let event_file = event_file(KernelEventFile::MAX_COUNTER_VALUE, false);
    ...
    fn event_file_read_consumes_counter() {
        let event_file = event_file(42, false);
