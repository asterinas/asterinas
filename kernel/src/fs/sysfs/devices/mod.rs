...
pub fn register_devices(root: Arc<KObject>) -> Result<()> {
    let devices_kobj = SysFS::create_kobject("devices", 0o755, root, None).unwrap();
    let system_kobject = SysFS::create_kobject("system", 0o755, devices_kobj.clone(), None)?;
    let cpu_kobject = SysFS::create_kobject("cpu", 0o755, system_kobject.clone(), None)?;
