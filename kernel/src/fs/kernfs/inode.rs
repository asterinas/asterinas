...
use super::{element::PseudoElement, DataProvider, EventHandler, PseudoFileSystem, BLOCK_SIZE};
use crate::{
    events::IoEvents,
    fs::{
        device::Device,
...
/// The event_handlers field provides different abstractions for different pseudo-file system requirements.
pub struct PseudoINode {
    metadata: RwMutex<Metadata>,
    elem: RwMutex<PseudoElement>,
    event_handlers: Option<Arc<dyn EventHandler>>,
    fs: Weak<dyn PseudoFileSystem>,
    parent: Option<Weak<PseudoINode>>,
    this: Weak<PseudoINode>,
}
...
impl PseudoINode {
    ...
    /// Set the event_handlers of the current node.
    /// The event_handlers provides different abstractions for different pseudo-file system requirements.
    pub fn set_event_handlers(&mut self, event_handlers: Arc<dyn EventHandler>) {
    ...
    /// Trigger the event_handlers's on_remove method if the event_handlers exists.
    fn remove(&self, name: &str) -> Result<Arc<dyn Inode>> {
        if let Some(event_handlers) = self.event_handlers.as_ref() {
            event_handlers.on_remove(name.to_string())?;
        }
    ...
    /// Trigger the event_handlers's on_create method if the event_handlers exists.
    fn insert(&self, name: String, node: Arc<dyn Inode>) -> Result<()> {
        if let Some(event_handler) = self.event_handlers.as_ref() {
            event_handler.on_create(name.clone(), node.clone())?;
        }
...
impl Inode for PseudoINode {
    ...
    fn create(&self, name: &str, type_: InodeType, mode: InodeMode) -> Result<Arc<dyn Inode>> {
        ...
        let new_node = match type_ {
            ...
            // InodeType::SymLink => {
            //     KernfsNode::new_symlink(name, name, self.this_weak(), None)
            // }
