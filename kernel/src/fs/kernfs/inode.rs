...
bitflags! {
    /// Some flags are used to indicate the status of the node. But we don't use them now.
    ///
    /// Indicates that the node is in an active state.
    // const KERNFS_ACTIVATED  = 0x0010;
    /// Indicates that the node is in a suicidal state.
    // const KERNFS_SUICIDAL   = 0x0400;
    /// Indicates that the node has already committed suicide.
    // const KERNFS_SUICIDED   = 0x0800;
    /// Indicates that the directory node is empty.
    // const KERNFS_EMPTY_DIR  = 0x1000;
    /// Indicates that the node supports the `release` operation.
    // const KERNFS_HAS_RELEASE = 0x2000;
    /// Indicates that the node is being removed.
    // const KERNFS_REMOVING   = 0x4000;
