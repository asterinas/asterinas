...
/// in the front of an xattr block.
#[repr(C)]
...
/// The number of blocks for an xattr (TBD).
...
impl Xattr {
    ...
    pub fn set(
        ...
        let cache = self.cache.upread();
        if let Some((offset, mut entry)) = cache.find_entry(namespace, name, &self.blocks_buf) {
    ...
    pub fn get(
        ...
        let (_, entry) = self
            .cache
            .read()
            .find_entry(namespace, name, &self.blocks_buf)
