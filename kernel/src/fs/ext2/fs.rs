...
pub struct Ext2 {
    ...
    inode_size: usize,
    block_size: usize,
    group_descriptors_segment: BioSegment,
    self_ref: Weak<Self>,
}
...
impl Ext2 {
    ...
    pub fn open(block_device: Arc<dyn BlockDevice>) -> Result<Arc<Self>> {
        ...
        let group_descriptors_segment = {
            ...
            .div_ceil(BLOCK_SIZE);
            let bio_segment = BioSegment::new(npages, BioDirection::Read);
            match block_device
                .read_blocks(super_block.group_descriptors_bid(0), bio_segment.clone())?
            {
                BioStatus::Complete => (),
                err_status => {
