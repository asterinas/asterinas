...
impl InodeImpl {
    ...
    fn try_expand_blocks(&mut self, range: Range<Ext2Bid>) -> Result<Ext2Bid> {
        ...
        if indirect_cnt == 0 {
            ...
            self.desc.sector_count =
                InodeDesc::blocks_to_sectors(range.start + device_range.len() as u32);
    ...
    fn shrink_blocks(&mut self, range: Range<Ext2Bid>) {
        ...
        self.desc.sector_count = InodeDesc::blocks_to_sectors(range.start);
...
impl InodeDesc {
    ...
    pub fn blocks_count(&self) -> Ext2Bid {
        ...
        assert!(blocks <= InodeDesc::sectors_to_blocks(self.sector_count));
        blocks
    }
    ...
    /// Converts block count to sector count with overflow protection.
    ///
    /// # Panics
    /// Panics if the conversion would cause u32 overflow.
