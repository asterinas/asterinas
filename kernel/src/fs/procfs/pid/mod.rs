...
use crate::{
    ...
    prelude::*,
    process::{posix_thread::AsPosixThread, Pid, PidEvent, PidNamespace, Process, UniqueId},
};
...
impl PidDirOps {
    pub fn new_inode(
        ...
        let attachment = {
            let unique_id = UniqueId::new(pid, &pid_ns);
            pid_ns.get_attachment_with_unique_id(&unique_id)?
        };
        let attachment_guard = attachment.read();
        let process_ref = attachment_guard.attached_process()?;
        ...
        attachment_guard.register_observer(Arc::downgrade(&pid_inode) as _);
...
impl Observer<PidEvent> for ProcDir<PidDirOps> {
    fn on_events(&self, _events: &PidEvent) {
...
impl DirOps for PidDirOps {
    fn lookup_child(&self, this_ptr: Weak<dyn Inode>, name: &str) -> Result<Arc<dyn Inode>> {
        let inode = match name {
            ...
            "task" => TaskDirOps::new_inode(self.process.clone(), this_ptr.clone()),
    ...
    fn populate_children(&self, this_ptr: Weak<dyn Inode>) {
        ...
        cached_children.put_entry_if_not_found("task", || {
            TaskDirOps::new_inode(self.process.clone(), this_ptr.clone())
        });
    }
}
