...
static CGROUP_PROC_THREAD_MUTEX: Mutex<()> = Mutex::new(());

/// A lock guard that provides exclusive access to modify cgroup membership.
///
/// This struct acts as a token that must be held to safely attach a task to a
/// cgroup, detach it, or move it between cgroups.
pub struct CgroupMembership {
    _guard: MutexGuard<'static, ()>,
}

impl CgroupMembership {
    /// Acquires the global cgroup membership lock, returning a guard.
    pub fn new() -> Self {
        let guard = CGROUP_PROC_THREAD_MUTEX.lock();
        CgroupMembership { _guard: guard }
    }

    /// Returns the process's current cgroup with guaranteed stability.
    ///
    /// While this guard is held, the process's cgroup membership cannot be
    /// concurrently modified, ensuring the returned value represents a
    /// consistent snapshot of the cgroup hierarchy.
    pub fn cgroup_of<'a>(&'a self, process: &'a Process) -> Option<Arc<CgroupNode>> {
        process.cgroup().get().as_deref().cloned()
    }
}

impl Default for CgroupMembership {
    fn default() -> Self {
        Self::new()
    }
}
...
impl CgroupNode {
    ...
    pub fn move_process(&self, process: Arc<Process>, cgroup_membership: &CgroupMembership) {
        if let Some(old_cgroup) = cgroup_membership.cgroup_of(&process) {
            ...
            {
                let old_count = old_cgroup.populated_count.fetch_sub(1, Ordering::AcqRel);
                if old_count == 1 {
        ...
        let mut current_process_set = self.processes.lock();
    ...
    pub fn remove_process(&self, process: &Process, _cgroup_membership: &CgroupMembership) {
    ...
    fn write_procs(&self, reader: &mut VmReader) -> Result<usize> {
        ...
        self.move_process(process, &cgroup_membership);
    ...
    fn write_attr(&self, name: &str, reader: &mut VmReader) -> Result<usize> {
        match name {
            "cgroup.procs" => {
                ...
                if let Some(old_cgroup) = cgroup_membership.cgroup_of(&process) {
                    old_cgroup.remove_process(&process, &cgroup_membership);
                }
