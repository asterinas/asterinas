...
static CGROUP_MEMBERSHIP_MUTEX: Mutex<()> = Mutex::new(());

/// A lock guard that provides exclusive access to modify cgroup membership.
///
/// Its existence serializes all cgroup membership modifications across the system,
/// preventing race conditions between operations. An instance of this guard **must**
/// be held to safely attach a task to a cgroup, detach it, or move it between cgroups.
pub struct CgroupMembershipGuard {
    _guard: MutexGuard<'static, ()>,
}

impl CgroupMembershipGuard {
    ...
    pub fn move_process_to_node(&self, process: Arc<Process>, new_cgroup: &CgroupNode) {
    ...
    pub fn move_process_to_root(&self, process: &Process) {
...
pub fn lock_cgroup_membership() -> CgroupMembershipGuard {
    let guard = CGROUP_MEMBERSHIP_MUTEX.lock();
    CgroupMembershipGuard { _guard: guard }
}
...
inherit_sys_branch_node!(CgroupSystem, fields, {
    ...
    fn write_attr(&self, name: &str, reader: &mut VmReader) -> Result<usize> {
        match name {
            "cgroup.procs" => {
                let content = reader
                    .read_string_with_max_len(MAX_ATTR_SIZE)
...
where
    F: FnOnce(Arc<Process>, &CgroupMembershipGuard) -> Result<()>,
{
    ...
    let cgroup_guard = lock_cgroup_membership();
    if process.status().is_zombie() {
        return Err(Error::InvalidOperation);
    }

    op(process, &cgroup_guard)
}
