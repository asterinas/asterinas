...
use crate::{
    error::Errno,
    fs::{
        cgroupfs::{systree_node::Normal, CgroupTreeNode, Unified},
        utils::{
...
impl Inode for CgroupInode {
    ...
    fn create(&self, name: &str, _type_: InodeType, _mode: InodeMode) -> Result<Arc<dyn Inode>> {
        ...
        let new_child = CgroupTreeNode::<Normal>::new(name.to_string().into());
        if branch_node.is_root() {
            let tree_node = branch_node
                .as_any()
                .downcast_ref::<CgroupTreeNode<Unified>>()
                .unwrap();
            tree_node.add_child(new_child.clone())?;
        } else {
            let tree_node = branch_node
                .as_any()
                .downcast_ref::<CgroupTreeNode<Normal>>()
                .unwrap();
            tree_node.add_child(new_child.clone())?;
        };
