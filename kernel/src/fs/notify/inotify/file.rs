...
impl InotifyFile {
    ...
    pub fn update_watch(&self, dentry: &Dentry, mask: u32) -> Result<u32> {
        ...
        let ret = self.update_existing_watch(dentry, mask);
        match ret {
            Ok(wd) => Ok(wd),
            Err(e) => {
                if e.error() == Errno::ENOENT {
                    // if the watch is not found, create a new watch
                    let wd = self.create_new_watch(dentry, mask)?;
                    Ok(wd)
                } else {
                    Err(e)
                }
    ...
    fn update_existing_watch(&self, dentry: &Dentry, mask: u32) -> Result<u32> {
    ...
    fn create_new_watch(&self, dentry: &Dentry, arg: u32) -> Result<u32> {
