...
use super::{
    ...
    utils::{AccessMode, ChannelPollee, InodeMode, InodeType, Metadata, Peered, StatusFlags},
};
use crate::{
    events::IoEvents,
    prelude::*,
...
pub fn new_pair_with_capacity(capacity: usize) -> Result<(Arc<PipeReader>, Arc<PipeWriter>)> {
    ...
    let (producer_pollee, consumer_pollee) =
        Peered::new_pair(ChannelPollee::new(), ChannelPollee::new());

    Ok((
        PipeReader::new(consumer, consumer_pollee, StatusFlags::empty())?,
        PipeWriter::new(producer, producer_pollee, StatusFlags::empty())?,
    ))
}

pub struct PipeReader {
    consumer: Mutex<RbConsumer<u8>>,
    pollee: Peered<ChannelPollee>,
...
impl PipeReader {
    ...
    fn try_read(&self, writer: &mut VmWriter) -> Result<usize> {
        ...
        self.pollee.read_with(read)
    }

    fn check_io_events(&self) -> IoEvents {
        let mut events = IoEvents::empty();
        if self.pollee.is_peer_shutdown() {
            events |= IoEvents::HUP;
        }
...
impl PipeWriter {
    ...
    fn try_write(&self, reader: &mut VmReader) -> Result<usize> {
        ...
        self.pollee.write_with(write)
    }

    fn check_io_events(&self) -> IoEvents {
        if self.pollee.is_shutdown() {
