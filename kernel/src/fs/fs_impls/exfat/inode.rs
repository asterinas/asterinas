...
impl ExfatInode {
    ...
    pub(super) fn read_bitmap(&self, root_chain: ExfatChain) -> Result<ExfatBitmap> {
        let inner = self.inner.read();
    ...
    pub(super) fn read_upcase_table(&self, root_chain: ExfatChain) -> Result<ExfatUpcaseTable> {
        let inner = self.inner.read();
        let dentry_iterator = ExfatDentryIterator::new(&inner.page_cache, 0, None)?;
