...
impl PathResolver {
    ...
    pub fn pivot_root(
        ...
        {
            return_errno_with_message!(Errno::EBUSY, "`new_root` is on the current root mount");
        }
        ...
        if !new_root_path.is_reachable_from(&self.root) {
            return_errno_with_message!(Errno::EINVAL, "`new_root` not underneath the current root");
        }
        ...
        let parent_path = {
            ...
            println!("new root mount id: {}", new_root_path.mount.id());
            println!("parent mount id: {}", parent_mount.id());
            println!("self root mount id: {}", self.root.mount.id());
