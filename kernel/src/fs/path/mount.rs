...
impl MountNode {
    ...
    fn move_process_root_and_cwd(&self, process: &Arc<Process>) {
        let mut stack = vec![self.this().clone()];
        let root = process.fs().read().root().clone();
        let cwd = process.fs().read().cwd().clone();
        ...
        while let Some(current_mount_node) = stack.pop() {
            if Arc::ptr_eq(
                ...
                root.move_root(&current_mount_node, process);
            }
    ...
    pub fn clone_mount_node_tree_and_move(&self, process: &Arc<Process>) -> Arc<Self> {
        let new_mount_node = self.clone_mount_node_tree(self.root_dentry(), true);
        new_mount_node.move_process_root_and_cwd(process);
        new_mount_node
    }
