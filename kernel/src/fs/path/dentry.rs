...
impl Dentry {
    ...
    /// Returns the underlying `RealDentryInner`, or `None` if it is a pseudo dentry.
    ///
    /// A `Dentry` is guaranteed to be real if it satisfies either of the following:
    /// - Its type is `InodeType::Dir`.
    /// - It is the parent or child of another `Dentry`.
