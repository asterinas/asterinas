...
use ostd::{
    mm::{Frame, UntypedMem, UntypedMeta, VmIo},
...
impl PageCacheBackend for RamInode {
    fn read_page_async(&self, _idx: usize, frame: &Frame<dyn UntypedMeta>) -> Result<BioWaiter> {
    ...
    fn write_page_async(&self, _idx: usize, _frame: &Frame<dyn UntypedMeta>) -> Result<BioWaiter> {
        // do nothing
        Ok(BioWaiter::new())
    }
