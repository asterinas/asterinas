...
pub fn do_memfd_create(name: String, memfd_flags: MemfdFlags) -> Result<InodeHandle> {
    if name.len() > MAX_MEMFD_NAME_LEN {
        return_errno_with_message!(Errno::EINVAL, "MemfdInode: `name` is too long.");
    }
    ...
    let memfd_inode = Arc::new_cyclic(|weak_self| {
        let ram_inode =
            RamInode::new_file_detached_in_memfd(weak_self, mode, Uid::new_root(), Gid::new_root());
    ...
    let path = Path::new_pseudo(memfd_tmpfs_mount().clone(), memfd_inode, |inode| {
...
pub fn do_add_seals(file: &InodeHandle, new_seals: FileSeals) -> Result<()> {
    ...
    memfd_inode_or_err(file)?.add_seals(new_seals)
}

pub fn do_get_seals(file: &InodeHandle) -> Result<FileSeals> {
    let rights = file.rights();
    if rights.is_empty() {
        return_errno_with_message!(Errno::EBADF, "the file is opened as a path");
    }

    Ok(memfd_inode_or_err(file)?.get_seals())
}
