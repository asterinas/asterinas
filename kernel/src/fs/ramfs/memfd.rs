...
pub trait MemfdInodeHandle {
    fn new_memfd(name: String, memfd_flags: MemfdFlags) -> Result<InodeHandle>;
...
impl MemfdInodeHandle for InodeHandle {
    ...
    fn add_seals(&self, new_seals: FileSeals) -> Result<()> {
        ...
        self.memfd_inode_or_err()?.add_seals(new_seals)
    }
    ...
    fn get_seals(&self) -> Result<FileSeals> {
        ...
        Ok(self.memfd_inode_or_err()?.get_seals())
    }
}

impl InodeHandle {
    fn memfd_inode_or_err(&self) -> Result<&MemfdInode> {
        self.path()
            .inode()
