...
pub(in crate::fs) trait SysTreeInodeTy: Send + Sync + 'static {
    ...
    where
        Self: Sized + 'static,
    {
        let inner_node = SysTreeNodeKind::Branch(root_node);
        let parent = Weak::new();
    ...
    where
        Self: Sized + 'static,
    {
        let inner_node = SysTreeNodeKind::Attr(attr.clone(), node);
    ...
    where
        Self: Sized + 'static,
    {
        let inner_node = SysTreeNodeKind::Symlink(symlink);
        let ino = ino::from_inner_node(&inner_node);
    ...
    fn new_branch_dir(
        inner_node: SysTreeNodeKind, // Must be SysTreeNodeKind::Branch
        ...
        Self: Sized + 'static,
    {
        ...
        let SysTreeNodeKind::Branch(branch_node) = &inner_node else {
    ...
    fn new_leaf_dir(
        inner_node: SysTreeNodeKind, // Must be SysTreeNodeKind::Leaf
        mode: Option<InodeMode>,
        parent: Weak<Self>,
    ) -> Arc<Self>
    where
        Self: Sized + 'static,
    {
        ...
        let SysTreeNodeKind::Leaf(leaf_node) = &inner_node else {
...
impl<KInode: SysTreeInodeTy + Send + Sync + 'static> Inode for KInode {
    ...
    default fn read_direct_at(&self, offset: usize, buf: &mut VmWriter) -> Result<usize> {
        let SysTreeNodeKind::Attr(attr, leaf) = &self.inner_node() else {
    ...
    default fn create(
        ...
        let SysTreeNodeKind::Branch(branch_node) = &self.inner_node() else {
    ...
    default fn poll(&self, mask: IoEvents, _poller: Option<&mut PollHandle>) -> IoEvents {
        ...
        if let SysTreeNodeKind::Attr(attr, _) = &self.inner_node() {
...
mod ino {
    ...
    pub fn from_inner_node(inner: &SysTreeNodeKind) -> Ino {
        match inner {
