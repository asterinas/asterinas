...
/// A trait that abstracts the functionality of the "view" part of Asterinas's
/// model-view-controller architecture.
///
/// The struct implementing this trait will have a default implementation for the `Inode` trait.
/// Users only need to additionally implement the [`Inode::fs`] method.
///
/// This is mainly used for the inode of kernelfs like sysfs, cgroupfs, etc.
#[expect(dead_code)]
pub(in crate::fs) trait KernelFsInode: Send + Sync + 'static {
    fn new_arc(
        ...
        Self: Sized + 'static,
    {
        ...
        let mode = flags_to_inode_mode(attr.flags());
        ...
        Self: Sized + 'static,
    {
        ...
        let InnerNode::Branch(_) = &inner_node else {
            panic!("new_branch_dir called with non-branch InnerNode");
        };

        // FIXME: This is a temporary hard-code solution.
        // We should get the mode from the inner node.
        ...
        Self: Sized + 'static,
    {
        ...
        let InnerNode::Leaf(_) = &inner_node else {
    ...
    fn is_dir(&self) -> bool {
        matches!(self.inner_node(), InnerNode::Branch(_) | InnerNode::Leaf(_))
    }
}
...
fn flags_to_inode_mode(flags: SysAttrFlags) -> InodeMode {
    ...
    if flags.contains(SysAttrFlags::CAN_WRITE) {
        bits |= 0o200; // Add write permissions if flag is set
    }
...
#[derive(Debug)]
pub(in crate::fs) enum InnerNode {
...
impl<KInode: KernelFsInode + Send + Sync + 'static> Inode for KInode {
    ...
    default fn readdir_at(&self, offset: usize, visitor: &mut dyn DirentVisitor) -> Result<usize> {
        ...
        let iter = self.new_dentry_iter(start_ino);
        let mut dentries: Vec<_> = iter.collect();
...
struct ThisAndParentDentryIter<'a, KInode: KernelFsInode> {
    inode: &'a KInode,
    min_ino: Ino,
    state: u8, // 0 = self, 1 = parent, 2 = done
}

impl<'a, KInode: KernelFsInode> ThisAndParentDentryIter<'a, KInode> {
...
impl<'a, KInode: KernelFsInode> Iterator for ThisAndParentDentryIter<'a, KInode> {
    type Item = Dentry;
