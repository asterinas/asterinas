...
/// PageCacheReclaimer works as a global page cache and records all file-backed pages
...
struct ReclaimerInner {
    /// Maximum number of entries in the PageCacheReclaimer
    capacity: usize,
...
struct ThresholdReclaimPolicy {
    /// Size threshold(x100) of the active LruCache for triggering demotion
    demote_threshold_times100: usize,
    /// Size threshold(x100) of the page cache for triggering reclamation
...
pub fn get_page_cache_reclaimer() -> &'static Mutex<PageCacheReclaimer> {
    if !PAGE_CACHE_RECLAIMER.is_completed() {
        // Page size equals to 4K by default
        let mut reclaimer = PageCacheReclaimer::new(mem_total() >> 12);
