...
use lru::LruCache;
use ostd::mm::{ExternalFrame, FrameAllocOptions, VmIo};
...
impl PageCacheManager {
    ...
    fn ondemand_readahead(&self, idx: usize) -> Result<ExternalFrame> {
        let mut pages = self.pages.lock();
...
impl Pager for PageCacheManager {
    fn commit_page(&self, idx: usize) -> Result<ExternalFrame> {
    ...
    fn commit_overwrite(&self, idx: usize) -> Result<ExternalFrame> {
...
impl Page {
    ...
    pub fn frame(&self) -> &ExternalFrame {
        &self.frame
    }
...
impl dyn PageCacheBackend {
    ...
    fn read_page(&self, idx: usize, frame: &ExternalFrame) -> Result<()> {
    ...
    fn write_page(&self, idx: usize, frame: &ExternalFrame) -> Result<()> {
