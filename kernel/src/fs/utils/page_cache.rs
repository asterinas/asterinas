...
use lru::LruCache;
use ostd::mm::{Frame, FrameAllocOptions, UntypedMeta, VmIo};
...
impl PageCacheManager {
    ...
    fn ondemand_readahead(&self, idx: usize) -> Result<Frame<dyn UntypedMeta>> {
        let mut pages = self.pages.lock();
...
impl Pager for PageCacheManager {
    fn commit_page(&self, idx: usize) -> Result<Frame<dyn UntypedMeta>> {
    ...
    fn commit_overwrite(&self, idx: usize) -> Result<Frame<dyn UntypedMeta>> {
...
impl Page {
    pub fn alloc() -> Result<Self> {
        let frame = FrameAllocOptions::new().zero_init(false).alloc_frame()?;
        Ok(Self {
            frame,
...
impl dyn PageCacheBackend {
    ...
    fn read_page(&self, idx: usize, frame: &Frame<dyn UntypedMeta>) -> Result<()> {
    ...
    fn write_page(&self, idx: usize, frame: &Frame<dyn UntypedMeta>) -> Result<()> {
