...
use crate::{
    ...
    prelude::*,
    process::{
        signal::{constants::SIGIO, signals::kernel::KernelSignal, Poller},
        Pid, Process,
    },
};
...
impl FileTableEntry {
    ...
    pub fn set_owner(&mut self, owner: Option<&Arc<Process>>) -> Result<()> {
        ...
        let mut poller = Poller::with_observer(OwnerObserver::new(
            self.file.clone(),
            Arc::downgrade(process),
        ));
        self.file
            .poll(IoEvents::IN | IoEvents::OUT, Some(poller.as_dyn()));
...
type Owner = (Pid, Poller<OwnerObserver>);
