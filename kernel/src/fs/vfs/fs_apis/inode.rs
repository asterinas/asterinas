...
pub trait Inode: Any + InodeIo + Send + Sync {
    ...
    ///
    /// Returns `true` if the cached child still matches `name`. Returns `false` if the cached
    /// entry is stale and should be dropped before retrying the lookup.
    ...
    ///
    /// Returns `true` if `name` should still be treated as absent. Returns `false` if the
    /// negative cache entry is stale and should be dropped before retrying the lookup.
