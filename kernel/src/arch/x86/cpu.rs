...
impl CpuInformation {
    ...
    fn fill_cache_info_intel(&mut self) {
        ...
        if l3size != 0 {
            self.cache_size = Some(l3size);
        } else if l2size != 0 {
            self.cache_size = Some(l2size);
        } else if l1size != 0 {
            self.cache_size = Some(l1size);
        }
    }
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(
            ...
            "physical id\t: 0\n\
             ...
             apicid\t\t: {}\n\
             initial apicid\t: {}\n",
            self.cpu_cores, self.core_id, self.cpu_cores, self.apicid, self.apicid
        )?;
        ...
        // TODO: Fill the `vmx flags` field and the `bugs` field.
        write!(
            f,
            "\n\
             vmx flags\t:\n\
        ...
        // TODO: Fill the `power_management` field.
        writeln!(f, "power_management:")?;
