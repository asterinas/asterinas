...
impl VtKeyboardHandler {
    fn handle_key_event(&self, keycode: KeyCode, key_status: KeyStatus) {
        ...
        if keyboard_mode == KeyboardMode::Raw {
            // TODO: Add support for Raw keyboard mode.
            log::warn!("Raw keyboard mode is not implemented yet");
        } else if keyboard_mode == KeyboardMode::MediumRaw {
        ...
        if let Some(modifier) = self.keycode_to_modifier_key(keycode) {
            self.handle_modifier(modifier, key_status == KeyStatus::Pressed);
            return;
        }

        let key_sym = get_keysym(MODIFIER_KEYS_STATE.flags(), keycode);
