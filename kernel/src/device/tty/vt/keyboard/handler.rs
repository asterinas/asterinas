    ...
    fn handle_key_event(&self, keycode: KeyCode, key_status: KeyStatus) {
        ...
        match key_sym {
            ...
            KeySym::Special(SpecialHandler::DecreaseConsole) => {
                ...
                if let Err(e) = guard.dec_console() {
                    ostd::warn!("VT decreasing failed: {:?}", e);
                }
            }
            KeySym::Special(SpecialHandler::IncreaseConsole) => {
                ...
                if let Err(e) = guard.inc_console() {
                    ostd::warn!("VT increasing failed: {:?}", e);
                }
            }
            ...
            KeySym::SwitchVt(index) => {
                ...
                if let Err(e) = guard.switch_vt(index) {
                    ostd::warn!("VT switching failed: {:?}", e);
                }
            }
            KeySym::AltNumpad(_) => {
    ...
    fn handle_special(
        ...
        match handler {
            ...
            SpecialHandler::Enter => {
                let _ = if vt_keyboard.mode_flags().contains(KeyboardModeFlags::CRLF) {
                    vt.push_input(b"\r\n")
                } else {
                    vt.push_input(b"\r")
                };
