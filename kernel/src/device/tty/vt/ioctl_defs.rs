...
pub type GetKeyboardType = ioc!(KDGKBTYPE, 0x4B33, OutData<u8>);
...
/// Disallocates the specified VT if argument is non-zero, or release all VTs if argument
/// is zero. But we don't disallocate VT 1 since it's the default VT and always open.
pub type DisallocateVt = ioc!(VT_DISALLOCATE, 0x5608, InData<i32, PassByVal>);
