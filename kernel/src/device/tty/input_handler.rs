...
use super::{n_tty::ConsoleDriver, Tty};

/// Global buffer to store pending characters across multiple event calls.
static PENDING_CHARS: Mutex<Vec<u8>> = Mutex::new(Vec::new());

/// TTY handler class.
#[derive(Debug)]
struct TtyHandlerClass {
    name: String,
}

impl TtyHandlerClass {
    pub fn new() -> Self {
        Self {
            name: "tty".to_string(),
        }
...
impl InputHandlerClass for TtyHandlerClass {
    fn name(&self) -> &str {
        &self.name
    }
    ...
    fn connect(&self, dev: Arc<dyn InputDevice>) -> Result<Arc<dyn InputHandler>, ConnectError> {
        ...
        let tty = super::system_console();
        let handler = TtyHandler::new(dev.clone(), tty.clone());

        log::info!("TTY handler connected to device: {}", dev.name());
        Ok(Arc::new(handler))
    }
...
impl Debug for TtyHandler {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("TtyHandler")
...
impl TtyHandler {
    fn new(device: Arc<dyn InputDevice>, tty: Arc<Tty<ConsoleDriver>>) -> Self {
        Self {
            device,
            tty,
...
impl InputHandler for TtyHandler {
        ...
        for event in events {
            match event {
                InputEvent::Key(keycode, key_status) => {
                    ...
                    match keycode {
                        ...
                        KeyCode::KeyCapsLock => {
                            if *key_status == KeyStatus::Pressed {
                                let new_caps_lock = !self.caps_lock.load(Ordering::Relaxed);
                                self.caps_lock.store(new_caps_lock, Ordering::Relaxed);
                            }
                            continue;
                        }
                        _ => {}
                    ...
                    // Convert `KeyCode` to ASCII character or control sequence.
                    if let Some(bytes) = self.keycode_to_ascii(*keycode, shift_pressed, caps_lock) {
                        let mut pending_chars = PENDING_CHARS.lock();
                ...
                InputEvent::Sync(_) => {
                    ...
                    if !pending_chars.is_empty() {
                        let _ = self.tty.push_input(&pending_chars);
                        pending_chars.clear();
                    }
                }
                _ => {
...
/// Initializes TTY input handler support.
pub fn init() -> RegisteredInputHandlerClass {
    let handler_class = Arc::new(TtyHandlerClass::new());
