...
//!
//! A [`RouteEntry`] is the forwarding state stored *inside* one route table.
//! Its fields have three distinct roles:
//!
//! - `dst` is the lookup key. It is always a canonical network CIDR, and
//!   longest-prefix matching selects the most specific matching entry.
//! - `type_`, `oif_index`, and `gateway` describe the forwarding action.
//!   The type determines the route's handling, while the output interface and
//!   optional gateway determine where a unicast packet is sent.
//! - `protocol` and `scope` describe the route's origin and reachability for
//!   rtnetlink reporting. The current lookup implementation does not select a
//!   route by either field.
//!
//! A table ID is deliberately not part of this type: table membership is the
//! responsibility of the containing `RouteTable`.
//! When lookup or enumeration must report that membership, the route manager
//! returns a `RouteTableEntry`, which pairs a route with its containing table
...
/// destination matching, route type, output interface, and optional next-hop
/// gateway. The gateway is part of the route state because two otherwise
/// equivalent routes may forward matching packets through different next hops.
...
pub struct RouteEntry {
    ...
    type_: RouteType,
    /// Whether this is a transient lookup result for the IPv4 limited
    /// broadcast address rather than a route stored in a FIB table.
    limited_broadcast: bool,
}
...
impl RouteEntry {
    ...
    /// Returns the output interface index, if specified.
    pub fn oif_index(&self) -> Option<NonZeroU32> {
        self.oif_index
    }
    ...
    /// Converts a matching route into a limited-broadcast lookup result.
    pub(in crate::net) fn into_limited_broadcast(mut self) -> Self {
        self.type_ = RouteType::Broadcast;
        self.limited_broadcast = true;
        self
    }

    /// Returns whether this entry is a transient limited-broadcast result.
    pub(in crate::net) fn is_limited_broadcast(&self) -> bool {
        self.limited_broadcast
    }
    ...
    pub(super) fn matches_lookup(&self, key: &RouteLookupKey) -> bool {
        matches!(
            ...
            && key
                .oif_index()
                .is_none_or(|oif_index| self.oif_index == Some(oif_index))
    }
}
