...
impl NetlinkRouteSocket {
    ...
    fn try_receive(&self, writer: &mut dyn MultiWrite, flags: SendRecvFlags) -> Result<usize> {
        let inner = self.inner.read();
        ...
        bound.try_receive(writer, flags)
    }
}
...
impl Socket for NetlinkRouteSocket {
    ...
    fn sendmsg(
        ...
        if let Some(addr) = message_header.addr() {
            ...
            if *netlink_addr != NetlinkSocketAddr::new_unspecified() {
                return_errno_with_message!(
                    Errno::EINVAL,
                    "only kernel socket address is supported"
                );
            }
        }
        ...
        let res = bound.try_send(reader, flags);
    ...
    fn recvmsg(
        ...
        let received_len = self.block_on(IoEvents::IN, || self.try_receive(writers, flags))?;

        let message_header = {
            // TODO: For netlink route socket, the message seems always to be from the kernel socket.
            // We may need to further check this behavior.
