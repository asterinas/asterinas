...
use crate::{net::socket::netlink::route::message::attr::noattr::NoAttr, prelude::*};
...
// Implementing `SegmentBody` for DoneSegmentBody requires
// implementing `From<CRtGenMsg>` for `DoneSegmentBody`.
// However, this method will never be called.
impl From<CRtGenMsg> for DoneSegmentBody {
    fn from(_: CRtGenMsg) -> Self {
        unreachable!("Cannot convert `CRtGenMsg` to `DoneSegmentBody`")
    }
}

impl DoneSegment {
    pub fn new_from_request(request_header: &CMessageSegmentHeader, error: Option<Error>) -> Self {
...
// Implementing `SegmentBody` for `ErrorSegmentBody requires
// implementing `From<CRtGenMsg>` for `ErrorSegmentBody`.
// However, this method will never be called.
impl From<CRtGenMsg> for ErrorSegmentBody {
    fn from(_: CRtGenMsg) -> Self {
        unreachable!("cannot convert `CRtGenMsg` to `ErrorSegmentBody`");
    }
}
