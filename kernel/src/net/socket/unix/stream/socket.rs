...
impl UnixStreamSocket {
    ...
    pub fn new_pair(is_nonblocking: bool) -> (Arc<Self>, Arc<Self>) {
        let (conn_a, conn_b) = Connected::new_pair(None, None, false, false, Pollee::new());
        (
    ...
    fn check_io_events(&self) -> IoEvents {
        ...
        if state.is_write_shutdown() && !matches!(state.as_ref(), State::Listen(_)) {
