...
impl ConnectedStream {
    ...
    pub fn shutdown(&self, cmd: SockShutdownCmd, pollee: &Pollee) -> Result<()> {
        if cmd.shut_read() {
            self.is_receiving_closed.store(true, Ordering::Release);
        }

        if cmd.shut_write() {
            self.is_sending_closed.store(true, Ordering::Release);
    ...
    pub(super) fn update_io_events(&self, pollee: &Pollee) {
        self.bound_socket.raw_with(|socket| {
            ...
            let is_receiving_closed = self.is_receiving_closed.load(Ordering::Acquire);
            let is_sending_closed = self.is_sending_closed.load(Ordering::Acquire);
