...
impl StreamSocket {
    ...
    fn new_accepted(mut connected_stream: ConnectedStream) -> Arc<Self> {
        let options = {
            let mut options = OptionSet::new();

            if connected_stream
                .for_raw_socket(|raw_tcp_socket| raw_tcp_socket.keep_alive().is_some())
            {
                options.socket.set_keep_alive(true);
            }

            if !connected_stream.for_raw_socket(|raw_tcp_socket| raw_tcp_socket.nagle_enabled()) {
...
impl State {
    ...
    ///
    /// Note that for listening socket, this `f` is called on all backlog sockets in `Listen` State.
...
impl SetSocketLevelOption for State {
    fn set_keep_alive(&mut self, keep_alive: bool) {
        ...
        let set_keep_alive = |raw_socket: &mut RawTcpSocket| {
            if keep_alive {
                raw_socket.set_keep_alive(Some(KEEPALIVE_INTERVAL))
            } else {
                raw_socket.set_keep_alive(None);
            }
        };
        match self {
            State::Init(init_stream) => init_stream.for_raw_socket(set_keep_alive),
            State::Connecting(connecting_stream) => {
                connecting_stream.for_raw_socket(set_keep_alive)
            }
            State::Connected(connected_stream) => connected_stream.for_raw_socket(set_keep_alive),
            State::Listen(listen_stream) => listen_stream.for_each_backlog(set_keep_alive),
        }
    }
}
