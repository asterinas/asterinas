...
pub fn sys_pidfd_send_signal(
    ...
    let is_self = match &target {
        SignalTarget::Thread { tid: _, tgid } => *tgid == ctx.process.pid(),
    ...
    match pidfd {
        PIDFD_SELF_THREAD => match flags {
            PidfdSendSignalFlags::Default | PidfdSendSignalFlags::Thread => {
                Ok(thread_target(ctx.posix_thread.tid(), ctx.process.pid()))
            }
            PidfdSendSignalFlags::ThreadGroup => Ok(process_target(ctx.process.pid())),
            PidfdSendSignalFlags::ProcessGroup => Ok(group_target(ctx.process.pid())),
        },
        PIDFD_SELF_THREAD_GROUP => match flags {
            PidfdSendSignalFlags::Default | PidfdSendSignalFlags::ThreadGroup => {
                Ok(process_target(ctx.process.pid()))
            }
            PidfdSendSignalFlags::Thread => Ok(thread_target(
