...
pub fn sys_mount(
    ...
    if mount_flags.contains(MountFlags::MS_REMOUNT) && mount_flags.contains(MountFlags::MS_BIND) {
        ...
        do_change_type(dst_path, mount_flags)?;
...
fn do_change_type(target_path: Path, flags: MountFlags) -> Result<()> {
    ...
    if !(flags & !ALLOWED_FLAGS).is_empty() {
        return_errno_with_message!(Errno::EINVAL, "unsupported flags for propagation change");
    }
    ...
    if flags.contains(MountFlags::MS_PRIVATE) {
        ...
        target_path.set_propagation(MountPropType::Private, recursive);
        Ok(())
    } else {
        Err(Error::with_message(
            Errno::EINVAL,
            "unsupported propagation type",
        ))
    }
}
