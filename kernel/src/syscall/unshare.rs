...
pub fn sys_unshare(unshare_flags: u32, ctx: &Context) -> Result<SyscallReturn> {
    let flags = CloneFlags::from_bits(unshare_flags)
...
fn unshare_namespaces(flags: CloneFlags, ctx: &Context) -> Result<()> {
    ...
    if flags.contains(CloneFlags::CLONE_NEWUSER) {
        return_errno_with_message!(Errno::EINVAL, "unsharing user namespace is not supported");
    }

    let user_ns_ref = ctx.thread_local.borrow_user_ns();
    let user_ns = user_ns_ref.unwrap();
    ...
    let new_ns_context = thread_local_ns_context.new_child(user_ns, flags, ctx.posix_thread)?;
