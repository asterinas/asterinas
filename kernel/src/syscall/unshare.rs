...
pub fn sys_unshare(unshare_flags: u32, ctx: &Context) -> Result<SyscallReturn> {
    let flags = {
        ...
        // FIXME: It’s unclear whether we should apply implied flags here directly,
        // or delegate this responsibility to each unshare namespace implementation.
        // Could the current implementation cause any issues?
...
fn unshare_namespaces(flags: CloneFlags, ctx: &Context) -> Result<()> {
    ...
    let new_ns_context = thread_local_ns_context.clone_new(flags, ctx.posix_thread)?;
