...
use crate::{
    fs::path::FsPath, prelude::*, process::process_table, syscall::constants::MAX_FILENAME_LEN,
};
...
pub fn sys_pivot_root(
    ...
    // TODO: Locking the global PID table here is a workaround. We need to use a more
    // suitable lock (i.e. the global mount lock or the namespace lock) to avoid deadlock.
    process_table::with_pid_table(move |pid_table| {
        let fs_ref = ctx.thread_local.borrow_fs();
        let mut fs_resolver = fs_ref.resolver().write();

        fs_resolver.pivot_root(new_root_path, put_old_path, pid_table, ctx)
    })?;
