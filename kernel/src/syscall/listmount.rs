...
use crate::{fs::vfs::path::MountNamespace, prelude::*};
...
pub fn sys_listmount(
    ...
    let mnt_ns: Arc<MountNamespace> = {
        let proxy = ctx.thread_local.borrow_ns_proxy();
        proxy.unwrap().mnt_ns().clone()
    };
    ...
    let ids = mnt_ns.descendant_ids_of(&parent_mount, req.param, reverse, nr_mnt_ids);
