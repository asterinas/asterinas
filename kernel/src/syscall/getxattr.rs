...
use crate::{
    ...
    prelude::*,
    process::credentials::{FileCapabilities, Uid, VfsCapFlags, VfsCapRevision},
...
///
/// FIXME: Apply filesystem-user-namespace and idmapped-mount UID mappings when
...
fn convert_file_cap(raw_value: &mut [u8]) -> Result<usize> {
    let magic_etc = FileCapabilities::read_u32_le(raw_value, 0)?;
    let (revision, _) = FileCapabilities::parse_header(magic_etc, raw_value.len())?;
    ...
    match revision {
        ...
        VfsCapRevision::V3 => {
            let root_uid = Uid::new(FileCapabilities::read_u32_le(raw_value, 5)?);
            ...
            if root_uid.is_root() {
                let converted_header = VfsCapRevision::V2 as u32 | (magic_etc & VfsCapFlags::MASK);
