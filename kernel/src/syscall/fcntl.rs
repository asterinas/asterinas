...
fn handle_addseal(fd: FileDesc, arg: u64, ctx: &Context) -> Result<SyscallReturn> {
    let mut file_table = ctx.thread_local.borrow_file_table_mut();
    let file = get_file_fast!(&mut file_table, fd);
    let mut file_seals = file.seals()?.lock();
    ...
    // Since this file is sealable, we can assume that it is a `MemfdFile`.
    // Then the `unwrap`s below are safe to use.
...
fn handle_getseal(fd: FileDesc, ctx: &Context) -> Result<SyscallReturn> {
    let mut file_table = ctx.thread_local.borrow_file_table_mut();
    let file = get_file_fast!(&mut file_table, fd);
    let file_seals = file.seals()?.lock();
