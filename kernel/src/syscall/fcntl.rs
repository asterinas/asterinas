...
use crate::{
    fs::{
        ...
        ramfs::memfd::{FileSeals, do_add_seals, do_get_seals},
...
fn handle_addseal(fd: FileDesc, arg: u64, ctx: &Context) -> Result<SyscallReturn> {
    ...
    do_add_seals(file.as_inode_handle_or_err()?, new_seals)?;
...
fn handle_getseal(fd: FileDesc, ctx: &Context) -> Result<SyscallReturn> {
    ...
    let file_seals = do_get_seals(file.as_inode_handle_or_err()?)?;
