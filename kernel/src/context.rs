...
impl<'a> CurrentUserSpace<'a> {
    ...
    ///
    /// This method is _not_ recommended for use, as it does not verify whether the current
    /// task has a binding `Vmar`.
    ///
    ...
    pub fn reader(&self, vaddr: Vaddr, len: usize) -> Result<VmReader<'_, Fallible>> {
        Ok(self.0.as_ref().unwrap().vm_space().reader(vaddr, len)?)
    }
    ...
    pub fn writer(&self, vaddr: Vaddr, len: usize) -> Result<VmWriter<'_, Fallible>> {
        Ok(self.0.as_ref().unwrap().vm_space().writer(vaddr, len)?)
    }
