...
use ostd::{
    mm::{Frame, UntypedMeta, VmIo, PAGE_SIZE},
    sync::SpinLock,
    Pod,
};
use spin::Once;
...
struct Vdso {
    ...
    vmo: Arc<Vmo>,
    /// The `Frame<dyn UntypedMeta>` that contains the VDSO data. This frame is contained in and
    /// will not be removed from the VDSO VMO.
    data_frame: Frame<dyn UntypedMeta>,
}
