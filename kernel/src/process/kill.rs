...
fn current_thread_sender_ids(signum: Option<&SigNum>, ctx: &Context) -> SignalSenderIds {
    ...
    let session = signum.and_then(|signum| {
        if *signum == SIGCONT {
            ctx.process.session()
        } else {
            None
    ...
    SignalSenderIds::new(ruid, euid, session)
}
...
impl SignalSenderIds {
    fn new(ruid: Uid, euid: Uid, session: Option<Arc<Session>>) -> Self {
        Self {
            ruid,
            euid,
            session,
