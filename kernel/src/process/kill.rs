...
/// any signal.
pub fn kill<S: Signal + Clone>(pid: Pid, signal: Option<S>, ctx: &Context) -> Result<()> {
...
pub fn kill_group<S: Signal + Clone>(pgid: Pgid, signal: Option<S>, ctx: &Context) -> Result<()> {
    ...
    for process in inner.iter() {
        let res = kill_process(process, signal.clone(), ctx);
...
fn kill_process<S: Signal + Clone>(
    process: &Process,
    signal: Option<S>,
    ctx: &Context,
) -> Result<()> {
