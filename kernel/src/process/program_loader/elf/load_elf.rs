...
pub struct ElfLoadInfo {
    /// Relocated entry point.
    pub entry_point: Vaddr,
    /// Relocated address of the user stack top.
...
pub fn map_segment_vmos(
    ...
    let map_range = if elf.is_shared_object() {
        ...
        aligned_range.start + (elf_va_range.start - elf_va_range_aligned.start)
            ..aligned_range.end - (elf_va_range_aligned.end - elf_va_range.end)
    } else {
...
impl RelocatedRange {
    ...
    pub fn new(original_range: Range<Vaddr>, relocated_start: Vaddr) -> Option<Self> {
        if relocated_start.checked_add(original_range.len()).is_none() {
            return None;
        }
        Some(Self {
            original_range,
