...
/// User-provided signal stack. Signal stack is per-thread, and each thread can have
/// at most one signal stack. If one signal handler specifying the `SA_ONSTACK` flag,
...
impl From<SigStackStatus> for SigStackFlags {
    fn from(value: SigStackStatus) -> Self {
        match value {
            SigStackStatus::Inactive => Self::empty(),
            SigStackStatus::Active => Self::SS_ONSTACK,
            SigStackStatus::Disable => Self::SS_DISABLE,
        }
    }
}
