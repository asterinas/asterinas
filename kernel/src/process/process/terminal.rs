...
impl dyn Terminal {
    ...
    fn set_foreground(self: Arc<Self>, pgid: Pgid, current: &CurrentProcess) -> Result<()> {
        // Lock order: task list -> group of process -> session inner -> job control
        let mut task_list_guard = TASK_LIST_LOCK.lock();
        let mut process_group_mut = current.process_group.lock();

        let pid_entry = current.pid_namespace().get_entry_by_id(pgid);

        let pid_entry_guard = pid_entry
            .as_ref()
            .map(|pid_entry| pid_entry.with_task_list_guard(&mut task_list_guard));

        self.is_control_and(&mut process_group_mut, |session, _| {
            let Some(process_group) = pid_entry_guard
                .as_ref()
                .and_then(|pid_entry_guard| pid_entry_guard.attached_process_group())
            else {
                return_errno_with_message!(
                    Errno::ESRCH,
