...
use super::{
    pid_namespace::{
        MapsOfProcess, PidNamespace, UniqueIdArray, UniqueIdsMapWithTasklistGuard, INIT_PROCESS_PID,
    },
...
impl ParentProcess {
    pub fn new(process: Weak<Process>, pid_ns: Arc<PidNamespace>) -> Self {
        let pid = match process.upgrade() {
            Some(process) => pid_ns.get_current_id(&process.unique_ids).unwrap_or(0),
            None => 0,
        };
...
impl ParentProcessGuard<'_> {
    ...
    pub fn set_process(&mut self, new_process: &Arc<Process>) {
        let pid = self
            .this
            .pid_ns
            .get_current_id(&new_process.unique_ids)
            .unwrap_or(0);
...
impl Process {
    ...
    pub fn parent_pid_in_ns(&self, pid_ns: &Arc<PidNamespace>) -> Option<Pid> {
        ...
        pid_ns.get_current_id(&parent.process().upgrade()?.unique_ids)
    }
    ...
    // FIXME: If we call this method without holding the task list
    // lock, it may return `None` if the process is reaped at the same time.
    pub fn session_unique_ids(&self) -> Option<UniqueIdArray> {
        self.process_group
            .lock()
            .upgrade()?
            .session()
            .map(|session| session.unique_ids().clone())
    }
    ...
    pub(super) fn clear_old_group_and_session(
        ...
        maps_of_process: &mut MapsOfProcess,
    ) {
    ...
    fn set_new_session(
        self: &Arc<Self>,
        process_group_mut: &mut MutexGuard<Weak<ProcessGroup>>,
        map_guard: &mut UniqueIdsMapWithTasklistGuard,
    ) -> Sid {
        ...
        **process_group_mut = Arc::downgrade(&process_group);
        ...
        map_guard.attach_session(session);
        map_guard.attach_process_group(process_group);
    ...
    fn to_existing_group(
        ...
        let (mut process_group_inner, mut new_group_inner) = match process_group
            .unique_ids()
            .cmp(&new_process_group.unique_ids())
        {
