...
pub struct Process {
    ...
    cgroup: Mutex<Option<Weak<CgroupNode>>>,
...
impl Process {
    ...
    pub fn cgroup(&self) -> Option<Arc<CgroupNode>> {
        self.cgroup
            .lock()
            .as_ref()
    ...
    pub fn bind_cgroup(&self, cgroup: Option<Weak<CgroupNode>>) -> Option<Arc<CgroupNode>> {
        let mut cgroup_guard = self.cgroup.lock();
        let old_cgroup = cgroup_guard
            .as_ref()
