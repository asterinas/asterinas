...
#[derive(Debug)]
pub struct Heap {
    inner: RwMutex<Option<HeapInner>>,
}

#[derive(Clone, Debug)]
struct HeapInner {
    /// The data segment range of the process.
...
impl Heap {
    ...
    pub fn modify_heap_end(
        ...
        if new_heap_end < heap_start
            || check_data_rlimit(&inner.data_range, &new_heap_range, ctx).is_err()
        {
        ...
        if old_size < new_size {
            ...
            // FIXME: There we don't consider the case that the mapped heap region contains
...
impl Clone for Heap {
    fn clone(&self) -> Self {
        Self {
            inner: RwMutex::new(self.inner.read().clone()),
        }
    }
}
...
fn check_data_rlimit(
    ...
    if data_range.len() as u64 + new_heap_range.len() as u64 > rlimit_data.get_cur() {
        return_errno_with_message!(Errno::ENOSPC, "data segment size limit was met.");
    }
    Ok(())
}
