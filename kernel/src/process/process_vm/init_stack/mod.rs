...
impl InitStackReader<'_> {
    ...
    pub fn argc(&self) -> Result<u64> {
        ...
        match self.process_vm.read_remote(
            self.init_stack_bottom(),
            &mut VmWriter::from(&mut buffer[..]).to_fallible(),
        ) {
            Ok(bytes) if bytes == buffer.len() => {}
            Ok(_) => return_errno_with_message!(Errno::EIO, "Failed to read full argc"),
    ...
    pub fn argv(&self) -> Result<Vec<u8>> {
        ...
        match self.process_vm.read_remote(
            ...
            Ok(bytes) if bytes == buffer.len() => {}
            Ok(_) => return_errno_with_message!(Errno::EIO, "Failed to read full argv"),
            Err((err, _)) => return Err(err),
        }
