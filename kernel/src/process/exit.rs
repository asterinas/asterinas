...
fn find_reaper_process(current_process: &Process) -> Option<Arc<Process>> {
    ...
    while let Some(process) = parent.upgrade() {
        ...
        let is_reaper = process.is_child_subreaper().load(Ordering::Acquire);
        if is_reaper {
            return Some(process);
        }
...
/// Copies the children of `current_process` to be the children of `reaper_process`.
///
/// If the `reaper_process` is zombie, returns `Err(())`.
fn copy_process_children(
...
fn move_children_to_reaper_process(current_process: &Process) {
    ...
    while let Some(reaper_process) = find_reaper_process(current_process) {
        if copy_process_children(current_process, &reaper_process).is_ok() {
            return;
        }
    }
    ...
    let _ = copy_process_children(current_process, &init_process);
}
