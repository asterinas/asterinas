...
/// 1. The user namespace, which is included in the `Process` struct (TODO).
...
impl NsProxy {
    /// Returns a reference to the singleton init `NsProxy`.
    pub(super) fn get_init_singleton() -> &'static Arc<Self> {
