...
impl Credentials_ {
    ...
    pub(super) fn prepare_exec_cred(
        ...
        let had_file_capabilities = file_capabilities.is_some();
        let (file_capabilities, setuid, setgid) = if self.no_new_privs() {
            (None, None, None)
        } else {
            (file_capabilities, setuid, setgid)
        };

        let new_euid = setuid.unwrap_or_else(|| self.euid());
        let new_egid = setgid.unwrap_or_else(|| self.egid());
        let (permitted, effective, ambient) = self.calculate_capsets_for_exec(
            file_capabilities,
            had_file_capabilities,
            new_euid,
            new_egid,
        );
        ...
        if file_capabilities.is_some_and(|file_capabilities| {
            file_capabilities.has_effective_flag()
                && !permitted.contains(file_capabilities.permitted())
        }) {
            return_errno_with_message!(
                Errno::EPERM,
    ...
    fn calculate_capsets_for_exec(
        ...
        // The original presence of file capabilities clears ambient capabilities even if
        // `no_new_privs` suppresses the resulting privilege transition.
        let new_ambient = if had_file_capabilities || id_changed {
            CapSet::empty()
        } else {
            self.ambient_capset()
        };
        ...
        let grant_root_capability_sets =
            !no_root && (self.ruid().is_root() || (!has_file_capabilities && new_euid.is_root()));
        ...
        let grant_root_effective_set = grant_root_capability_sets && new_euid.is_root();
        ...
        (new_permitted, new_effective, new_ambient)
    }
