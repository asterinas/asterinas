...
bitflags! {
    ...
    pub struct SecureBits: u16 {
        ...
        /// Make `NOROOT` bit locked (irreversible).
        ...
        /// Make `NO_SETUID_FIXUP` bit locked (irreversible).
        ...
        /// Make `KEEP_CAPS` bit locked (irreversible).
        ...
        /// Make `NO_CAP_AMBIENT_RAISE` bit locked (irreversible).
...
impl AtomicSecureBits {
    ...
    pub(super) fn try_store(&self, bits: SecureBits, ordering: Ordering) -> Result<()> {
        ...
        // Preserve locked bits.
        let bits =
            SecureBits::from_bits_truncate(bits.bits() | (current.bits() & SecureBits::LOCK_MASK));
