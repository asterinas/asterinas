...
use crate::{
    ...
    process::{
        ...
        process_vm::{unshare_vmar_and_map_heap, MAX_LEN_STRING_ARG, MAX_NR_STRING_ARGS},
...
pub fn do_execve(
    ...
    // state may be left inconsistent and it is unsafe to return to user mode.
...
fn do_execve_no_return(
    ...
    // then promote the current thread to be the process's main thread.
    wait_other_threads_exit(ctx)?;
    make_current_main_thread(ctx);

    // Reset the virtual memory state.
    unshare_vmar_and_map_heap(ctx);
