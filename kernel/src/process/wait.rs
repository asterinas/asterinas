...
use crate::{
    prelude::*,
    process::{posix_thread::AsPosixThread, process::ProcessAttachment},
};
...
fn reap_zombie_child(process: &Process, pid: Pid) -> ExitCode {
    ...
    // Lock order: group_of_process -> attachments
    let mut child_group_mut = child_process.process_group.lock();
    let attachment = {
        let tid = child_process.nested_id();
        ProcessAttachment::get_all(tid, &child_process, &mut child_group_mut)
    };
    let mut attachment_guard = attachment.lock(&mut child_group_mut);
    ...
    for task in child_process.tasks().lock().as_slice() {
        ...
        debug_assert_eq!(
            posix_thread.tid(),
            child_process
                .pid_in_ns(&child_process.pid_namespace())
                .unwrap()
        );

        attachment_guard.detach_thread();
    }

    // Remove the process from the global table
    attachment_guard.detach_process();

    // Remove the process group and the session from global table, if necessary
    child_process.clear_old_group_and_session(&mut child_group_mut, &mut attachment_guard);
