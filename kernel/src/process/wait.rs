...
use crate::{
    prelude::*,
    process::{pid_namespace::ProcessPidEntries, posix_thread::AsPosixThread, TASK_LIST_LOCK},
};
...
fn reap_zombie_child(process: &Process, pid: Pid) -> ExitCode {
    ...
    // Lock order: task list -> group_of_process
    ...
    let mut process_pid_entries =
        ProcessPidEntries::get_entries(&child_process, task_list_guard, &mut child_group_mut);
    ...
    for task in child_process.tasks().lock().as_slice() {
        ...
        process_pid_entries.detach_thread();
    }

    // Remove the process from the global table
    process_pid_entries.detach_process();

    // Remove the process group and the session from global table, if necessary
    child_process.clear_old_group_and_session(&mut child_group_mut, &mut process_pid_entries);
