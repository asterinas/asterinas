...
use super::{
    pid_namespace::{AncestorNsPids, TASK_LIST_LOCK},
...
fn clone_child_task(
    ...
    let child_task = {
        ...
        let child_ns_tids = pid_ns.allocate_ids();
        let child_tid = pid_ns.get_current_id(&child_ns_tids).unwrap();
        ...
        let mut thread_builder =
            PosixThreadBuilder::new(child_tid, child_user_ctx, credentials, child_ns_tids)
    ...
    let child_pid_entry = {
        let child_ns_tids = child_task.as_posix_thread().unwrap().ns_tids();
        process
            .pid_namespace()
            .get_entry_by_ids(&child_ns_tids)
            .unwrap()
    };
    child_pid_entry
...
fn clone_child_process(
    ...
    let child_ns_pids = child_pid_ns.allocate_ids();
    let child_pid = child_pid_ns.get_current_id(&child_ns_pids).unwrap();
...
fn set_parent_and_group(parent: &Process, child: &Arc<Process>) {
    // Lock order: children of process -> task list-> group of process
    // -> group inner
    ...
    let child_pid_entry = child
        .pid_namespace()
        .get_entry_by_ids(child.ns_pids())
        .unwrap();
    let mut child_pid_entry_guard = child_pid_entry.with_task_list_guard(&mut task_list_guard);
    ...
    *child.process_group.lock() = Arc::downgrade(&process_group);
    ...
    child_pid_entry_guard.attach_thread(child.main_thread());
    child_pid_entry_guard.attach_process(child.clone());
}
