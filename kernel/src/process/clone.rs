...
fn clone_child_task(
    ...
    // Clone namespaces
    let child_user_ns = thread_local.borrow_user_ns().unwrap().clone();
    let child_ns_context = clone_ns_context(
...
fn clone_user_ns(
    ...
    if clone_flags.contains(CloneFlags::CLONE_NEWUSER) {
        return_errno_with_message!(Errno::EINVAL, "cloning new user namespace is not supported");
    } else {
        Ok(thread_local.borrow_user_ns().unwrap().clone())
    }
}

fn clone_ns_context(
    parent_ns_context: &Arc<NsContext>,
    user_ns: &Arc<UserNamespace>,
    clone_flags: CloneFlags,
    posix_thread: &PosixThread,
) -> Result<Arc<NsContext>> {
    parent_ns_context.new_child(user_ns, clone_flags, posix_thread)
}
