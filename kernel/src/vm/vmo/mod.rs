...
use ostd::{
    ...
    mm::{Frame, FrameAllocOptions, UntypedMem, UntypedMeta, VmReader, VmWriter},
};
...
/// around its low-level counterpart [`ostd::mm::Frame<dyn UntypedMeta>`].
/// Compared with `Frame<dyn UntypedMeta>`,
...
/// `Pages` is the struct that manages the `Frame<dyn UntypedMeta>`s stored in `Vmo_`.
pub(super) enum Pages {
    /// `Pages` that cannot be resized. This kind of `Pages` will have a constant size.
    Nonresizable(Mutex<XArray<Frame<dyn UntypedMeta>>>, usize),
    /// `Pages` that can be resized and have a variable size.
    Resizable(Mutex<(XArray<Frame<dyn UntypedMeta>>, usize)>),
}
...
impl Pages {
    ...
    where
        F: FnOnce(&mut XArray<Frame<dyn UntypedMeta>>, usize) -> R,
    {
        match self {
...
impl Vmo_ {
    /// Prepares a new `Frame<dyn UntypedMeta>` for the target index in pages, returns this new frame.
    fn prepare_page(&self, page_idx: usize) -> Result<Frame<dyn UntypedMeta>> {
        match &self.pager {
    ...
    /// Prepares a new `Frame<dyn UntypedMeta>` for the target index in the VMO, returns this new frame.
    fn prepare_overwrite(&self, page_idx: usize) -> Result<Frame<dyn UntypedMeta>> {
    ...
    fn commit_with_cursor(
        &self,
        cursor: &mut CursorMut<'_, Frame<dyn UntypedMeta>>,
        commit_flags: CommitFlags,
    ) -> Result<Frame<dyn UntypedMeta>> {
        let new_page = {
    ...
    pub fn commit_page(&self, offset: usize) -> Result<Frame<dyn UntypedMeta>> {
    ...
    where
        F: FnMut(&mut dyn FnMut() -> Result<Frame<dyn UntypedMeta>>) -> Result<()>,
    {
        self.pages.with(|pages, size| {
            if range.end > size {
    ...
    pub fn read(&self, offset: usize, writer: &mut VmWriter) -> Result<()> {
        ...
        let read = move |commit_fn: &mut dyn FnMut() -> Result<Frame<dyn UntypedMeta>>| {
            let frame = commit_fn()?;
    ...
    pub fn write(&self, offset: usize, reader: &mut VmReader) -> Result<()> {
        ...
        let mut write = move |commit_fn: &mut dyn FnMut() -> Result<Frame<dyn UntypedMeta>>| {
            let frame = commit_fn()?;
    ...
    fn decommit_pages(
        &self,
        pages: &mut XArray<Frame<dyn UntypedMeta>>,
        range: Range<usize>,
    ) -> Result<()> {
        let page_idx_range = get_page_idx_range(&range);
        let mut cursor = pages.cursor_mut(page_idx_range.start as u64);
    ...
    fn replace(&self, page: Frame<dyn UntypedMeta>, page_idx: usize) -> Result<()> {
        self.pages.with(|pages, size| {
