...
use ostd::{
    ...
    mm::{ExternalFrame, FrameAllocOptions, VmReader, VmWriter},
};
...
/// around its low-level counterpart [`ostd::mm::ExternalFrame`].
/// Compared with `ExternalFrame`,
...
/// `Pages` is the struct that manages the `ExternalFrame`s stored in `Vmo_`.
pub(super) enum Pages {
    /// `Pages` that cannot be resized. This kind of `Pages` will have a constant size.
    Nonresizable(Mutex<XArray<ExternalFrame>>, usize),
    /// `Pages` that can be resized and have a variable size.
    Resizable(Mutex<(XArray<ExternalFrame>, usize)>),
}
...
impl Pages {
    ...
    where
        F: FnOnce(&mut XArray<ExternalFrame>, usize) -> R,
    {
        match self {
...
impl Vmo_ {
    /// Prepares a new `ExternalFrame` for the target index in pages, returns this new frame.
    fn prepare_page(&self, page_idx: usize) -> Result<ExternalFrame> {
        match &self.pager {
    ...
    /// Prepares a new `ExternalFrame` for the target index in the VMO, returns this new frame.
    fn prepare_overwrite(&self, page_idx: usize) -> Result<ExternalFrame> {
    ...
    fn commit_with_cursor(
        &self,
        cursor: &mut CursorMut<'_, ExternalFrame>,
        commit_flags: CommitFlags,
    ) -> Result<ExternalFrame> {
        let new_page = {
    ...
    pub fn commit_page(&self, offset: usize) -> Result<ExternalFrame> {
    ...
    where
        F: FnMut(&mut dyn FnMut() -> Result<ExternalFrame>) -> Result<()>,
    {
        self.pages.with(|pages, size| {
            if range.end > size {
    ...
    pub fn read(&self, offset: usize, writer: &mut VmWriter) -> Result<()> {
        ...
        let read = move |commit_fn: &mut dyn FnMut() -> Result<ExternalFrame>| {
            let frame = commit_fn()?;
    ...
    pub fn write(&self, offset: usize, reader: &mut VmReader) -> Result<()> {
        ...
        let mut write = move |commit_fn: &mut dyn FnMut() -> Result<ExternalFrame>| {
            let frame = commit_fn()?;
    ...
    fn decommit_pages(&self, pages: &mut XArray<ExternalFrame>, range: Range<usize>) -> Result<()> {
        let page_idx_range = get_page_idx_range(&range);
        let mut cursor = pages.cursor_mut(page_idx_range.start as u64);
        for page_idx in page_idx_range {
            if cursor.remove().is_some()
    ...
    fn replace(&self, page: ExternalFrame, page_idx: usize) -> Result<()> {
        self.pages.with(|pages, size| {
