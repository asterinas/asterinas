...
use ostd::{
    collections::xarray::{Cursor, LockedXArray, XArray},
...
impl Vmo_ {
    ...
    fn try_commit_with_cursor(
        ...
        if let Some(pager) = &self.pager {
            // TODO: Here `Vmo` treat all instructions in `pager` as I/O instructions
            // since it needs to take the inner `Mutex` lock and users cannot hold a `SpinLock`
            // to do such instructions. In the future we should solve the redundancy of `Vmo` and
            // the pagecache make sure return such error when really needing I/Os.
    ...
    ///
    /// Once a commit operation needs to perform I/O, it will return a `VmoCommitError`.
    ...
    pub fn resize(&self, new_size: usize) -> Result<()> {
        ...
        let locked_pages = self.pages.lock();
        let old_size = self.size.swap(new_size, Ordering::Release);
