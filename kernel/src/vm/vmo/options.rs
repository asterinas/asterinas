...
use ostd::{
    ...
    mm::{Frame, FrameAllocOptions, Segment, UntypedMeta},
};
...
fn committed_pages_if_continuous(
    flags: VmoFlags,
    size: usize,
) -> Result<XArray<Frame<dyn UntypedMeta>>> {
    if flags.contains(VmoFlags::CONTIGUOUS) {
        ...
        let segment: Segment<dyn UntypedMeta> =
            FrameAllocOptions::new().alloc_segment(frames_num)?.into();
