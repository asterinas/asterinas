...
impl VmMapping {
    ...
    /// # Notice
    ///
    /// The populate operation is **non-transactional**. It attempts to
    /// prefault the specified pages on a **best-effort** basis. If an error
    /// occurs during population, the pages that were successfully populated
    /// remain mapped, and the errors do not cause the associated mmap() call
    /// to fail.
...
fn try_merge(left: &VmMapping, right: &VmMapping) -> Option<VmMapping> {
    ...
    let vmo = match (&left.vmo, &right.vmo) {
        ...
        (MappedVmObj::IoMem(l_iomem), MappedVmObj::IoMem(r_iomem)) => {
            let is_offset_contiguous = l_iomem.range.start + left.map_size() == r_iomem.range.start
