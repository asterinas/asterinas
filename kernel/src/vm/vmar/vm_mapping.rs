...
use ostd::mm::{
    ...
    PageProperty, UntypedMeta, VmSpace,
};
...
impl VmMapping {
    ...
    fn prepare_page(
        ...
        write: bool,
    ) -> Result<(Frame<dyn UntypedMeta>, bool)> {
    ...
    fn handle_page_faults_around(&self, vm_space: &VmSpace, page_fault_addr: Vaddr) -> Result<()> {
        ...
        let operate = move |commit_fn: &mut dyn FnMut() -> Result<Frame<dyn UntypedMeta>>| {
...
impl MappedVmo {
    ...
    fn get_committed_frame(&self, page_offset: usize) -> Result<Frame<dyn UntypedMeta>> {
