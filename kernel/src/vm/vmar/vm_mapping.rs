...
use ostd::mm::{
    tlb::TlbFlushOp, vm_space::VmItem, CachePolicy, ExternalFrame, FrameAllocOptions, PageFlags,
...
impl VmMapping {
    ...
    fn prepare_page(&self, page_fault_addr: Vaddr, write: bool) -> Result<(ExternalFrame, bool)> {
    ...
    fn handle_page_faults_around(&self, vm_space: &VmSpace, page_fault_addr: Vaddr) -> Result<()> {
        ...
        let operate = move |commit_fn: &mut dyn FnMut() -> Result<ExternalFrame>| {
...
impl MappedVmo {
    ...
    fn get_committed_frame(&self, page_offset: usize) -> Result<ExternalFrame> {
