...
use crate::vm::vmar::{
    interval_set::Interval, util::get_intersected_range, vm_allocator::PerCpuAllocatorGuard,
    vm_mapping::VmMapping, vmar_impls::RsAsDelta,
};
...
pub(super) fn unmap_count_rs_as(
    ...
    allocator: &mut Option<&mut PerCpuAllocatorGuard<'_>>,
) -> usize {
    ...
    while let Some(vm_mapping) = find_next_mapped!(cursor, end) {
        ...
        rs_as_delta.add_as(-(vm_mapping_range.len() as isize));

        total_frames_unmapped += frames_mapped;

        if let Some(allocator) = allocator.as_mut() {
            allocator.dealloc(vm_mapping_range.clone());
        }
...
pub fn check_range_not_mapped(cursor: &mut VmarCursorMut, range: Range<Vaddr>) {
    ...
    if let Some(vm_mapping) = find_next_mapped!(cursor, range.end) {
        panic!(
            "existing VM mapping found in range to clear: {:?}",
            vm_mapping
        );
    }
    cursor.jump(range.start).unwrap();
    if let Some(va) = cursor.find_next(range.len()) {
        panic!(
            "existing page table mapping found in range to clear: {:?}",
            va
        );
    }
}
