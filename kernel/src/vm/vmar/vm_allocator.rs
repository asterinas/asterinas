...
pub fn lock_and_alloc<'a>(
    ...
    if let Ok((va, cursor)) = try_lock_and_alloc_from(guard, vmspace, &assigned_range, size, align)
    {
        return Ok((va, cursor));
    }
...
pub fn lock_covering_range_and_alloc<'a>(
    ...
    if let Ok((va, cursor)) = try_lock_and_alloc_from(guard, vmspace, &range, size, align) {
        return Ok((va, cursor));
    }
