...
use crate::{
    prelude::*,
    vm::vmar::interval_set::{Interval, IntervalSet},
};
...
impl RangeAllocator {
    ...
    pub fn new(full_range: Range<Vaddr>) -> Self {
        let mut allocator = Self::new_empty();
    ...
    pub fn reset(&mut self, full_range: Range<Vaddr>) {
        self.freelist.clear();
        if !full_range.is_empty() {
            self.freelist.insert(FreeRange::new(full_range));
        }
    }
    ...
    pub fn alloc_specific(&mut self, range: Range<Vaddr>) -> Result<()> {
        if range.is_empty() {
    ...
    /// [`Self::add_range_try_merge`] first.
    ///
    /// The number of bytes overwritten is returned.
    pub fn alloc_specific_overwrite(&mut self, range: Range<Vaddr>) -> Result<usize> {
        if range.is_empty() {
            return_errno_with_message!(Errno::EINVAL, "cannot allocate empty range");
        }

        let mut overwritten_size = range.len();

        let mut remnant = Vec::new();

        for free in self.freelist.take(&range) {
            let (before, overlapping, after) = free.truncate(range.clone());
            if let Some(before) = before {
                remnant.push(before);
            }
            overwritten_size -= overlapping.unwrap().range().len();
            if let Some(after) = after {
                remnant.push(after);
            }
        }

        remnant.into_iter().for_each(|r| self.freelist.insert(r));
    ...
    pub fn count_free_size_overlapping(&self, range: &Range<Vaddr>) -> usize {
        ...
        for free in self.freelist.find(range) {
            let free_range = free.range();
            if range_overlaps(&free_range, range) {
                let overlap_start = free_range.start.max(range.start);
                let overlap_end = free_range.end.min(range.end);
                total_free_size += overlap_end - overlap_start;
            }
        }
...
mod test {
    ...
    fn alloc_specific_overwrite() {
        let mut allocator = RangeAllocator::new(0..128);
        allocator
            .alloc_specific(32..64)
            .expect("initial specific allocation succeeds");
        allocator
            .alloc_specific(80..96)
            .expect("initial specific allocation succeeds");

        let overwritten_size = allocator
            .alloc_specific_overwrite(48..88)
            .expect("overwrite allocation should succeed");
        assert_eq!(overwritten_size, 24);

        let free_ranges: Vec<Range<Vaddr>> =
            allocator.freelist.iter().map(|free| free.range()).collect();
        assert_eq!(free_ranges, vec![0..32, 96..128]);
    }
    ...
    fn count_free_size_overlapping() {
        ...
        let count1 = allocator.count_free_size_overlapping(&(16..80));
        assert_eq!(count1, 32);

        let count2 = allocator.count_free_size_overlapping(&(32..64));
        assert_eq!(count2, 0);

        let count3 = allocator.count_free_size_overlapping(&(0..128));
        assert_eq!(count3, 64);
    }
}
