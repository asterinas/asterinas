...
use crate::{
    ...
    vm::vmar::{
        ...
        vm_allocator::lock_covering_range_and_alloc,
...
impl AllocatedRemapOp {
    ...
    fn alloc_from<'a>(
        ...
        let (move_op, cursor) = if let Some(new_addr) = new_addr {
            ...
            let cursor = vmspace.cursor_mut(guard, &(lowest_addr..highest_addr))?;
            ...
            if let Some(new_end) = new_end_if_expand
                && let Ok(cursor) = lock_range_and_check_empty(
                    guard,
                    vmspace,
                    &(old_addr..new_end),
                    &(old_end..new_end),
                )
            {
                ...
                let (new_addr, cursor) = lock_covering_range_and_alloc(
                    guard,
                    vmspace,
                    old_addr..old_end,
                    new_size,
...
impl MoveRangeOp {
    fn perform<'a, 'rcu>(&self, cursor: &'a mut VmarCursorMut<'rcu>) -> Result<()> {
