...
impl<'a> VmarMapOptions<'a> {
    ...
    pub fn build(self) -> Result<Vaddr> {
        ...
        if offset.is_some_and(|(_, typ)| typ == OffsetType::Hint) {
            // For other types we check later after locking the PT.
            parent.check_extra_size_fits_rlimit(map_size.get())?;
        };
...
fn allocate_range<'a>(
    ...
    let (map_to_addr, mut cursor) = match offset {
        None => lock_and_alloc(preempt_guard, parent.vm_space(), map_size_bytes, align)?,
        ...
        Some((offset, OffsetType::Hint)) => {
            ...
            let cursor_checked = if find_next_mapped!(cursor, range.end).is_some() {
                drop(cursor);
                let (va, cursor_checked) =
                    lock_and_alloc(preempt_guard, parent.vm_space(), map_size_bytes, align)?;
                range = va..va + map_size_bytes;
                cursor_checked
            } else {
                cursor
            };

            (range.start, cursor_checked)
        }
    };
