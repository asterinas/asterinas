            ...
            Some(Mappable::Vmo(vmo)) => {
                if let Some(path) = &self.path {
                    debug_assert!(Arc::ptr_eq(
                        &vmo,
                        path.inode().page_cache().unwrap().as_vmo()
                    ));
                }

                let is_writable_tracked = if let Some(path) = &self.path
...
fn readahead_for_populate(vmo: Arc<Vmo>, vmo_offset: usize, map_size: NonZeroUsize) -> Result<()> {
    for offset in (vmo_offset..vmo_offset + map_size.get()).step_by(PAGE_SIZE) {
        vmo.commit_on(offset / PAGE_SIZE)?;
    }
    Ok(())
}
