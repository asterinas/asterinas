...
impl Vmar {
    ...
    /// or allocated.
    pub fn reserve(&self, range: Range<Vaddr>) -> Result<()> {
        self.allocator.alloc_specific(range).map(|_| ())
    }
}
...
impl<'a> VmarMapOptions<'a> {
    ...
    pub fn build(self) -> Result<Vaddr> {
        ...
        // We do not have a reliable way to check the RLIMIT when `mmap`s over
        // different ranges can be executed concurrently. So we just ensure
        // that the before-hand check is precise when the user synchronizes all
        // `mmap`s with a big lock.
        parent
            .check_extra_size_fits_rlimit(map_size.get())
            .or_else(|e| {
                if let Some((offset, OffsetType::Fixed)) = offset {
                    // `MAP_FIXED` may remove pages overlapped with requested mapping.
                    let free = parent
                        .allocator
                        .count_free_size_overlapping(&(offset..offset + map_size.get()));
                    parent.check_extra_size_fits_rlimit(free)
                } else {
                    Err(e)
                }
            })?;
    ...
    fn check_options(&self) -> Result<()> {
        ...
        if !self.size.get().is_multiple_of(self.align) {
...
fn allocate_range<'a>(
    ...
    let (allocator_guard, map_to_addr, mut cursor) = match offset {
        None => {
            let (allocator_guard, addr) = parent.allocator.alloc(map_size_bytes, align)?;
            let range = addr..addr + map_size_bytes;

            let cursor = parent.vm_space().cursor_mut(preempt_guard, &range).unwrap();
            (allocator_guard, addr, cursor)
        }
        Some((offset, OffsetType::Fixed)) => {
            ...
            let (allocator_guard, _) = parent.allocator.alloc_specific_overwrite(range.clone())?;

            let mut cursor = parent.vm_space().cursor_mut(preempt_guard, &range).unwrap();

            // We've already done `alloc_specific_overwrite`, so don't deallocate.
            parent.remove_mappings(&mut cursor, range.len(), &mut rs_as_delta, None)?;

            (allocator_guard, range.start, cursor)
        }
        Some((offset, OffsetType::FixedNoReplace)) => {
            let range = offset..offset + map_size_bytes;

            let allocator_guard = parent
                .allocator
                .alloc_specific(range.clone())
                .map_err(|_| {
                    Error::with_message(Errno::EEXIST, "mapping exists at fixed no-replace offset")
                })?;
        ...
        Some((offset, OffsetType::Hint)) => {
            let mut range = offset..offset + map_size_bytes;

            let allocator_guard = match parent.allocator.alloc_specific(range.clone()) {
                Ok(guard) => guard,
                Err(_) => {
                    let (guard, addr) = parent.allocator.alloc(map_size_bytes, align)?;
                    range = addr..addr + map_size_bytes;
                    guard
                }
            };
    ...
    // We can drop the allocator lock here to scale mapping operations that
    // happen on near but non-overlapping ranges. If overlapping overwriting
    // mapping operations happen concurrently, they are blocked by our cursor.
...
fn map_populate(cursor: &mut VmarCursorMut<'_>, vm_mapping: VmMapping) -> Result<()> {
