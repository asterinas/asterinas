...
use core::{array, cmp::min, num::NonZeroUsize, ops::Range};
...
impl Vmar_ {
    ...
    fn remap(
        ...
        while current_offset < old_size {
            ...
            let (va, Some(VmQueriedItem::MappedRam { frame, prop })) = cursor.query().unwrap()
            else {
...
fn cow_copy_pt(src: &mut CursorMut<'_>, dst: &mut CursorMut<'_>, size: usize) -> usize {
    ...
    while let Some(mapped_va) = src.find_next(remain_size) {
        let (va, Some(VmQueriedItem::MappedRam { frame, mut prop })) = src.query().unwrap() else {
            panic!("Found mapped page but query failed");
        };
        debug_assert_eq!(mapped_va, va.start);
    ...
    Vmo<R2>: VmoRightsOp,
{
    ...
    pub fn build(self) -> Result<Vaddr> {
        ...
        let vmo = if let Some(vmo) = vmo {
            MappedVmObj::Vmo(MappedVmo::new(vmo.to_dyn(), vmo_offset))
        } else if let Some(iomem) = iomem {
            let mapped_size = min(iomem.length(), vmo_offset + map_size);
            MappedVmObj::IoMem(MappedIoMem::new(iomem, vmo_offset..mapped_size))
        } else {
            MappedVmObj::None
        };
        ...
        //
        // It might look weird that we populate the mapping before adding it to
        // the VMAR. But it's ok since we hold the write lock on the VMAR, so
        // the operation is atomic and extrange the sequence permits more
        ...
        if map_populate {
            ...
            let _ = vm_mapping.map_populate(parent.vm_space(), &mut rss_delta);
        }
