...
impl<R> Vmar<R> {
    ...
    /// Reads memory from the VMAR.
    ///
    /// The `VmSpace` corresponding to the VMAR is not required be activated on the current CPU.
    pub fn read_maybe_remote(&self, vaddr: Vaddr, writer: &mut VmWriter) -> Result<usize> {
        self.0.read_maybe_remote(vaddr, writer)
    }

    /// Writes memory to the VMAR.
    ///
    /// The `VmSpace` corresponding to the VMAR is not required be activated on the current CPU.
...
impl Vmar_ {
    ...
    /// Access memory at `vaddr..vaddr+len` within the VMAR using `op`.
    ///
    /// The `VmSpace` corresponding to the VMAR is not required be activated on
    /// the current CPU. If any page in the range is not mapped or does not have
    /// the required page flags, a page fault will be handled to try to make the
    /// page accessible.
        ...
        F: FnMut(UFrame, usize) -> Result<usize>,
    {
        ...
        while current_va < range.end {
                ...
                .is_none_or(|item| !item.prop().flags.contains(required_page_flags))
            {
                ...
                if self.handle_page_fault(&page_fault_info).is_err() {
                    if bytes == 0 {
