...
fn cow_copy_pt(src: &mut CursorMut<'_, '_>, dst: &mut CursorMut<'_, '_>, size: usize) {
...
mod test {
    ...
    fn test_cow_copy_pt() {
        ...
        {
            let mut child_cursor = child_space.cursor_mut(&cow_range).unwrap();
            let mut parent_cursor = vm_space.cursor_mut(&cow_range).unwrap();
        ...
        // Confirms that parent and child VAs map to the same physical address.
        {
            let child_map_frame_addr = {
                let VmItem::Mapped { frame, .. } =
                    child_space.cursor(&map_range).unwrap().query().unwrap()
                else {
        ...
        {
            let mut sibling_cursor = sibling_space.cursor_mut(&cow_range).unwrap();
            let mut parent_cursor = vm_space.cursor_mut(&cow_range).unwrap();
        ...
        assert!(matches!(
            sibling_space.cursor(&map_range).unwrap().query().unwrap(),
            VmItem::NotMapped { .. }
        ));
