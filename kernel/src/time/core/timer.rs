...
impl TimerCallback {
    ...
    fn call(&self) {
        ...
        let interval = timer_locked.interval();
        if interval != Duration::ZERO {
            timer_locked.set_timeout(Timeout::After(interval));
        }

        // Pass the `timer_locked` guard to the callback, allowing it to prevent race conditions.
        // The callback may choose to use the guard or drop it if not needed.
        (timer.registered_callback)(timer_locked);
    }
}
