...
impl BlockDevice for PartitionNode {
    fn enqueue(&self, bio: SubmittedBio) -> Result<(), BioEnqueueError> {
        // FIXME: This function should not be blocked by the new bio,
        // and cloning the entire block I/O request is too inefficient.
        ...
        let Ok(status) = new_bio.submit_and_wait(self.device.as_ref()) else {
    ...
    fn metadata(&self) -> BlockDeviceMeta {
        ...
        metadata.max_nr_segments_per_bio = self.device.metadata().max_nr_segments_per_bio;
