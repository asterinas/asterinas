...
use ostd::{
    mm::{
        DmaDirection, DmaStream, DmaStreamSlice, FrameAllocOptions, Infallible, Segment,
        UntypedMeta, VmIo, VmReader, VmWriter,
    },
...
impl<'a> BioSegment {
    ...
    /// Constructs a new `BioSegment` with a given `Segment<dyn UntypedMeta>` and the bio direction.
    pub fn new_from_segment(segment: Segment<dyn UntypedMeta>, direction: BioDirection) -> Self {
    ...
    #[cfg(ktest)]
    pub fn inner_segment(&self) -> &Segment<dyn UntypedMeta> {
