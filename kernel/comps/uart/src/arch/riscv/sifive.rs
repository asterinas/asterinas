...
//!
//! This module registers SiFive FU540-compatible UART devices with the
//! architecture-independent [`UartConsole`] layer. It discovers the UART from
//! the RISC-V device tree and maps the device interrupt through the platform
//! IRQ chip.
...
pub(super) fn is_compatible(fdt_node: &FdtNode) -> bool {
    fdt_node
        .compatible()
        .is_some_and(|compatibles| compatibles.all().any(|c| FDT_COMPATIBLES.contains(&c)))
}
...
impl Uart for SpinLock<SifiveUart, LocalIrqDisabled> {
    fn send(&self, buf: &[u8]) {
