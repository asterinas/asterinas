...
use aster_frame::arch::{read_tsc, register_interrupt_callback, x86::tsc_freq, TIMER_FREQ};
use spin::Once;
...
fn init_timer() {
    ...
    let update = move || {
        let counter = TSC_UPDATE_COUNTER.fetch_add(1, Ordering::SeqCst);
    ...
    // TODO: re-organize the code structure and use the `IntervalTimer` to achieve the updating.
    register_interrupt_callback(Box::new(update));
}
