...
use crate::dma_pool::{DmaBlock, DmaPool};
    ...
    pub fn new<H: Pod>(header: &H, packet: &[u8]) -> Self {
        ...
        let dma_stream = if let Some(stream) = get_tx_stream_from_pool(nbytes) {
            ...
            let segment = {
                ...
                let mut options = VmAllocOptions::new(nframes);
...
impl RxBuffer {
    pub fn new(header_len: usize) -> Self {
        let block = RX_BUFFER_POOL.get().unwrap().alloc_block().unwrap();
        Self {
            block,
            header_len,
            packet_len: 0,
...
static RX_BUFFER_POOL: Once<DmaPool> = Once::new();
...
pub fn init() {
    ...
    RX_BUFFER_POOL.call_once(|| DmaPool::new(RX_BUFFER_LEN, DmaDirection::FromDevice, false));
