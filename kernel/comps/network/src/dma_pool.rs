...
use ostd::{
    mm::{
        ...
        Daddr, FrameAllocOptions, HasDaddr, Infallible, VmReader, VmWriter, PAGE_SIZE,
    },
...
mod test {
    ...
    fn alloc_small_size_segment() {
        ...
        let pool: Arc<DmaPool<Bidirectional>> = DmaPool::new(SEGMENT_SIZE, 0, 10, false);
        let segments1: Vec<_> = (0..100)
            .map(|_| {
                let segment = pool.alloc_segment().unwrap();
    ...
    fn read_dma_segments() {
        const SEGMENT_SIZE: usize = PAGE_SIZE / 4;
        let pool: Arc<DmaPool<Bidirectional>> = DmaPool::new(SEGMENT_SIZE, 1, 2, false);
        let segment = pool.alloc_segment().unwrap();
