...
bitflags::bitflags! {
    /// KMS capabilities enabled by a userspace client for this open DRM file.
    ///
    /// These flags correspond to selected boolean fields in Linux
    /// `struct drm_file` and are configured through
    /// `DRM_IOCTL_SET_CLIENT_CAP`. Their bit positions are internal to
    /// Asterinas and do not correspond to DRM UAPI capability values.
    pub(super) struct DrmClientCaps: u32 {
...
impl From<u32> for DrmClientCaps {
    fn from(value: u32) -> Self {
        Self::from_bits_truncate(value)
    }
}

impl From<DrmClientCaps> for u32 {
    fn from(value: DrmClientCaps) -> Self {
        value.bits()
    }
}

define_atomic_version_of_integer_like_type!(DrmClientCaps, {
    /// An atomic version of [`DrmClientCaps`].
    #[derive(Debug, Default)]
    struct AtomicDrmClientCaps(AtomicU32);
});

impl AtomicDrmClientCaps {
    fn set(&self, flags: DrmClientCaps, enabled: bool) {
...
impl DrmFile {
    ...
    pub(super) fn set_client_caps(&self, caps: DrmClientCaps, enabled: bool) {
        self.client_caps.set(caps, enabled);
    }
    ...
    pub(super) fn set_master(&self) -> Result<()> {
        ...
        // Bind ownership to the process that completes the first successful
        // master transition while preventing concurrent ioctls from replacing it.
        if !auth_state.was_master {
            auth_state.owner_process = Arc::downgrade(&owner_process);
        }
