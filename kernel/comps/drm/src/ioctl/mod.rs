...
impl DrmFile {
    fn check_required_driver_features(&self, required_flags: DrmIoctlFlags) -> Result<()> {
        ...
        for (required_flag, driver_feature) in required_features {
            if required_flags.contains(required_flag) && !self.has_features(driver_feature) {
    ...
    fn check_ioctl_flags(&self, required_flags: DrmIoctlFlags) -> Result<()> {
        ...
        if required_flags.contains(DrmIoctlFlags::MASTER) && !self.is_master() {
    ...
    pub(super) fn dispatch_ioctl(&self, raw_ioctl: RawIoctl) -> Result<i32> {
        dispatch_ioctl!(match raw_ioctl {
            ...
            cmd @ DrmIoctlGetUnique => {
                self.check_ioctl_flags(DrmIoctlFlags::empty())?;
