...
use super::utils::PAGE_SIZE; // Use utils for Vm* types
use super::{Error, Result, SysAttrSet, SysStr}; // Import common types from parent
...
pub trait SysBranchNode: SysNode {
    ...
    fn visit_child_with(&self, name: &str, f: &mut dyn FnMut(Option<&dyn SysNode>)); // Changed signature
    ...
    fn visit_children_with(&self, min_id: u64, f: &mut dyn FnMut(&dyn SysObj) -> Option<()>);
...
pub trait SysNode: SysObj {
    ...
    fn show_attr(&self, name: &str) -> Result<String> {
        ...
        let attr_val = String::from_utf8(buf[..read_len].to_vec()).map_err(|_| Error)?;
        Ok(attr_val)
    }
    ...
    fn store_attr(&self, name: &str, new_val: &str) -> Result<()> {
        // Use as_bytes() which is equivalent to as_slice() for &str
...
pub trait SysObj: Any + Send + Sync + Debug + 'static {
    ...
    /// Attempts to view this object as a `SysNode`.
    fn as_node(&self) -> Option<&dyn SysNode> {
        None
    }

    /// Attempts to view this object as a `SysBranchNode`.
    ...
    ///
    /// Returns `None` if this node is the root or detached.
    fn parent(&self) -> Option<Weak<dyn SysBranchNode>>;

    /// Returns whether a node is the root of a `SysTree`.
    fn is_root(&self) -> bool {
        self.parent().as_ref().and_then(|w| w.upgrade()).is_none()
    }
