...
impl InputCore {
    ...
    pub(crate) fn unregister_handler_class(
        ...
        let removed = self.handler_classes.swap_remove(pos);
    ...
    pub(crate) fn unregister_device(
        ...
        let device_registry = self.devices.remove(pos);

        // Collect handler classes actually connected to this device, then clear handlers.
        let classes: Vec<Arc<dyn InputHandlerClass>> = {
            let handlers = device_registry.handlers.write();
            let classes = handlers.iter().map(|h| h.handler_class.clone()).collect();
            drop(handlers);
        ...
        for handler_class in classes.into_iter() {
            handler_class.disconnect(&device_registry.device);
        }
