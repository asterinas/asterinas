...
impl InputCapability {
    ...
    /// Linux-like keyboard detection: device is a keyboard if
    ...
    pub fn look_like_keyboard(&self) -> bool {
        ...
        self.supported_keys
            .contain_any_in_range(0, KeyCode::KeyBtnMisc as usize)
    }
...
impl Drop for RegisteredInputDevice {
    fn drop(&mut self) {
        ...
        let removed = unregister_device(&self.device);
        removed.expect("RegisteredInputDevice drop: device must be present in registry");
    }
}
