...
impl EventTypes {
    ...
    /// # Panics
    ///
    /// Panics if more than one event type flag is set, as evdev events
    /// should only have one type at a time.
    pub const fn as_u16(&self) -> u16 {
        let bits = self.bits();

        // Checks if exactly one bit is set
        if bits == 0 || (bits & (bits - 1)) != 0 {
            panic!("EventTypes::as_u16() expects exactly one flag to be set");
        }
...
impl RelCodeMap {
    pub fn new() -> Self {
        // Initializes with all zeros, sized to hold all possible relative codes.
    ...
    pub fn contains(&self, rel_code: RelCode) -> bool {
        let index = rel_code as usize;
        if index < REL_COUNT {
