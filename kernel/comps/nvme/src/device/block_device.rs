    ...
    pub(crate) fn init(transport: NvmePciTransport) -> Result<(), NvmeDeviceError> {
        ...
        let block_device = Arc::new(Self {
            ...
            queue: BioRequestSingleQueue::with_max_nr_segments_per_bio(
                (NvmeDeviceInner::QUEUE_SIZE - 2) as usize,
            ),
        });
...
pub struct NvmeDeviceInner {
    ...
    transport: SpinLock<Box<NvmePciTransport>>,
...
impl NvmeDeviceInner {
    ...
    pub fn init(transport: NvmePciTransport) -> Result<Arc<Self>, NvmeDeviceError> {
        let device = Arc::new(NvmeDeviceInner {
            ...
            transport: SpinLock::new(Box::new(transport)),
    ...
    pub fn read(&self, request: BioRequest) {
        ...
        while blocks_num > 0 {
            let once_blocks_num = if blocks_num < 8 { blocks_num } else { 8 };
            let ptr1 = 0;
...
mod test {
    ...
    fn write_then_read() {
        ...
        let val = b'A';
        let _ = create_and_submit_bio_request(nvme_block_device, BioType::Write, buf_len, val);
        nvme_block_device.handle_requests();
        let read_bio_segment =
            create_and_submit_bio_request(nvme_block_device, BioType::Read, buf_len, val);
        nvme_block_device.handle_requests();
        ...
        assert!(read_buf.iter().all(|&x| x == b'A'));
    }
}
