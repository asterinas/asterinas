...
use crate::transport::{TransportGuard, VirtioTransport};
...
fn virtio_component_init() -> Result<(), ComponentInitError> {
    ...
    while let Some(mut transport) = pop_device_transport() {
        ...
        let transport_guard = TransportGuard::new(transport);

        let res = match device_type {
            VirtioDeviceType::Block => BlockDevice::init(transport_guard),
            VirtioDeviceType::Console => ConsoleDevice::init(transport_guard),
            VirtioDeviceType::Entropy => EntropyDevice::init(transport_guard),
            VirtioDeviceType::Input => InputDevice::init(transport_guard),
            VirtioDeviceType::Network => NetworkDevice::init(transport_guard),
            VirtioDeviceType::Socket => SocketDevice::init(transport_guard),
            VirtioDeviceType::FileSystem => FileSystemDevice::init(transport_guard),
            _ => {
