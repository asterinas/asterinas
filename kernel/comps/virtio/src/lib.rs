...
use device::{
    block::device::BlockDevice, console::device::ConsoleDevice, crypto::device::CryptoDevice, input::device::InputDevice, network::device::NetworkDevice, socket::{self, device::SocketDevice}, VirtioDeviceType
};
use log::{error, warn};
...
fn virtio_component_init() -> Result<(), ComponentInitError> {
    ...
    while let Some(mut transport) = pop_device_transport() {
        ...
        let res = match transport.device_type() {
            ...
            VirtioDeviceType::Crypto => VirtioDeviceType::init(transport),
            _ => {
