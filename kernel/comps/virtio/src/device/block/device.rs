...
fn sync_dma(slice: &Slice<&Arc<DmaStream>>) {
    slice.mem_obj().sync(slice.offset().clone()).unwrap();
}

fn sync_dma_owned(slice: &Slice<Arc<DmaStream>>) {
    slice.mem_obj().sync(slice.offset().clone()).unwrap();
}
