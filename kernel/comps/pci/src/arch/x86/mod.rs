...
pub(crate) fn write32(
    ...
    PCI_DATA_PORT.get().unwrap().write(value.to_le());
    Ok(())
}

pub(crate) fn read32(location: &PciDeviceLocation, offset: u32) -> Result<u32, ostd::Error> {
    PCI_ADDRESS_PORT
        .get()
        .unwrap()
        .write(encode_as_port(location) | (offset & BIT32_ALIGN_MASK));
    Ok(PCI_DATA_PORT.get().unwrap().read().to_le())
}
