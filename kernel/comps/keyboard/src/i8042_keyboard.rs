...
use ostd::{
    arch::device::i8042_controller::{
        DATA_PORT, KEYBOARD_IRQ_LINE, STATUS_OR_COMMAND_PORT as STATUS_PORT,
    },
    trap::TrapFrame,
};
...
pub fn init() {
    if let Some(irq) = KEYBOARD_IRQ_LINE.get() {
        irq.lock().on_active(handle_keyboard_input);
    }

    // TODO: maybe should we reset the keyboard device here?
    //
...
fn handle_keyboard_input(_trap_frame: &TrapFrame) {
    if !DATA_PORT.is_completed() || !STATUS_PORT.is_completed() {
        return;
    }
...
impl Status {
    fn read() -> Self {
        Self::from_bits_truncate(STATUS_PORT.get().unwrap().read())
    }
