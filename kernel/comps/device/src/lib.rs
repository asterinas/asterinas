...
pub use id::{register_device_ids, DeviceId, DeviceIdAllocator, DeviceType};
use spin::Once;
...
pub trait Device: Any + Debug + Send + Sync {
    ...
    /// Returns the sysfs node representation of this device.
    ///
    /// This method returns an `Arc<dyn SysBranchNode>` that represents
    /// the device in the sysfs hierarchy.
    fn sysnode(&self) -> Arc<dyn SysBranchNode>;
}
...
///
/// 1. If the device's sysnode doesn't have a parent, it adds the device to the
///    global devices node.
