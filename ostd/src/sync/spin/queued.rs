...
//! <https://elixir.bootlin.com/linux/v7.1.2/source/kernel/locking/qspinlock.c>.
...
use crate::{
    cpu::CpuId, cpu_local, cpu_local_cell, task::atomic_mode::InAtomicMode, util::id_set::Id,
};
...
impl LockBody {
    ...
    fn lock_enqueue(&self, guard: &dyn InAtomicMode) {
        ...
        // Publish the updated tail while preserving the locked bit.
        let this_node_encoded = Self::encode_tail(cur_cpu, top);
        debug_assert_ne!(this_node_encoded, Self::UNLOCKED_VAL);
        // The release ordering on the successful CAS ensures that others read
        // our initialized node.
        let old_val = loop {
            let current = self.val.load(Ordering::Acquire);
            let new_val = this_node_encoded | (current & Self::LOCKED_VAL);
            match self
                .val
        ...
        // The locked bit must be clear here.
        debug_assert_eq!(self.val.load(Ordering::Relaxed) & Self::LOCKED_VAL, 0);
