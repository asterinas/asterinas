...
unsafe impl<L: NonNullPtr, R: NonNullPtr> NonNullPtr for Either<L, R> {
    ...
    unsafe fn from_raw(ptr: NonNull<Self::Target>) -> Self {
        ...
        if is_right == 0 {
            // SAFETY: `from_raw` guarantees that `real_ptr` comes from `L::into_raw`. Other safety
            // requirements are upheld by the caller.
            Either::Left(unsafe { L::from_raw(real_ptr.cast()) })
        } else {
            // SAFETY: `from_raw` guarantees that `real_ptr` comes from `R::into_raw`. Other safety
            // requirements are upheld by the caller.
