...
unsafe impl<P: NonNullPtr> Sync for RcuInner<P> where P: Send {}
...
impl<P: NonNullPtr> RcuInner<P> {
    ...
    fn read_with<'a>(&'a self, _guard: &'a DisabledPreemptGuard) -> Option<P::Ref<'a>> {
...
impl<P: NonNullPtr> Rcu<P> {
    ...
    pub fn read_with<'a>(&'a self, guard: &'a DisabledPreemptGuard) -> P::Ref<'a> {
...
impl<P: NonNullPtr> RcuOption<P> {
    ...
    /// writers. You may do it via a [`super::SpinLock`].
    pub fn read_with<'a>(&'a self, guard: &'a DisabledPreemptGuard) -> Option<P::Ref<'a>> {
        self.0.read_with(guard)
    }
}

// RCU guards that have a non-null pointer can be directly dereferenced.
impl<P: NonNullPtr> Deref for RcuReadGuard<'_, P> {
    type Target = P;
