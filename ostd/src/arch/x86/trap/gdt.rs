...
pub(super) unsafe fn init() {
    ...
    // These are global invariants set by the boot GDT. This method is not intended for switching
    // to a new kernel CS or a new kernel SS.
    assert_eq!(CS::get_reg(), KERNEL_CS);
    assert_eq!(SS::get_reg(), KERNEL_SS);
