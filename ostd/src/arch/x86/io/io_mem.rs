...
macro_rules! do_copy_mmio_impl {
    (align: $io_ptr:ident, dst: $dst:ident, src: $src:ident, count: $count:ident) => {{
        if $count == 0 { return; }

        // String instructions depend on DF. Keep forward copy explicit here
        // rather than relying on a global calling-convention invariant.
        unsafe {
        ...
        if $io_ptr.addr() & 1 != 0 {
            ...
            unsafe {
                asm!("movsb", inout("rdi") $dst, inout("rsi") $src,
                     options(nostack, preserves_flags));
            }
            $count -= 1;
        }
        if $count >= 2 && $io_ptr.addr() & 2 != 0 {
            // SAFETY: The caller guarantees both pointers are valid for 2 bytes.
            unsafe {
                asm!("movsw", inout("rdi") $dst, inout("rsi") $src,
                     options(nostack, preserves_flags));
            }
            $count -= 2;
        }
        if $count >= 4 && $io_ptr.addr() & 4 != 0 {
            // SAFETY: The caller guarantees both pointers are valid for 4 bytes.
            unsafe {
                asm!("movsd", inout("rdi") $dst, inout("rsi") $src,
                     options(nostack, preserves_flags));
            }
            $count -= 4;
        }

        // Now the address is 8-byte aligned.
        if $count > 0 {
            // SAFETY: The caller guarantees both pointers are valid for `count` bytes.
            unsafe {
                asm!("rep movsb",
                     inout("rdi") $dst, inout("rsi") $src, inout("rcx") $count,
                     options(nostack, preserves_flags));
            }
            let _ = ($dst, $src, $count);
        }
...
macro_rules! do_memset_mmio_impl {
    (dst: $dst:ident, value: $value:ident, count: $count:ident) => {{
        ...
        // String instructions depend on DF. Keep forward store explicit here
        // rather than relying on a global calling-convention invariant.
        unsafe {
            asm!("cld", options(nostack, nomem));
        }
...
pub(crate) unsafe fn copy_from_mmio(
    ...
    if_tdx_enabled!({
        // SAFETY: The caller guarantees both pointers are valid for `count` bytes.
        unsafe { cvm::copy_from_mmio(dst_ptr, src_io_ptr, count) };
    });

    do_copy_mmio_impl!(align: src_io_ptr, dst: dst_ptr, src: src_io_ptr, count: count);
}
...
pub(crate) unsafe fn copy_to_mmio(
    ...
    if_tdx_enabled!({
        // SAFETY: The caller guarantees both pointers are valid for `count` bytes.
        unsafe { cvm::copy_to_mmio(src_ptr, dst_io_ptr, count) };
    });

    do_copy_mmio_impl!(align: dst_io_ptr, dst: dst_io_ptr, src: src_ptr, count: count);
}
...
pub(crate) unsafe fn copy_mmio_to_mmio(
    ...
    if_tdx_enabled!({
        // SAFETY: The caller guarantees both pointers are valid for `count` bytes.
        unsafe { cvm::copy_mmio_to_mmio(dst_io_ptr, src_io_ptr, count) };
    });

    do_copy_mmio_impl!(align: dst_io_ptr, dst: dst_io_ptr, src: src_io_ptr, count: count);
}
...
///
/// - `dst_io_ptr` must be valid for MMIO writes of `count` bytes.
pub(crate) unsafe fn memset_mmio(mut dst_io_ptr: *mut u8, value: u8, mut count: usize) {
    if_tdx_enabled!({
