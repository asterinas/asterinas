...
// SAFETY: `AcpiTables` is effectively read-only after initialization, and the
// memory mappings for the ACPI table regions remain unchanged after initialization.
// Therefore, it is safe to share references to `SyncAcpiTables` across threads.
