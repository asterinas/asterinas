...
use crate::{boot::memory_region::MemoryRegionType, io::io_mem::allocator::IoMemAllocatorBuilder};
...
pub(super) fn construct_io_mem_allocator_builder() -> IoMemAllocatorBuilder {
    ...
    let regions = crate::boot::memory_regions().to_owned();
    let mut ranges = Vec::with_capacity(2);

    let reserved_filter = regions
        .iter()
        .filter(|r| r.typ() != MemoryRegionType::Reserved);
