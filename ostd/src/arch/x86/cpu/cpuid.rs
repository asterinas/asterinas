...
/// Calculates the frequency in Hz of the Time Stamp Counter (TSC).
///
...
pub(in crate::arch) fn calc_tsc_freq() -> Option<u64> {
    let CpuidResult {
        eax: denominator,
        ebx: numerator,
        ecx: crystal_freq,
...
/// Gets the supported XSTATE features, i.e., the supported bits of `XCR0` and `IA32_XSS`.
pub(in crate::arch) fn get_xstate_max_features() -> Option<u64> {
...
/// Gets the size in bytes of the XSAVE area containing states enabled by `XCRO` and `IA32_XSS`.
pub(in crate::arch) fn get_xsave_area_size() -> Option<u32> {
