...
impl PageTableEntry {
    ...
    fn prop(&self) -> PageProperty {
        ...
        let cache = if self.0 & PteFlags::NO_CACHE.bits() != 0 {
            CachePolicy::Uncacheable
        } else if self.0 & PteFlags::WRITE_THROUGH.bits() != 0 {
    ...
    fn new_page(paddr: Paddr, level: PagingLevel, prop: PageProperty) -> Self {
        ...
        match prop.cache {
            CachePolicy::Writeback => {}
            CachePolicy::Writethrough => {
                flags |= PteFlags::WRITE_THROUGH.bits();
            }
            CachePolicy::Uncacheable => {
                flags |= PteFlags::NO_CACHE.bits();
            }
        ...
        assert_eq!(
            paddr & !pa_mask,
            0,
            "physical address contains invalid bits"
        );

        Self(paddr & pa_mask | flags)
    }
    ...
    fn new_pt(paddr: Paddr, flags: PageTableFlags) -> Self {
        ...
        Self(paddr & Self::CHILD_PT_ADDR_MASK | flags)
    }
}
...
unsafe impl PteTrait for PageTableEntry {
    fn from_repr(repr: &PteScalar) -> Self {
        match repr {
            ...
            PteScalar::PageTable(paddr, _level, flags) => Self::new_pt(*paddr, *flags),
            PteScalar::Mapped(paddr, level, prop) => Self::new_page(*paddr, *level, *prop),
        }
    }
    ...
    fn to_repr(&self, level: PagingLevel) -> PteScalar {
        ...
        if level == 1 || (self.0 & PteFlags::HUGE.bits() != 0) {
            let pa_mask = Self::pa_mask_at_level(level);
            let paddr = self.0 & pa_mask;
            let prop = self.prop();
            PteScalar::Mapped(paddr, level, prop)
        } else {
            let paddr = self.0 & Self::CHILD_PT_ADDR_MASK;
            PteScalar::PageTable(paddr, level, self.pt_flags())
        }
    }
}
