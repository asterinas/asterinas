...
impl LocalVmcsState {
    ...
    unsafe fn do_activate(
            ...
            && active_cpu != preempt_guard.current_cpu()
        {
            // SAFETY: `vmcs_state` is the locked state of `vmcs`.
            unsafe {
                Self::do_deactivate(vmcs, vmcs_state, _vmx_guard)?;
            }
        }
    ...
    pub(super) unsafe fn deactivate_all(irq_guard: &DisabledLocalIrqGuard) -> Result<()> {
        let local = LOCAL_VMCS_STATE.get_with(irq_guard);
        // Although there is a window of time between releasing the `local` lock and
        // clearing each VMCS's `active_cpu`, we're dropping the last VMX guard,
        // so concurrent activation and deactivation are not possible.
        let active = {
            let mut local = local.lock();
            local.current = None;
