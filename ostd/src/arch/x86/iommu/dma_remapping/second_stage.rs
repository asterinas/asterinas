...
unsafe impl PageTableConfig for IommuPtConfig {
    ...
    unsafe fn item_ref_from_raw<'a>(
        ...
        PtItemRef((paddr, level, prop), PhantomData)
    }
}

pub(crate) type PtItem = (Paddr, PagingLevel, PageProperty);
pub(crate) struct PtItemRef<'a>(#[expect(dead_code)] pub PtItem, PhantomData<&'a ()>);
...
impl PageTableEntry {
    ...
    fn is_present(&self) -> bool {
        self.0 & (PteFlags::READABLE | PteFlags::WRITABLE).bits() != 0
    }
    ...
    fn prop(&self) -> PageProperty {
        ...
        let priv_flags = parse_flags!(self.0, PteFlags::IGN2, PageFlags::AVAIL2);
...
unsafe impl PteTrait for PageTableEntry {
    ...
    fn to_repr(&self, level: PagingLevel) -> PteScalar {
        ...
        if self.is_last(level) {
            PteScalar::Mapped(paddr, level, self.prop())
        } else {
