...
pub(super) fn init_bsp() -> IrqLine {
    ...
    if is_tsc_deadline_mode_supported() {
        ...
        info!(
            "[Timer]: Enable APIC periodic mode with frequency {} Hz.",
            TIMER_FREQ
        );
        calc_apic_freq_from_pit();
...
fn calc_apic_freq_from_pit() {
    ...
    fn pit_callback(_trap_frame: &TrapFrame) {
        ...
        if IN_TIME.load(Ordering::Relaxed) < CALLBACK_TIMES || IS_FINISHED.load(Ordering::Acquire) {
