...
use self::apic::{apic_tsc_deadline_mode_callback, IN_APIC_TSC_DEADLINE_MODE};
use crate::{
    arch::x86::kernel,
...
/// Initialize global timer resources and enable timer interrupt on BSP.
pub(super) fn init_global_and_bsp() {
    let mut timer_irq = if kernel::apic::exists() {
        apic::init_global_and_bsp()
    } else {
...
/// Enable timer interrupt on this AP.
...
fn timer_callback(_: &TrapFrame) {
    ...
    if IN_APIC_TSC_DEADLINE_MODE.load(Ordering::Relaxed) {
