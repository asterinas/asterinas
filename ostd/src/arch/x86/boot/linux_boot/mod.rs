...
fn parse_unaccepted_memory_table(
    ...
    if systab.configuration_table.is_null() || systab.number_of_configuration_table_entries == 0 {
        return None;
    }

    // SAFETY: `configuration_table` points to a firmware-provided array in the EFI system table,
    // and `number_of_configuration_table_entries` is its element count validated above as non-zero.
    let entries = unsafe {
    ...
    if version != 1 {
        log::warn!("Unknown unaccepted memory table version: {}", version);
        return None;
    }
