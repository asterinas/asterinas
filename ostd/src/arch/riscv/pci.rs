...
pub(crate) fn init(io_mem_builder: &IoMemAllocatorBuilder) {
    ...
    let addr_range = addr_start..addr_end;

    io_mem_builder.remove(addr_range.clone());
    PCI_BASE_ADDR
        .call_once(|| unsafe { IoMem::new(addr_range, PageFlags::RW, CachePolicy::Uncacheable) });
}
