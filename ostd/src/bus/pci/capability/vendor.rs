// SPDX-License-Identifier: MPL-2.0

//! Vendor-specific capability support.

use crate::{
    bus::pci::{common_device::PciCommonDevice, PciDeviceLocation},
    Error, Result,
};

/// Vendor specific capability. Users can access this capability area at will,
/// except for the PCI configuration space which cannot be accessed at will through this structure.
#[derive(Debug, Clone)]
pub struct CapabilityVndrData {
    location: PciDeviceLocation,
    cap_ptr: usize,
    length: usize,
}

impl CapabilityVndrData {
    pub(super) fn new(dev: &PciCommonDevice, cap_ptr: usize, length: usize) -> Self {
        Self {
            location: dev.location().clone(),
            cap_ptr,
            length,
        }
    }

    /// The length of this capability
    #[expect(clippy::len_without_is_empty)]
    pub fn len(&self) -> usize {
        self.length
    }

    /// Reads u8 from the capability.
    pub fn read8(&self, offset: usize) -> Result<u8> {
        self.check_range(offset)?;
        self.location.read8(self.cap_ptr + offset)
    }

    /// Writes u8 to the capability.
    pub fn write8(&self, offset: usize, value: u8) -> Result<()> {
        self.check_range(offset)?;
        self.location.write8(self.cap_ptr + offset, value)?;
        Ok(())
    }

    /// Reads usize from the capability.
    pub fn read16(&self, offset: usize) -> Result<u16> {
        self.check_range(offset)?;
        self.location.read16(self.cap_ptr + offset)
    }

    /// Writes usize to the capability.
    pub fn write16(&self, offset: usize, value: u16) -> Result<()> {
        self.check_range(offset)?;
        self.location.write16(self.cap_ptr + offset, value)?;
        Ok(())
    }

    /// Reads u32 from the capability.
    pub fn read32(&self, offset: usize) -> Result<u32> {
        self.check_range(offset)?;
        self.location.read32(self.cap_ptr + offset)
    }

    /// Writes u32 to the capability.
    pub fn write32(&self, offset: usize, value: u32) -> Result<()> {
        self.check_range(offset)?;
        self.location.write32(self.cap_ptr + offset, value)?;
        Ok(())
    }

    #[inline]
    fn check_range(&self, offset: usize) -> Result<()> {
        if self.length < offset {
            return Err(Error::InvalidArgs);
        }
        Ok(())
    }
}
