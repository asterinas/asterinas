...
impl IoPortAllocator {
    ...
    pub fn acquire<A>(&self, mut range: Range<u16>) -> Option<IoPort<A>> {
        let mut allocator = self.allocator.disable_irq().lock();
        ...
        unsafe { Some(IoPort::new_with_size(range.start, range.end - range.start)) }
    }
...
pub(super) static IO_PORT_DISPATCHER: Once<IoPortAllocator> = Once::new();

/// Initializes the static `IO_PORT_DISPATCHER` based on builder.
///
/// # Safety
///
/// User must ensure all the port I/O regions that belong to the system device have been removed by calling the
/// `remove` function.
pub(crate) unsafe fn init(io_mem_builder: IoPortAllocatorBuilder) {
    IO_PORT_DISPATCHER.call_once(|| IoPortAllocator {
        allocator: SpinLock::new(io_mem_builder.allocator),
    });
}
