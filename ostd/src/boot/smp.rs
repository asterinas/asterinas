...
pub(crate) unsafe extern "C" fn ap_early_entry(cpu_id: u32) -> ! {
    ...
    crate::if_tdx_enabled!({
        // Acceptance must finish before the BSP publishes these frames. It is safe without TLB
        // coherence because it neither updates page tables nor allocates memory.
