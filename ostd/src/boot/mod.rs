...
pub(crate) fn call_ostd_main() -> ! {
    ...
    // Initialize coverage collector if coverage parameters are provided
    #[cfg(feature = "coverage")]
    {
        let cmdline = EARLY_INFO.get().unwrap().kernel_cmdline;
        if let Some((paddr, size)) = crate::coverage::parse_coverage_cmdline(cmdline) {
            crate::coverage::init_coverage_collector(paddr, size);
        }
    }
