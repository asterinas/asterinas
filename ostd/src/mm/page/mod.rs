...
use meta::{mapping, MetaSlot, PageMeta, PAGE_METADATA_MAX_SIZE};
...
impl<M: PageMeta> Page<M> {
    ...
    ///  - the metadata's size exceeds [`PAGE_METADATA_MAX_SIZE`];
    ///  - the alignment of the metadata is more than 8 bytes.
    pub fn from_unused(paddr: Paddr, metadata: M) -> Self {
        assert!(paddr % PAGE_SIZE == 0);
        assert!(paddr < MAX_PADDR.load(Ordering::Relaxed) as Paddr);

        // They can indeed be checked at compile time, but the compiler
        // currently struggles with the constant generics.
        assert!(size_of::<M>() <= PAGE_METADATA_MAX_SIZE);
        assert!(align_of::<M>() <= 8);
        ...
        unsafe {
            vtable_ptr.write(core::ptr::metadata(
                &metadata as &dyn PageMeta as *const dyn PageMeta,
            ))
        };
