...
use crate::{
    ...
    mm::{
        ...
        FallibleVmRead, FallibleVmWrite, Infallible, Paddr, PodOnce, Vaddr, VmIo, VmIoOnce,
...
pub struct VirtAddrAllocator {
    ...
    freelist: SpinLock<Option<BTreeMap<Vaddr, KvaFreeNode>>>,
}
...
impl VirtAddrAllocator {
    ...
    fn alloc(&self, size: usize) -> Result<Range<Vaddr>> {
        ...
        if lock_guard.is_none() {
            ...
            freelist.insert(
                ...
                KvaFreeNode::new(self.fullrange.clone()),
            );
    ...
    fn free(&self, range: Range<Vaddr>) {
        ...
        let freelist = lock_guard.as_mut().unwrap_or_else(|| {
            panic!("Free a 'Kva' when 'VirtAddrAllocator' has not been initialized.")
        });
        ...
        freelist.insert(free_range.start, KvaFreeNode::new(free_range.clone()));
...
static KVA_TRACKED_ALLOCATOR: VirtAddrAllocator =
    VirtAddrAllocator::new(TRACKED_MAPPED_PAGES_RANGE);
static KVA_UNTRACKED_ALLOCATOR: VirtAddrAllocator = VirtAddrAllocator::new(VMALLOC_VADDR_RANGE);
...
impl AllocatorSelector for Tracked {
    fn select_allocator() -> &'static VirtAddrAllocator {
        &KVA_TRACKED_ALLOCATOR
    }
}

impl AllocatorSelector for Untracked {
    fn select_allocator() -> &'static VirtAddrAllocator {
        &KVA_UNTRACKED_ALLOCATOR
    }
}

/// Kernel Virtual Area.
///
/// A tracked kernel virtual area (`Kva<Tracked>`) manages a range of memory in
/// `TRACKED_MAPPED_PAGES_RANGE`. It can map a inner part or all of its virtual memory
/// to some physical tracked pages.
///
/// A untracked kernel virtual area (`Kva<Untracked>`) manages a range of memory in
/// `VMALLOC_VADDR_RANGE`. It can map a inner part or all of its virtual memory to
/// some physical untracked pages.
#[derive(Debug)]
pub struct Kva<M: AllocatorSelector + 'static> {
    range: Range<Vaddr>,
    phantom: PhantomData<M>,
}

impl<M: AllocatorSelector + 'static> Kva<M> {
...
impl Kva<Tracked> {
    ...
    pub fn map_pages<T: PageMeta>(
        ...
        for page in pages.into_iter() {
            // SAFETY: The constructor of the `Kva<Tracked>` structure has already ensured this
    ...
    pub fn get_page(&self, addr: Vaddr) -> Option<DynPage> {
        ...
        match query_result {
            ...
            _ => {
                panic!(
                    "Found '{:?}' mapped into tracked `Kva`, expected `Mapped`",
                    query_result
                );
            }
        }
...
impl Kva<Untracked> {
    ...
    pub fn get_untracked_page(&self, addr: Vaddr) -> Option<(Paddr, usize)> {
        ...
        match query_result {
            ...
            _ => {
                panic!(
                    "Found '{:?}' mapped into untracked `Kva`, expected `MappedUntracked`",
                    query_result
                );
            }
        }
...
impl<M: AllocatorSelector + 'static> Drop for Kva<M> {
    fn drop(&mut self) {
            ...
            match result {
                PageTableItem::Mapped { va, page, .. } => match TypeId::of::<M>() {
                    ...
                    id if id == TypeId::of::<Untracked>() => {
                        panic!("Found tracked memory mapped into untracked `Kva`");
                    }
                    _ => panic!("Unexpected `Kva` type"),
                },
                PageTableItem::MappedUntracked { va, .. } => match TypeId::of::<M>() {
                    ...
                    id if id == TypeId::of::<Tracked>() => {
                        panic!("Found untracked memory mapped into tracked `Kva`");
                    }
                    _ => panic!("Unexpected `Kva` type"),
                },
                ...
                PageTableItem::PageTableNode { .. } => {
                    panic!("Found page table node in `Kva`");
                }
            }
        }
...
impl Kva<Untracked> {
    fn reader(&self) -> VmReader<'_, Infallible> {
        // SAFETY: The constructor of the `Kva<Untracked>` structure has already ensured the
        // safety of reading from the mapped physical address, and the mapping is valid.
        unsafe { VmReader::from_kernel_space(self.range.start as *mut u8, self.range.len()) }
    }

    fn writer(&self) -> VmWriter<'_, Infallible> {
        // SAFETY: The constructor of the `Kva<Untracked>` structure has already ensured the
        // safety of writing to the mapped physical address, and the mapping is valid.
        unsafe { VmWriter::from_kernel_space(self.range.start as *mut u8, self.range.len()) }
    }
}

impl VmIo for Kva<Untracked> {
...
impl VmIoOnce for Kva<Untracked> {
