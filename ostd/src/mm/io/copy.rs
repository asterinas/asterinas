...
use crate::arch::{
    io::io_mem::{
        copy_from_mmio, copy_from_mmio_fallible, copy_mmio_to_mmio, copy_to_mmio,
        copy_to_mmio_fallible, memset_mmio,
    },
...
// SAFETY: Delegates to the architecture MMIO<->fallible copy entry,
...
// SAFETY: Delegates to `copy_to_mmio`, which correctly copies from kernel
// memory to MMIO using MMIO-safe load/store operations.
...
// SAFETY: Delegates to architecture MMIO copy entry,
// which uses MMIO-safe accesses on CVM guests and optimized string ops otherwise.
unsafe impl Memcpy<Io> for Io {
    type Result = ();

    unsafe fn memcpy(dst: *mut u8, src: *const u8, len: usize) {
        // SAFETY: The safety is upheld by the caller.
        unsafe { copy_mmio_to_mmio(dst, src, len) };
    }
}
