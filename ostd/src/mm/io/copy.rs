...
/// # Safety
///
/// The implementation must correctly copy `len` bytes from `src` to `dst`
/// when the caller upholds the preconditions of [`Memcpy::memcpy`].
///
/// An implementation must handle the case
/// where the source and destination virtual memory regions overlap.
/// When they do,
/// a naïve forward copy would clobber source bytes before they are read.
/// The implementation may need to copy in the backward direction
/// to preserve correctness.
/// For reference, compare [`core::ptr::copy_nonoverlapping`] (assumes no overlap)
/// with [`core::ptr::copy`] (handles overlap).
...
// which correctly copies bytes between valid kernel pointers.
