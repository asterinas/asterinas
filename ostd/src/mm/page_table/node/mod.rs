    ...
    [(); C::NR_LEVELS as usize]:,
{
    ...
    /// # Safety
    ///
    /// The caller must ensure that the index is within the bound, and the
    /// provided PTE is valid. A valid PTE, if points to a page, must point
    /// to a valid owned page and the child should be compatible with the
    /// node. The compatibility is specified by the [`Child::is_compatible`].
    unsafe fn write_pte(&mut self, idx: usize, pte: E) {
        debug_assert!(idx < nr_subpage_per_huge::<C>());
        let ptr = paddr_to_vaddr(self.page.paddr()) as *mut E;
        ptr.add(idx).write(pte)
    }
