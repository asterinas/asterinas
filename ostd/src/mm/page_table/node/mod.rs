...
impl<'rcu, C: PageTableConfig> PageTableGuard<'rcu, C> {
    ...
    /// level.
    ///
    /// Callers must provide the virtual address that corresponds to a PTE in
    /// this node.
    ///
    /// # Panics
    ///
    /// Panics if the virtual address is not aligned to the page size at the
    /// current level.
    pub(super) fn entry(&mut self, pte_vaddr: Vaddr) -> Entry<'_, 'rcu, C> {
        assert_eq!(pte_vaddr % page_size::<C>(self.level()), 0);
        Entry::new_at(self, pte_vaddr)
    }
    ...
    pub(super) fn entry_state(&self, idx: usize) -> PteStateRef<'rcu, C> {
        ...
        // SAFETY:
        //  - The PTE is the result of `PteState::into_pte` because it was
...
pub struct PageTableFrameMeta<C: PageTableConfig> {
    ...
    /// The level of the page table page. A page table frame cannot be
