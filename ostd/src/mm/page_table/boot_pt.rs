...
use crate::{
    ...
    mm::{
        ...
        Frame, FrameAllocOptions, Paddr, PageProperty, PagingConstsTrait, PagingLevel, Vaddr,
        PAGE_SIZE,
    },
    sync::SpinLock,
};
...
impl<E: PteTrait, C: PagingConstsTrait> BootPageTable<E, C> {
    ...
    unsafe fn from_current_pt() -> Self {
        ...
        dfs_walk_on_leave::<E, C>(
            ...
            &mut |pte: &mut E, pa, level, mut flags| {
                ...
                *pte = E::from_repr(&PteScalar::PageTable(pa, level, flags));
            },
        );
        Self {
            root_pt,
    ...
    pub unsafe fn map_base_page(&mut self, from: Vaddr, to: Paddr, prop: PageProperty) {
        ...
        while level > 1 {
            ...
            match pte.to_repr(level) {
                ...
                PteScalar::PageTable(ch_pt_pa, _, _) => {
                    pt = ch_pt_pa;
                }
            };
            level -= 1;
    ...
    pub unsafe fn protect_base_page(
        ...
        while level > 1 {
            ...
            match pte.to_repr(level) {
                ...
                PteScalar::PageTable(child_pt, _, _) => {
                    pt = child_pt;
                }
                PteScalar::Mapped(huge_pa, _, prop) => {
                    // Split the huge page.
    ...
    fn alloc_child(&mut self, level: PagingLevel) -> (E, Paddr) {
        ...
        (
            E::from_repr(&PteScalar::PageTable(
                frame_paddr,
                level,
