...
mod test_utils {
    ...
    pub fn unmap_range<C: PageTableConfig>(page_table: &PageTable<C>, range: Range<usize>) {
        ...
        while let PageTablePart::Mapped {
            va: _,
            item: _,
            prop: _,
    ...
    /// Asserts a `PageTablePart` is `Mapped` and cast it to `PageTableItem`.
    #[track_caller]
    pub fn part_to_mapped_item<C: PageTableConfig>(part: PageTablePart<C>) -> PageTableItem<C>
    where
        <C as PageTableConfig>::Item: core::fmt::Debug,
    {
        match part {
            PageTablePart::Mapped { va, item, prop } => PageTableItem::Mapped { va, item, prop },
            _ => panic!("Expected `PageTablePart::Mapped`, got {:#x?}", part),
        }
    }
...
mod range_checks {
    ...
    fn start_boundary_mapping() {
        ...
        let PageTablePart::NotMapped { .. } = (unsafe {
            page_table
                .cursor_mut(&virt_range)
                .unwrap()
...
mod navigation {
    ...
    fn setup_page_table_with_two_frames() -> (PageTable<UserPtConfig>, Frame<()>, Frame<()>) {
        ...
        let old1 = unsafe {
            page_table
                .cursor_mut(&(FIRST_MAP_ADDR..FIRST_MAP_ADDR + PAGE_SIZE))
                .unwrap()
                .map(&frame1_range, page_property)
        };
        assert!(matches!(old1, PageTablePart::NotMapped { .. }));

        let old2 = unsafe {
            page_table
                .cursor_mut(&(SECOND_MAP_ADDR..SECOND_MAP_ADDR + PAGE_SIZE))
                .unwrap()
                .map(&frame2_range, page_property)
        };
        assert!(matches!(old2, PageTablePart::NotMapped { .. }));
    ...
    fn jump() {
        ...
        let mut cursor = page_table
            .cursor_mut(&(0..SECOND_MAP_ADDR + PAGE_SIZE))
            .unwrap();
...
mod mapping {
    ...
    fn mixed_granularity_map_unmap() {
        ...
        {
            let mut cursor = pt.cursor_mut(&unmap_va_range).unwrap();
            ...
            assert_item_is_mapped(
                part_to_mapped_item(unmapped_part),
