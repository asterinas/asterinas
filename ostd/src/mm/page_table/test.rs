...
mod test_utils {
    use super::*;
    use crate::mm::vm_space::{MappedItem, UserPtConfig};

    /// Creates a new user page table that have mapped a virtual range to a physical frame.
    #[track_caller]
    pub fn create_user_pt_mapped_at(virt_range: Range<Vaddr>) -> PageTable<UserPtConfig> {
        ...
        unsafe {
            page_table
                ...
                .unwrap()
                .map(MappedItem::Tracked(frame.into(), page_property))
        }
    ...
    pub fn map_untracked(
        ...
        let _ = unsafe { cursor.map((pa, prop)) };
    }
    ...
    pub fn pa_prop_from_item<C: PageTableConfig>(item: C::Item) -> (Range<Paddr>, PageProperty) {
        let raw = C::item_into_raw(item);
        let res = (raw.0.clone(), raw.1);
        drop(unsafe { C::item_from_raw(raw.0, raw.1) });
        res
    }
    ...
    unsafe impl PageTableConfig for TestPtConfig {
        ...
        type Item = (Range<Paddr>, PageProperty);

        fn item_into_raw(item: Self::Item) -> (Range<Paddr>, PageProperty) {
            item
        }

        unsafe fn item_from_raw(paddr: Range<Paddr>, prop: PageProperty) -> Self::Item {
            (paddr, prop)
        }
    }
}
...
mod page_properties {
    use super::*;
    use crate::mm::{
        vm_space::{MappedItem, UserPtConfig},
        PrivilegedPageFlags,
    };
    ...
    fn check_map_with_property(prop: PageProperty) {
        ...
        let _ = unsafe {
            page_table
                ...
                .unwrap()
                .map(MappedItem::Tracked(frame.into(), prop))
        };
    ...
    fn overlapping_mappings() {
        ...
        let res2 = unsafe {
            page_table
                ...
                .unwrap()
                .map((prange2.clone(), page_property))
        };
        let Err(e) = res2 else {
            panic!(
        ...
        assert_eq!(e.frag.va_range(), vrange1);
        let (remain_pa_range, remain_prop) = TestPtConfig::item_into_raw(e.remain.unwrap());
        assert_eq!(remain_pa_range, (PAGE_SIZE * 201)..(PAGE_SIZE * 202));
        assert_eq!(remain_prop, page_property);
    ...
    fn setup_page_table_with_two_frames() -> (PageTable<UserPtConfig>, Frame<()>, Frame<()>) {
        ...
        unsafe {
            page_table
                ...
                .unwrap()
                .map(MappedItem::Tracked(frame1.clone().into(), page_property))
                .unwrap();
        }
        ...
        unsafe {
            page_table
                ...
                .unwrap()
                .map(MappedItem::Tracked(frame2.clone().into(), page_property))
                .unwrap();
        }
...
mod mapping {
    ...
    fn remap_yields_original() {
        ...
        let Err(e) = (unsafe {
            pt.cursor_mut(&preempt_guard, &virt_range)
                .unwrap()
                .map(MappedItem::Tracked(frame2.into(), page_property))
        }) else {
            panic!("Expected to get error on remapping, got `Ok`");
        };

        assert_eq!(e.frag.va_range(), virt_range);
        assert!(e.remain.is_none());
    }
    ...
    fn mixed_granularity_map_unmap() {
...

        {
            ...
            assert_eq!(frag_pa.start, expected_pa_start);
            assert_eq!(frag_pa.end, expected_pa_start + PAGE_SIZE);
    ...
    fn mixed_granularity_protect_query() {
        ...
        {
            let Some((pa, prop)) = item else {
                panic!("Expected mapped untracked physical address, got `None`");
            };

            assert_eq!(pa.start, mapped_pa_of_va(va.start));
