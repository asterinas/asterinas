...
use crate::{
    mm::{
        frame::{allocator, external::ExternalMeta},
...
fn test_tracked_map_unmap() {
    ...
    let page = allocator::alloc_single(ExternalMeta::default()).unwrap();
...
fn test_base_protect_query() {
    ...
    let to = allocator::alloc_contiguous(999 * PAGE_SIZE, |_| ExternalMeta::default()).unwrap();
    let prop = PageProperty::new(PageFlags::RW, CachePolicy::Writeback);
    unsafe {
