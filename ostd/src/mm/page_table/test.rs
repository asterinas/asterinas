...
mod create_page_table {
    ...
    fn clear_user_page_table() {
        ...
        let page_property = PageProperty::new(PageFlags::RW, 0, CachePolicy::Writeback);
...
mod page_properties {
    ...
    fn uncacheable_policy_mapping() {
        ...
        let invalid_prop = PageProperty::new(PageFlags::RW, 0, CachePolicy::Uncacheable);
        unsafe {
            page_table
    ...
    fn read_write_mapping_preserves_flags() {
        check_map_with_property(PageProperty::new(PageFlags::RW, 0, CachePolicy::Writeback));
    }

    #[ktest]
    fn read_only_mapping_preserves_flags() {
        check_map_with_property(PageProperty::new(PageFlags::R, 0, CachePolicy::Writeback));
    }

    #[ktest]
    fn read_execute_mapping_preserves_flags() {
        check_map_with_property(PageProperty::new(PageFlags::RX, 0, CachePolicy::Writeback));
    }

    #[ktest]
    fn read_write_execute_mapping_preserves_flags() {
        check_map_with_property(PageProperty::new(PageFlags::RWX, 0, CachePolicy::Writeback));
    }
    ...
    fn writethrough_cache_policy_mapping() {
        check_map_with_property(PageProperty::new(
            PageFlags::RW,
            0,
...
mod tracked_mapping {
    ...
    fn remapping_same_range() {
        let page_table = setup_page_table::<UserMode>();
        let range = PAGE_SIZE..(PAGE_SIZE * 2);
        let initial_prop = PageProperty::new(PageFlags::RW, 0, CachePolicy::Writeback);
        let new_prop = PageProperty::new(PageFlags::R, 0, CachePolicy::Writeback);
    ...
    fn user_copy_on_write() {
        ...
        assert_item_is_tracked_frame(
            ...
            PageProperty::new(PageFlags::R, 0, CachePolicy::Writeback), // Parent prop changed by copy_from
        );
        assert!(page_table.query(range.start + 10).is_none());
        ...
        assert_item_is_tracked_frame(
            ...
            PageProperty::new(PageFlags::R, 0, CachePolicy::Writeback), // Child prop was R
        );
        assert!(child_pt.query(range.start + 10).is_none());

        // Maps the range in the sibling using the third clone.
        let sibling_prop_final = PageProperty::new(PageFlags::RW, 0, CachePolicy::Writeback);
        unsafe {
            sibling_pt
                .cursor_mut(&range)
                .unwrap()
...
mod untracked_mapping {
    ...
    fn untracked_large_protect_query() {
        ...
        assert_item_is_untracked_map(
            ...
            PAGE_SIZE,
            PageProperty::new(PageFlags::RW, 0, CachePolicy::Writeback),
        );
            ...
            .zip(protect_ppn_range.clone())
        {
            assert_item_is_untracked_map(
                ...
                PageProperty::new(PageFlags::R, 0, CachePolicy::Writeback),
            );
        }
...
mod boot_pt {
    ...
    fn map_protect() {
        ...
        let from1 = 0x2000;
        let to_phys1 = 0x2;
        let prop1 = PageProperty::new(PageFlags::RW, 0, CachePolicy::Writeback);
        ...
        let expected_prop1_protected = PageProperty::new(PageFlags::RX, 0, CachePolicy::Writeback);
        assert_eq!(
            unsafe { page_walk::<PageTableEntry, PagingConsts>(root_paddr, from1 + 1) },
        ...
        let from2 = 0x3000;
        let to_phys2 = 0x3;
        let prop2 = PageProperty::new(PageFlags::RX, 0, CachePolicy::Uncacheable);
        ...
        let expected_prop2_protected =
            PageProperty::new(PageFlags::RW, 0, CachePolicy::Uncacheable);
        assert_eq!(
            unsafe { page_walk::<PageTableEntry, PagingConsts>(root_paddr, from2 + 2) },
