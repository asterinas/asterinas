...
use super::{
    page_size, pte_index, Entry, PageTable, PageTableConfig, PageTableError, PageTableGuard,
    PagingConstsTrait, PagingLevel, PteState, PteStateRef,
};
use crate::{
    mm::{
...
impl<C: PageTableConfig> CursorMut<'_, C> {
    ...
    pub unsafe fn map(&mut self, item: C::Item) {
        ...
        let frag = self.replace_cur_entry(PteState::Mapped(item));

        self.move_forward();

        if let Some(frag) = frag {
            match frag {
                PageTableFrag::Mapped { va, item: _ } => {
                    panic!("Mapping over an already mapped page at {va:#x}");
                }
                PageTableFrag::StrayPageTable {
                    ...
                    if num_frames > 0 {
                        panic!("Mapping over an already mapped page table subtree at {va:#x}");
                    }
                }
            }
        }
    ...
    ///  - the items mapped in `PageTableFrag` must outlive `'rcu` and any TLB
    ///    entries that caches the mappings in it.
    ///
    /// # Panics
    ///
    /// Panics if:
    ...
    pub unsafe fn take_next(&mut self, len: usize) -> Option<PageTableFrag<C>> {
        self.find_next_impl(len, true, true)?;
    ...
    pub unsafe fn protect_next(
        ...
        self.find_next_impl(len, false, true)?;
