    ...
    pub(crate) fn va_range(&self) -> Range<Vaddr> {
        match self {
            PageTableFrag::Mapped { va, item } => {
                let (_pa, level, _prop) = C::item_into_raw(item.clone());
...
impl<'rcu, C: PageTableConfig> Cursor<'rcu, C> {
    ...
    /// any level, pointing to a child page table within the range.
    ///
...
impl<'rcu, C: PageTableConfig> CursorMut<'rcu, C> {
    ...
    pub unsafe fn map(&mut self, item: C::Item) -> Result<(), PageTableFrag<C>> {
        ...
        let old = self.0.cur_entry().replace(Child::Frame(pa, level, prop));
        self.0.move_forward();
    ...
    pub unsafe fn take_next(&mut self, len: usize) -> Option<PageTableFrag<C>> {
        ...
        if !C::TOP_LEVEL_CAN_UNMAP && self.0.level == C::NR_LEVELS {
            // Go down a level.
            let ChildRef::PageTable(pt) = self.0.cur_entry().to_ref() else {
                panic!("Second-top level page table not exist");
            };
            // SAFETY: The `pt` must be locked and no other guards exist.
            let pt_guard = unsafe { pt.make_guard_unchecked(rcu_guard) };
            self.0.push_level(pt_guard);
        ...
        let old = self.0.cur_entry().replace(Child::None);

        let frag = PageTableFrag::<C>::from_child(rcu_guard, old, self.0.va, self.0.level);
