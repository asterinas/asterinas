    ...
    Clone + Debug + Send + Sync + 'static
{
    ...
    /// Other page tables do not need setting this to `false`.
    ...
    /// # Safety
    ///
    /// The caller must ensure that the physical address points to an item
    /// that was forgotten using [`PageTableConfig::item_into_raw`]. All the
    /// arguments, including the page property, must match the exact values
    /// returned from [`PageTableConfig::item_into_raw`]. For each forgotten
    /// item, this function can be called no more than once.
...
const fn is_valid_range<C: PageTableConfig>(r: &Range<Vaddr>) -> bool {
    ...
    *va_range.start() <= r.start && r.end.saturating_sub(1) <= *va_range.end()
}
