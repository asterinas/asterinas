...
use crate::{
    ...
    mm::{
        ...
        HasPaddr, Infallible, Paddr, Segment, UntypedMem, UntypedMeta, VmIo, VmReader, VmWriter,
        PAGE_SIZE,
    },
};
...
struct DmaStreamInner {
    segment: Segment<dyn UntypedMeta>,
    start_daddr: Daddr,
...
impl DmaStream {
    /// Establishes DMA stream mapping for a given [`Segment<dyn UntypedMeta>`].
    ///
    ...
    /// Gets the underlying [`Segment<dyn UntypedMeta>`].
    ///
    ...
    pub fn segment(&self) -> &Segment<dyn UntypedMeta> {
        &self.inner.segment
    }
