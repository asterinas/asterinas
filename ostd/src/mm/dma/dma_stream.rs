...
impl<D: DmaDirection> DmaStream<D> {
    ...
    pub fn map(segment: USegment, is_cache_coherent: bool) -> Result<Self, Error> {
        ...
        let (inner, paddr) = if (can_sync_dma() || is_cache_coherent) && !has_tdx {
            let paddr = segment.paddr();

            (Inner::Segment(segment), paddr)
        } else {
            let (kva, paddr) = alloc_kva(size, has_tdx, is_cache_coherent)?;
    ...
    /// has been updated by the device side. And the CPU side needs to call the
    /// [`Self::sync_from_device`] method before reading data (e.g., using
    /// [`read_bytes`]).
    ///
...
impl<D: DmaDirection> Drop for DmaStream<D> {
    fn drop(&mut self) {
        // SAFETY: The physical address range was marked in `map`.
        #[cfg(any(debug_assertions, all(target_arch = "x86_64", feature = "cvm_guest")))]
        unsafe {
