...
use crate::{
    mm::{
        frame::{external::ExternalMeta, Segment},
        io::{FallibleVmRead, FallibleVmWrite},
        ExternalFrame, HasPaddr, Infallible, Paddr, VmIo, VmReader, VmWriter,
    },
    Error, Result,
};
...
/// A contiguous segment of untyped memory pages.
///
/// A [`ExternalSegment`] object is a handle to a contiguous range of untyped memory
/// pages, and the underlying pages can be shared among multiple threads.
/// [`ExternalSegment::slice`] can be used to clone a slice of the segment (also can be
...
///
/// Other [`ExternalFrame`] handles can also refer to the pages in the segment. And
...
pub struct ExternalSegment {
    pages: Segment<ExternalMeta>,
}

impl HasPaddr for ExternalSegment {
    fn paddr(&self) -> Paddr {
        self.pages.start_paddr()
    }
}

impl Clone for ExternalSegment {
    fn clone(&self) -> Self {
        Self {
...
impl From<ExternalFrame> for ExternalSegment {
...
impl From<Segment<ExternalMeta>> for ExternalSegment {
    fn from(pages: Segment<ExternalMeta>) -> Self {
        Self { pages }
    }
}

impl VmIo for ExternalSegment {
...
impl Iterator for ExternalSegment {
    type Item = ExternalFrame;

    fn next(&mut self) -> Option<Self::Item> {
        self.pages.next().map(|page| ExternalFrame { page })
    }
}
