...
pub use segment::ExternalSegment;
...
///
/// An instance of `ExternalFrame` is a handle to a page frame (a physical memory
/// page). A cloned `ExternalFrame` refers to the same page frame as the original.
...
/// `ExternalFrame` that refer to the same page frame are dropped, the page frame
/// will be globally freed.
#[derive(Debug, Clone)]
pub struct ExternalFrame {
    page: Frame<ExternalMeta>,
}
...
impl ExternalFrame {
    ...
    pub fn copy_from(&self, src: &ExternalFrame) {
    ...
    /// existing page handles ([`ExternalFrame`]) and all the mappings in the page
...
impl From<Frame<ExternalMeta>> for ExternalFrame {
    fn from(page: Frame<ExternalMeta>) -> Self {
        Self { page }
    }
}

impl TryFrom<DynPage> for ExternalFrame {
    type Error = DynPage;

    /// Try converting a [`DynPage`] into the statically-typed [`ExternalFrame`].
    ///
    ...
    fn try_from(page: DynPage) -> core::result::Result<Self, Self::Error> {
        page.try_into().map(|p: Frame<ExternalMeta>| p.into())
    }
}

impl From<ExternalFrame> for Frame<ExternalMeta> {
    fn from(frame: ExternalFrame) -> Self {
        frame.page
    }
}

impl HasPaddr for ExternalFrame {
    fn paddr(&self) -> Paddr {
        self.start_paddr()
    }
}

impl<'a> ExternalFrame {
...
impl_frame_meta_for!(ExternalMeta);

// Here are implementations for `xarray`.

use core::{marker::PhantomData, ops::Deref};

/// `FrameRef` is a struct that can work as `&'a ExternalFrame`.
///
/// This is solely useful for [`crate::collections::xarray`].
pub struct FrameRef<'a> {
    inner: ManuallyDrop<ExternalFrame>,
    _marker: PhantomData<&'a ExternalFrame>,
}

impl Deref for FrameRef<'_> {
    type Target = ExternalFrame;
...
// SAFETY: `ExternalFrame` is essentially an `*const MetaSlot` that could be used as a `*const` pointer.
// The pointer is also aligned to 4.
unsafe impl xarray::ItemEntry for ExternalFrame {
    type Ref<'a>
        = FrameRef<'a>
    where
        Self: 'a;
    ...
    unsafe fn raw_as_ref<'a>(raw: *const ()) -> Self::Ref<'a> {
        Self::Ref {
            inner: ManuallyDrop::new(ExternalFrame::from_raw(raw)),
            _marker: PhantomData,
        }
    }
}
