...
use external::{ExternalFrame, ExternalMeta};
...
impl<M: FrameMeta> Frame<M> {
    ...
    /// existing page handles ([`Frame`], [`DynPage`]), and all the mappings in the
...
impl<M: FrameMeta> TryFrom<DynPage> for Frame<M> {
    type Error = DynPage;
...
impl<M: FrameMeta> From<Frame<M>> for DynPage {
...
impl From<ExternalFrame> for DynPage {
    fn from(frame: ExternalFrame) -> Self {
        Frame::<ExternalMeta>::from(frame).into()
    }
}
