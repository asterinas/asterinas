...
use crate::{
    mm::{
        page::{cont_pages::ContPages, meta::FrameMeta},
...
/// ```
#[derive(Debug, Clone)]
pub struct Segment<M: FrameMetaExt + ?Sized = DefaultFrameMeta> {
    inner: Arc<ContPages<FrameMeta<M>>>,
    range: Range<usize>,
}

impl<M: FrameMetaExt + ?Sized> HasPaddr for Segment<M> {
    fn paddr(&self) -> Paddr {
        self.start_paddr()
    }
}

impl<M: FrameMetaExt + ?Sized> Segment<M> {
...
impl<'a, M: FrameMetaExt + ?Sized> Segment<M> {
...
impl<M: FrameMetaExt + ?Sized + Send> VmIo for Segment<M> {
...
impl<M: FrameMetaExt + ?Sized> From<Frame<M>> for Segment<M> {
    fn from(frame: Frame<M>) -> Self {
        Self {
            inner: Arc::new(frame.into()),
            range: 0..1,
        }
    }
}

impl<M: FrameMetaExt + ?Sized> From<ContPages<FrameMeta<M>>> for Segment<M> {
