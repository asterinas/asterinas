...
///
/// If a structure implements `UntypedMeta`, it can be used as the metadata of
/// an untyped frame. All frames of such types will be accessible as untyped
/// memory.
pub trait UntypedMeta: FrameMeta {}

/// Checks if a frame metadata type refers to untyped frames.
pub fn is_untyped<M: FrameMeta>() -> bool {
    pub trait IsUntyped {
        const RES: bool;
    }

    impl<MX: FrameMeta> IsUntyped for MX {
        default const RES: bool = false;
    }

    impl<MX: UntypedMeta> IsUntyped for MX {
        const RES: bool = true;
    }
...
/// [`FrameMeta`] and [`UntypedMeta`] with compile-time checks.
#[macro_export]
macro_rules! impl_untyped_meta_for {
    ...
    ($t:ty) => {
        ...
        impl $crate::mm::frame::untyped::UntypedMeta for $t {}
    };
    ...
    ($t:ty, $body:expr) => {
        ...
        unsafe impl $crate::mm::frame::meta::FrameMeta for $t {
            fn on_drop(&mut self, reader: $crate::mm::VmReader<$crate::mm::Infallible>) {
                $body
            }
        }
        impl $crate::mm::frame::untyped::UntypedMeta for $t {}
...
macro_rules! impl_untyped_for {
    ($t:ident) => {
        impl<UM: UntypedMeta + ?Sized> UntypedMem for $t<UM> {
            fn reader(&self) -> VmReader<'_, Infallible> {
        ...
        impl<UM: UntypedMeta + ?Sized> VmIo for $t<UM> {
...
pub struct FrameRef<'a, M: UntypedMeta + ?Sized> {
    inner: ManuallyDrop<Frame<M>>,
    _marker: PhantomData<&'a Frame<M>>,
}

impl<M: UntypedMeta + ?Sized> Deref for FrameRef<'_, M> {
    type Target = Frame<M>;
...
unsafe impl<M: UntypedMeta + ?Sized> xarray::ItemEntry for Frame<M> {
    type Ref<'a>
        = FrameRef<'a, M>
    where
        Self: 'a;
