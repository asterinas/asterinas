...
impl FrameAllocOptions {
    ...
    pub fn alloc_frame_with<M: AnyFrameMeta>(&self, metadata: M) -> Result<Frame<M>> {
        ...
        #[cfg(all(target_arch = "x86_64", feature = "cvm_guest"))]
        if_tdx_enabled!({
            if let Err(err) = unaccepted::accept_memory_if_needed(paddr, PAGE_SIZE) {
                log::error!(
                    "Failed to accept allocated frame: paddr={:#x}, size={:#x}, err={:?}",
                    paddr,
                    PAGE_SIZE,
                    err
                );
                get_global_frame_allocator().dealloc(paddr, PAGE_SIZE);
        ...
        F: FnMut(Paddr) -> M,
    {
        ...
        #[cfg(all(target_arch = "x86_64", feature = "cvm_guest"))]
        if_tdx_enabled!({
            if let Err(err) = unaccepted::accept_memory_if_needed(start, total_size) {
                log::error!(
                    "Failed to accept allocated segment: start={:#x}, size={:#x}, err={:?}",
                    start,
                    total_size,
                    err
                );
                get_global_frame_allocator().dealloc(start, total_size);
                return Err(Error::NoMemory);
            }
        });
...
impl EarlyFrameAllocator {
        ...
        for (tail, end) in [
            ...
            while cursor < end {
                ...
                let accepted = {
                    ...
                    {
                        Self::try_accept_if_needed(allocated, size)
                    }
    ...
    #[cfg(feature = "cvm_guest")]
    fn try_accept_if_needed(start: Paddr, size: usize) -> bool {
        #[cfg(target_arch = "x86_64")]
        if_tdx_enabled!({
            return unaccepted::accept_memory_if_needed(start, size).is_ok();
        });
