...
impl FrameAllocOptions {
    ...
    pub fn alloc_frame_with<M: AnyFrameMeta>(&self, metadata: M) -> Result<Frame<M>> {
        ...
        let paddr = alloc_with_refill(single_layout).ok_or(Error::NoMemory)?;
        ...
        F: FnMut(Paddr) -> M,
    {
        ...
        let start = alloc_with_refill(layout).ok_or(Error::NoMemory)?;
        ...
        if region.typ() == MemoryRegionType::Usable {
            ...
            for r1 in range_difference(&(region.base()..region.end()), &range_1) {
                for r2 in range_difference(&r1, &range_2) {
                    #[cfg(all(target_arch = "x86_64", feature = "cvm_guest"))]
                    let deferred = if_tdx_enabled!({
                        unaccepted::defer_usable_memory(r2.start, r2.len())
                    } else {
                        false
                    });
                    ...
                    if deferred {
                        log::info!("Deferring unaccepted frames from usable range: {:x?}", r2);
                        continue;
                    }
...
fn alloc_with_refill(layout: Layout) -> Option<Paddr> {
    ...
    #[cfg(all(target_arch = "x86_64", feature = "cvm_guest"))]
    {
        crate::if_tdx_enabled!({
            if let Ok(refilled) = unaccepted::refill_for_allocation(layout, false)
                && refilled
                && let Some(paddr) = get_global_frame_allocator().alloc(layout)
            {
                return Some(paddr);
            }

            if let Ok(refilled) = unaccepted::refill_for_allocation(layout, true)
                && refilled
            {
                return get_global_frame_allocator().alloc(layout);
            }
