...
impl FrameAllocOptions {
    ...
    ///
    /// If `zero_init` is `true`, the allocated frames are filled with zeros.
    ...
    pub fn zero_init(&mut self, zero_init: bool) -> &mut Self {
    ...
    pub fn alloc_frame_with<M: FrameMeta>(&self, metadata: M) -> Result<Frame<M>> {
        ...
        if self.zero_init {
            let addr = frame.start_paddr() as *mut u8;
        ...
        F: FnMut(Paddr) -> M,
    {
        ...
        if self.zero_init {
            let addr = segment.start_paddr() as *mut u8;
...
fn test_alloc_dealloc() {
    ...
    contiguous_options.zero_init(false);
