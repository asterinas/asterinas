        ...
        if region.typ() == MemoryRegionType::Usable {
            ...
            if (start..end).contains(&early_allocated_regions.0.start) {
                ...
                if new_start < end {
                    ...
                    debug_assert_eq!(start, early_allocated_regions.0.start);
                    continue;
                }
            }
            if let Some(max_region) = early_allocated_regions.1.as_ref() {
                if (start..end).contains(&max_region.start) {
                    ...
                    if new_start < end {
                        ...
                        debug_assert_eq!(start, max_region.start);
                        continue;
                    }
                }
            }
...
///
/// Calling this method is safe, but using the allocated memory region must be
/// done with care. If the memory region is not tracked in [`super::meta::init`],
/// it should be treated as automatically freed by [`super::meta::init`] and
/// must not be accessed after [`super::meta::init`].
