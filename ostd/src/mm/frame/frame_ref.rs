...
impl<M: AnyFrameMeta + ?Sized> FrameRef<'_, M> {
    /// Gets a [`FrameRef`] from a raw physical address.
    ///
    /// # Safety
    ///
    /// The caller must ensure that:
    ///  - there's one frame that outlives the created reference, so that the
    ///    reference can be seen as borrowed from that frame.
    ///  - the type of the [`FrameRef`] (`M`) matches the borrowed frame.
    pub(crate) unsafe fn borrow_from_raw(raw: Paddr) -> Self {
        Self {
