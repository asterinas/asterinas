...
pub struct FrameAllocOptions<M: FrameMetaExt + ?Sized = DefaultFrameMeta> {
    nframes: usize,
    is_contiguous: bool,
    uninit: bool,
    _type_of_ext_meta: PhantomData<M>,
}

impl<M: FrameMetaExt + ?Sized + Default> FrameAllocOptions<M> {
    ...
    pub fn alloc_single(&self) -> Result<Frame<M>> {
        ...
        let frame = page::allocator::alloc_single().ok_or(Error::NoMemory)?;
        if !self.uninit {
            frame.writer().fill(0);
        }
