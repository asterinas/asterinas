...
impl<A: AuxPageTableMeta> Cursor<'_, A> {
    ...
    /// and before `end`, it will return that mapped address. In this case,
...
impl<'a, A: AuxPageTableMeta> CursorMut<'a, A> {
    ...
    /// whichever comes first. The cursor stops at the last mapped slot.
    ///
    /// # Limitations
    ///
    ...
    pub fn protect(&mut self, mut protect_fn: impl FnMut(&mut PageFlags, &mut CachePolicy)) {
        ...
        unsafe {
            self.pt_cursor.protect(&mut |prop| {
                protect_fn(&mut prop.flags, &mut prop.cache);
            })
        }
    }
}
