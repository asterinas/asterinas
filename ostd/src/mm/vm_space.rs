...
pub struct VmSpace {
    ...
    cpus: AtomicCpuSet,
    iomems: SpinLock<BTreeSet<IoMem>>,
}
...
impl VmSpace {
    ...
    pub fn new() -> Self {
        Self {
            ...
            iomems: SpinLock::new(BTreeSet::new()),
        }
    }
...
impl<'a> CursorMut<'a> {
    ...
    pub fn map_iomem(&mut self, io_mem: IoMem, prop: PageProperty, len: usize, offset: usize) {
        ...
        if offset >= io_mem.length() {
            return;
        }

        let paddr_begin = io_mem.paddr() + offset;
        let paddr_end = if io_mem.length() - offset < len {
            io_mem.paddr() + io_mem.length()
        } else {
            io_mem.paddr() + offset
        };
        ...
        // If the `iomems` set in `VmSpace` does not contain the current I/O
...
impl VmItem {
    ...
    /// Converts the `VmItem` into a `VmQueriedItem`.
    fn into_queried_item(self) -> VmQueriedItem {
