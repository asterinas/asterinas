    ...
    fn handle_remapped_frag(&mut self, frag: PageTableFrag<UserPtConfig>, start_va: Vaddr) {
        match frag {
            PageTableFrag::Mapped { va, item } => {
                ...
                match unsafe { RcuDrop::into_inner(item) }.mapped_item {
        ...
        loop {
            ...
            match frag {
                PageTableFrag::Mapped { va, item, .. } => {
                    ...
                    // pointer), we will drop it after the RCU grace period
                    // (see `issue_tlb_flush_with`).
                    match unsafe { RcuDrop::into_inner(item) } {
                        VmItem {
                ...
                PageTableFrag::StrayPageTable {
                    ...
                    // SAFETY: By `issue_tlb_flush_with`, the page table will
                    // be dropped after the RCU grace period.
                    let pt = unsafe { RcuDrop::into_inner(pt) };
                    self.flusher
                        .issue_tlb_flush_with(TlbFlushOp::for_range(va..va + len), pt.into());
                }
            }
        }
