            ...
            PageTableFrag::Mapped { va, item } => {
                ...
                match item.mapped_item {
                    MappedItem::TrackedFrame(old_frame) => {
                        let drop_after_flush = Frame::rcu_from_unsized(RcuDrop::new(old_frame));
                ...
                PageTableFrag::Mapped { va, item, .. } => {
                    ...
                    match item {
                        VmItem {
                            ...
                            let drop_after_flush = Frame::rcu_from_unsized(RcuDrop::new(old_frame));
                            self.flusher
                                .issue_tlb_flush_with(TlbFlushOp::for_single(va), drop_after_flush);
                        }
                        VmItem {
                            mapped_item: MappedItem::UntrackedIoMem { level, .. },
                            ..
                        } => {
                            num_unmapped += nr_base_per_page::<UserPtConfig>(level);
