...
use super::{frame::AnyFrame, Vaddr, PAGE_SIZE};
use crate::{
...
impl<G: PinCurrentCpu> TlbFlusher<G> {
    ...
    pub fn issue_tlb_flush_with(&self, op: TlbFlushOp, drop_after_flush: AnyFrame) {
    ...
    fn issue_tlb_flush_(&self, op: TlbFlushOp, drop_after_flush: Option<AnyFrame>) {
...
cpu_local! {
    ...
    static PAGE_KEEPER: SpinLock<Vec<AnyFrame>, LocalIrqDisabled> = SpinLock::new(Vec::new());
}
