...
/// I/O memory dispatcher that dispatches memory I/O access to device drivers.
pub struct IoMemDispatcher {
    allocators: Vec<VirtAddrAllocator>,
}

impl IoMemDispatcher {
    /// Gets the I/O memory access for `range`.
    ///
    ...
    pub fn get(&self, range: Range<usize>) -> Option<IoMem> {
        ...
        debug!("Getting MMIO range:{:x?}..{:x?}", range.start, range.end);
    ...
    /// # Safety
    ///
    /// The caller must have ownership of the MMIO region through the `IoMemDispatcher::get` interface.
    #[allow(dead_code)]
    pub(in crate::device) unsafe fn recycle(&self, range: Range<usize>) {
...
/// Builder for `IoMemDispatcher`.
///
/// The builder must contains the memory I/O regions that don't belong to the physical memory. Also, OSTD
/// must exclude the memory I/O regions of the system device before building the `IoMemDispatcher`.
pub(crate) struct IoMemDispatcherBuilder {
    allocators: Vec<VirtAddrAllocator>,
}

impl IoMemDispatcherBuilder {
...
pub static IO_MEM_DISPATCHER: Once<IoMemDispatcher> = Once::new();

/// Initializes the static `IO_MEM_DISPATCHER` based on builder.
///
/// # Safety
///
/// User must ensure all the memory I/O regions that belong to the system device have been removed by calling the
/// `remove` function.
pub(crate) unsafe fn init(builder: IoMemDispatcherBuilder) {
    IO_MEM_DISPATCHER.call_once(|| IoMemDispatcher::new(builder.allocators));
}
...
mod test {
    ...
    use super::{IoMemDispatcher, IoMemDispatcherBuilder};
    ...
    fn illegal_region() {
        ...
        let dispatcher =
            unsafe { IoMemDispatcher::new(IoMemDispatcherBuilder::new(range).allocators) };
