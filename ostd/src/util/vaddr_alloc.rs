...
use crate::{
    mm::kspace::kvirt_area::KVirtAreaFreeNode,
    prelude::*,
...
impl VirtAddrAllocator {
    ...
    pub fn alloc_specific(&self, allocate_range: Range<Vaddr>) -> Result<()> {
        ...
        for (key, value) in freelist.iter() {
            ...
            if block.end >= allocate_range.end && block.start <= allocate_range.start {
                target_node = Some(*key);
                left_length = allocate_range.start - block.start;
                right_length = block.end - allocate_range.end;
                break;
            }
        }
        ...
        if let Some(key) = target_node {
            if left_length == 0 {
                ...
                freenode.block_mut().end = allocate_range.start;
            }
    ...
    pub fn alloc(&self, size: usize) -> Result<Range<Vaddr>> {
        ...
        for (key, value) in freelist.iter() {
            ...
            if block.end - block.start >= size {
                allocate_range = Some((block.end - size)..block.end);
                to_remove = Some(*key);
                break;
            }
        }
        ...
        if let Some(key) = to_remove {
            if let Some(freenode) = freelist.get_mut(&key) {
                let freenode_block = freenode.block_mut();
                if freenode_block.end - size == freenode_block.start {
                    freelist.remove(&key);
                } else {
                    freenode_block.end -= size;
                }
            }
        }
    ...
    pub fn free(&self, range: Range<Vaddr>) {
        ...
        if let Some((prev_va, prev_node)) = freelist
            ...
            .peek_prev()
        {
            let prev_node_block = prev_node.block();
            if prev_node_block.end == free_range.start {
                let prev_va = *prev_va;
                free_range.start = prev_node_block.start;
            ...
            .peek_next()
        {
            ...
            if free_range.end == next_node_block.start {
                let next_va = *next_va;
                free_range.end = next_node_block.end;
                freelist.remove(&next_va);
                freelist.get_mut(&free_range.start).unwrap().block_mut().end = free_range.end;
            }
        }
    }
