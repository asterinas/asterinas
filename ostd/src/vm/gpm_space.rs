...
impl Drop for GuestPhysMemSpace {
    fn drop(&mut self) {
        // The page table still owns the root, so its frame is in use.
        let root = RcuDrop::new(Frame::from_in_use(self.pt.root_paddr()).unwrap());
        invept::invalidate(&self.vmx_guard, core::slice::from_ref(&root));
        // SAFETY: Every CPU now retains the root frame until its translations
        // are invalidated. This is the only place that drops the page table.
...
impl<'a> CursorMut<'a> {
    ...
    /// # Panics
    ///
    /// Panics if `len` is unaligned or exceeds the remaining
    ...
    pub fn protect_next(
        ...
        invept::invalidate(self.vmx_guard, &[]);
        Some(range)
    }
    ...
    pub fn unmap(&mut self, len: usize) -> usize {
        ...
        if !frames.is_empty() {
            invept::invalidate(self.vmx_guard, &frames);
        }
...
mod test {
    ...
    fn guest_mapping_lifecycle() {
...

        {
            ...
            // This retains backing frames until asynchronous INVEPT completes.
            assert_eq!(cursor.unmap(2 * PAGE_SIZE), 2);
            cursor.jump(PAGE_SIZE).unwrap();
            assert!(cursor.find_next(3 * PAGE_SIZE).is_none());
        }

        // Reuse the same GPA after unmapping and observe the replacement PTE.
        {
            let guard = disable_preempt();
