...
use crate::{
    ...
    error::Error,
    mm::{
        AnyUFrameMeta, PageFlags, PageProperty, UFrame,
        frame::FrameRef,
...
impl GuestPhysMemSpace {
    ...
    pub fn new() -> Result<Self> {
        if !invept::SUPPORT_EPT.load(Ordering::Relaxed) {
    ...
    pub fn cursor<'a, G: AsAtomicModeGuard>(
        ...
        // Keep IRQs enabled while waiting for page-table locks so this CPU
        // can service invalidation IPIs sent by another lock holder.
...
impl Drop for GuestPhysMemSpace {
    fn drop(&mut self) {
        invept::invalidate(&self.vmx_guard).unwrap();
...
impl<'a> CursorMut<'a> {
    ...
    pub fn protect_next(
        ...
        invept::invalidate(self._vmx_guard).unwrap();
        Some(range)
    }
    ...
    pub fn unmap(&mut self, len: usize) -> usize {
        ...
        loop {
            ...
            // 2. Removed fragments retain ownership of their backing frames and
            //    page-table nodes in `ManuallyDrop`. Following code releases them
            //    only after EPT invalidation succeeds on all CPUs.
        ...
        // Invalidate EPT translations on all CPUs before dropping the fragments.
        invept::invalidate(self._vmx_guard).unwrap();
        for frag in frags {
...

        {
            ...
            assert_eq!(
                cursor
                    ...
                    .unwrap()
                    .1
                    .map(|(frame, prop)| (frame.paddr(), prop)),
