...
use super::{
    entry::{NodeEntryRef, XEntry},
    mark::{NoneMark, XMark},
    node::{Height, XNode},
    XArray, SLOT_SIZE,
};
use crate::{
    sync::non_null::{NonNullPtr, OwnedPtrRef},
    task::atomic_mode::AsAtomicModeGuard,
};

/// A type representing the state of a [`Cursor`] or a [`CursorMut`]. Currently,
...
///    to it.
///
/// Currently, a cursor never ends up on an interior node. In other words, when methods
...
impl<P: NonNullPtr + Sync> Default for CursorState<'_, P> {
    fn default() -> Self {
        Self::Inactive
    }
}

impl<'a, P: NonNullPtr + Sync> CursorState<'a, P> {
...
/// target index and can perform read-only operations to the target item represented
/// by the target index.
///
...
impl<'a, P: NonNullPtr + Sync, M: Into<XMark>> Cursor<'a, P, M> {
    ...
    fn traverse_to_target(&mut self) {
        ...
        let head_node = head.as_node_ref().unwrap();

        let max_index = head_node.height().max_index();
        if max_index < self.index {
            return;
        }

        self.state.move_to(head_node, self.index);
    ...
    fn continue_traverse_to_target(&mut self) {
        while !self.state.is_at_leaf() {
            ...
            let Some(next_node) = current_node
                ...
                .map(|operated_entry| operated_entry.as_node_ref().unwrap())
            else {
                self.reset();
                return;
            };
    ...
    pub fn load(&mut self) -> Option<P::Ref<'a>> {
        ...
        node.read_target()
            .entry_with(self.guard, operation_offset)
            .and_then(|item_entry| item_entry.as_item_ref())
    }
    ...
    /// Updates marks on ancestors if necessary and resets the cursor to the
    /// inactive state.
    ...
    pub fn next(&mut self) {
        ...
        while operation_offset == SLOT_SIZE as u8 {
            let Some(parent_node) = current_node.read_target().parent(self.guard) else {
                self.reset();
                return;
            };
...
/// target index and can perform read-write operations to the target item represented
/// by the target index.
///
...
impl<'a, P: NonNullPtr + Sync, M: Into<XMark>> CursorMut<'a, P, M> {
    pub(super) fn new(xa: &'a XArray<P, M>, guard: &'a dyn AsAtomicModeGuard, index: u64) -> Self {
    ...
    fn reserve(&self, index: u64) {
        let Some(mut head) = self.xa.head.read_with(self.guard) else {
            let height = Height::from_index(index);
            let new_entry = XEntry::from_node(XNode::new_root(height));
            self.xa.head.update(Some(new_entry));
            return;
        };

        loop {
            let head_node = head.as_node_ref().unwrap();
            let height = head_node.height();
            if height.max_index() >= index {
                return;
            }

            let new_entry = {
                let new_node = XNode::new_root(height.go_root());
                new_node.slots[0].swap_with(&self.xa.head);
                XEntry::from_node(new_node)
            };

            self.xa.head.update(Some(new_entry));
            head = self.xa.head.read_with(self.guard).unwrap();

            head_node.set_parent(&head.as_node_ref().unwrap())
        }
    }

    /// Traverses from an interior node to the leaf node according to the target index,
    /// potentially with creating new nodes.
    ...
    fn expand_and_traverse_to_target(&mut self) {
        ...
        self.0
            .state
            .move_to(head.as_node_ref().unwrap(), self.0.index);
    ...
    pub fn store(&mut self, item: P) {
        ...
        node.set_entry(self.guard, operation_offset, Some(XEntry::from_item(item)));
    }

    /// Removes the item at the target index, and returns the removed item
    ...
    pub fn remove(&mut self) -> Option<P::Ref<'a>> {
        ...
        self.state.as_node().and_then(|(node, off)| {
            let res = node
                ...
                .and_then(|entry| entry.as_item_ref());
            node.set_entry(self.guard, off, None);
            res
        })
    }

    /// Sets the input `mark` for the item at the target index if the
    /// target item exists, otherwise returns an error.
        ...
        self.state
            ...
            .filter(|(node, off)| {
                node.entry_with(self.guard, *off)
                    .is_some_and(|entry| entry.is_item())
            })
            .map(|(node, off)| {
    ...
    /// Unsets the input `mark` for the item at the target index if
    /// the target item exists, otherwise returns an error.
...
impl<'a, P: NonNullPtr + Sync, M: Into<XMark>> Deref for CursorMut<'a, P, M> {
...
impl<P: NonNullPtr + Sync, M: Into<XMark>> DerefMut for CursorMut<'_, P, M> {
