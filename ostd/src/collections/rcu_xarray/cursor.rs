...
use crate::{
    sync::non_null::NonNullPtr, task::atomic_mode::AsAtomicModeGuard, util::either::Either,
};
...
impl<'a, P: NonNullPtr + Send + Sync, M: Into<XMark>> Cursor<'a, P, M> {
    ...
    fn traverse_to_target(&mut self) {
        ...
        let head_node = head.left().unwrap();
...
impl<'a, P: NonNullPtr + Send + Sync, M: Into<XMark>> CursorMut<'a, P, M> {
    pub(super) fn new(xa: &'a XArray<P, M>, guard: &'a dyn AsAtomicModeGuard, index: u64) -> Self {
    ...
    fn reserve(&self, index: u64) {
        if self.xa.head.read_with(self.guard).is_none() {
            ...
            let new_entry = Either::Left(Arc::new(XNode::new_root(height)));
        ...
        loop {
            ...
            let new_entry = {
                let new_node = Arc::new(XNode::new_root(height.go_root()));
                new_node.slots[0].update(Some(Either::Left(head_node.clone())));
                head_node.set_parent(&new_node);
                Either::Left(new_node)
            };
    ...
    fn expand_and_traverse_to_target(&mut self) {
        ...
        self.0.state.move_to(head.left().unwrap(), self.0.index);
...
impl<'a, P: NonNullPtr + Send + Sync, M: Into<XMark>> Deref for CursorMut<'a, P, M> {
...
impl<P: NonNullPtr + Send + Sync, M: Into<XMark>> DerefMut for CursorMut<'_, P, M> {
