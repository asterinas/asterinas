...
impl<P: NonNullPtr + Sync, M: Into<XMark>> Default for XArray<P, M> {
    fn default() -> Self {
        Self::new()
    }
}

impl<P: NonNullPtr + Sync, M: Into<XMark>> XArray<P, M> {
    ...
    G: SpinGuardian,
{
    ...
    /// Stores the provided item in the [`XArray`] at the target index, and returns
    /// the old item if some item was previously stored in the same position.
