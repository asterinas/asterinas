...
use super::{
    entry::{NodeEntryRef, XEntry, XEntryRef},
    mark::{Mark, NUM_MARKS},
    BITS_PER_LAYER, SLOT_MASK, SLOT_SIZE,
};
use crate::{
...
impl Height {
    ...
    /// Creates a `Height` which has the minimal height value but allows
    /// the `index`-th item to be stored.
...
where
    P: NonNullPtr + Sync,
{
    /// The pointer refers to the parent node.
    ///
    /// If the current node is the head node, its parent pointer will
    /// be `None`. If the parent pointer is `Some(_)`, it must be a valid
    /// pointer refers to a `XNode`.
    parent: RcuOption<Weak<XNode<P>>>,
    /// The height of the subtree rooted at the current node. The height of a
    /// leaf node, which stores the user-given items, is 1.
    height: Height,
    ...
    offset_in_parent: u8,
    /// The slots storing `XEntry`s, which point to user-given items for leaf
    /// nodes and other `XNode`s for interior nodes.
    pub(super) slots: [RcuOption<XEntry<P>>; SLOT_SIZE],
...
/// SAFETY: Once `P` is `Sync`, the `slots` will be `Sync` and `Send` if
/// the node is a leaf node. Then, `Sync` and `Send` can be propagate from
/// the leaf node to the root node.
unsafe impl<P: NonNullPtr + Sync> Send for XNode<P> {}
unsafe impl<P: NonNullPtr + Sync> Sync for XNode<P> {}
...
impl<P: NonNullPtr + Sync> XNode<P> {
    ...
    pub fn new(height: Height, offset: u8) -> Self {
        Self {
            ...
            slots: core::array::from_fn::<RcuOption<XEntry<P>>, SLOT_SIZE, _>(|_| {
                RcuOption::new_none()
            }),
    ...
    pub fn set_parent(&self, parent: &NodeEntryRef<'_, P>) {
        let parent_ptr = Arc::downgrade(parent);
        self.parent.update(Some(parent_ptr));
    }

    pub fn parent<'a>(&'a self, guard: &'a dyn AsAtomicModeGuard) -> Option<NodeEntryRef<'a, P>> {
        let parent = self.parent.read_with(guard)?;
        // SAFETY: This parent pointer is generated by `Arc<XNode<P>>::downgrade`, and
        // the `Arc` pointer remains valid throughout the `'a` lifetime without any mutable
        // references.
        unsafe { Some(Arc::raw_as_ref(Weak::ref_as_raw(parent))) }
    }
    ...
    pub fn set_entry(&self, guard: &dyn AsAtomicModeGuard, offset: u8, entry: Option<XEntry<P>>) {
        let is_new_node = entry.as_ref().is_some_and(|entry| entry.is_node());
    ...
    pub fn set_mark(&self, guard: &dyn AsAtomicModeGuard, offset: u8, mark: usize) {
        let changed = self.marks[mark].update(offset, true);
        if changed {
    ...
    /// This method will also update the marks on the ancestors of this node
    /// if necessary to ensure that the marks on the ancestors are up to date.
    pub fn unset_mark(&self, guard: &dyn AsAtomicModeGuard, offset: u8, mark: usize) {
        let changed = self.marks[mark].update(offset, false);
        if changed {
            self.propagate_mark(guard, mark);
        }
    }
    ...
    /// This method will also update the marks on the ancestors of this node
    /// if necessary to ensure that the marks on the ancestors are up to date.
    fn update_mark(&self, guard: &dyn AsAtomicModeGuard, offset: u8) {
        let entry = self.slots[offset as usize].read_with(guard);
        let Some(node) = entry.and_then(|entry| entry.as_node_ref()) else {
            return;
        };

        for i in 0..NUM_MARKS {
            let changed = self.marks[i].update(offset, !node.is_mark_clear(i));
            if changed {
    ...
    /// Propagates the updating to the marks on the ancestors of this node
    /// if necessary to ensure that the marks on the ancestors are up to date.
    fn propagate_mark(&self, guard: &dyn AsAtomicModeGuard, mark: usize) {
        let Some(parent) = self.parent(guard) else {
            return;
        };

        let changed = parent.marks[mark].update(self.offset_in_parent, !self.is_mark_clear(mark));
        if changed {
