...
#[derive(Debug)]
struct CpuLocalStoragePointerMeta;
impl_frame_meta_for!(CpuLocalStoragePointerMeta);
...
impl CpuLocalStoragePointers {
    ...
    unsafe fn allocate(num_cpus: usize) -> Self {
        ...
        if num_aps == 0 {
            panic!("No APs to allocate CPU-local storage");
        }
        ...
        // SAFETY: The memory would not be deallocated since we
        // call `initialize_frame_metadata` after. And it is valid.
    ...
    /// # Safety
    ///
    /// Should only be called once in [`initialize_frame_metadata`].
    unsafe fn initialize_frame_metadata(&self) {
        let size = core::mem::size_of_val(self.0).align_up(PAGE_SIZE);
        let paddr = self.0.as_ptr() as usize - LINEAR_MAPPING_BASE_VADDR as Paddr;
        let seg =
            Segment::from_unused(paddr..paddr + size, |_| CpuLocalStoragePointerMeta).unwrap();
...
/// Initializes the frame metadata of early allocated frames in this module.
///
/// # Safety
///
/// This function should only be called once on the BSP, inside
/// [`crate::mm::frame::meta::init`].
pub(crate) unsafe fn initialize_frame_metadata() {
    let Some(pointers) = CPU_LOCAL_STORAGES.get() else {
        debug_assert_eq!(super::num_cpus(), 1);
        return;
    };

    // SAFETY: It is required to be called here.
    unsafe { pointers.initialize_frame_metadata() };

    let bsp_base_va = __cpu_local_start as usize;
    let bsp_end_va = __cpu_local_end as usize;
    let nbytes = (bsp_end_va - bsp_base_va).align_up(PAGE_SIZE);

    for id in 1..super::num_cpus() {
        let ap_pages = pointers.get(CpuId::try_from(id).unwrap());
        let seg =
            Segment::from_unused(ap_pages..ap_pages + nbytes, |_| CpuLocalStorageMeta).unwrap();
        let _ = ManuallyDrop::new(seg);
    }
}
