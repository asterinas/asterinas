...
/// [`CpuLocalAllocator`], not directly. Dropping it without deallocation
/// will cause panic.
///
/// We expect the user to implement both allocation and deallocation for
/// `CpuLocal<T, DynamicStorage<T>>`. However, we cannot call the user-defined
/// `dealloc<T>` when dropping a `DynamicStorage<T>`. This is because the call
/// is an up-call, and a dynamic reference (`dyn Trait`) cannot carry a generic
/// type parameter like `T`. As a result, the user must ensure that
/// `CpuLocalAllocator::dealloc<T>(cpu_local)` is explicitly invoked. Dropping
/// `DynamicStorage<T>` directly would bypass this deallocation and lead to a
/// memory leak.
pub struct DynamicStorage<T>(*mut T);

unsafe impl<T> AnyStorage<T> for DynamicStorage<T> {
    fn get_ptr_on_current(&self, guard: &DisabledLocalIrqGuard) -> *mut T {
        self.get_ptr_on_target(guard.current_cpu())
    }

    fn get_ptr_on_target(&self, cpu_id: CpuId) -> *mut T {
...
impl<T: alloc::fmt::Debug> alloc::fmt::Debug for DynamicStorage<T> {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        let mut list = f.debug_list();
        for cpu in all_cpus() {
            let ptr = self.get_ptr_on_target(cpu);
