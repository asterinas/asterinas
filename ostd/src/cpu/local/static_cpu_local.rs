...
/// Defines a static CPU-local variable.
///
...
unsafe impl<T: 'static> AnyStorage<T> for StaticStorage<T> {
    fn get_ptr_on_current(&self, _guard: &DisabledLocalIrqGuard) -> *mut T {
        self.as_ptr()
    }
    ...
    fn get_ptr_on_target(&self, cpu_id: CpuId) -> *mut T {
        ...
        if cpu_id == 0 {
            return (&self.0 as *const T).cast_mut();
        }
...
impl<T: 'static> CpuLocal<T, StaticStorage<T>> {
    /// Creates a new static CPU-local object.
    ///
