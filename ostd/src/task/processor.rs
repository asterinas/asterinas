...
use super::{context_switch, CurrentTask, Task, TaskContext};
...
pub(super) fn switch_to_task(next_task: Arc<Task>) {
    ...
    let current_task_ptr = Arc::into_raw(next_task);
    CURRENT_TASK_PTR.store(current_task_ptr);

    // SAFETY: `current_task_ptr` now points to the current `Task`.
    let current_task =
        unsafe { CurrentTask::new(NonNull::new(current_task_ptr.cast_mut()).unwrap()) };
    if let Some(vm_space) = current_task.local_data().vm_space() {
        vm_space.activate();
    }
