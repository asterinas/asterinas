...
fn prepare_lazy_accept_info() -> LazyAcceptInfoResult {
    ...
    if !raw_kernel_ranges.is_empty() {
        let min_start = raw_kernel_ranges.first().unwrap().0;
        let max_end = raw_kernel_ranges.last().unwrap().1;

        // Add a safety margin to the kernel footprint to account for
        // any additional memory that may be accessed during early boot.
        let safety_margin = 2 * 1024 * 1024;
        let footprint_end = (max_end + safety_margin + 0xFFF) & !0xFFF;
    ...
    // 3. Manually add the AP execution page to the kernel load ranges, which is required for AP startup.
    // In the linker script, AP_EXEC_MA = 0x8000, this memory is not within the ELF's LMA range and must be added manually.
    kernel_load_ranges.push((0x8000, 0x9000));
    ...
    for entry in pre_exit_memory_map.entries() {
        if entry.ty != uefi::table::boot::MemoryType::UNACCEPTED {
            continue;
        }

        let entry_start = entry.phys_start;
        let entry_end = entry.phys_start + entry.page_count * PAGE_SIZE;
        unaccepted_ranges.push(UnacceptedRange {
            start: entry_start,
            end: entry_end,
        });
    }
