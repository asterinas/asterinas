...
pub(crate) fn allocate_unaccepted_bitmap(
    ...
    match uefi::boot::allocate_pages(AllocateType::AnyPages, MemoryType::ACPI_RECLAIM, pages) {
        Ok(addr) => {
            ...
            table.version = 1;
            table.unit_size = u32::try_from(unit_size).ok()?;
            table.phys_base = aligned_start;
            table.size = bitmap_bytes;
...
pub(crate) fn install_unaccepted_bitmap(
    ...
    #[cfg(feature = "debug_print")]
    {
        ...
        let version = table.version;
        let unit_size = table.unit_size;
        let phys_base = table.phys_base;
        let size = table.size;

        uefi::println!("  Version: {}", version);
        uefi::println!("  Unit size: {}KB", unit_size / 1024);
        uefi::println!("  Physical base: {:#x}", phys_base);
        uefi::println!("  Bitmap size: {} bytes", size);
    }
