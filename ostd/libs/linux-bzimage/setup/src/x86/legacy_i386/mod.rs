...
extern "cdecl" fn main_legacy32(boot_params_ptr: *mut BootParams) -> ! {
    ...
    // SAFETY: We get boot parameters from the boot loader. By contract the memory map is correct.
    unsafe { alloc::init(&boot_params.e820_table) };
...
fn panic(info: &core::panic::PanicInfo) -> ! {
    ...
    loop {
        // SAFETY: `halt` has no effect other than to stop the CPU and wait for another interrupt.
