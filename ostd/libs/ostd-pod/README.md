...
This crate provides the [`Pod`](https://docs.rs/ostd-pod/latest/trait.Pod.html) trait, 
which marks types that can be safely converted to and from arbitrary byte sequences. 
It's built on top of the mature [`zerocopy`](https://docs.rs/zerocopy/) crate to ensure type safety.

## Features

- **Safe Byte Conversion**: POD types can be safely converted to byte sequences and created from
  byte sequences.
- **Based on zerocopy**: Built on top of the [zerocopy](https://docs.rs/zerocopy/) crate for type
  safety guarantees.
...
To address this, we provide a [`pod_union`](https://docs.rs/ostd-pod/latest/attr.pod_union.html) macro
...
The [`padding_struct`](https://docs.rs/ostd-pod/latest/attr.padding_struct.html) macro
automatically inserts explicit padding fields
