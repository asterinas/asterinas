...
//! for example, ensure padding bytes are initialized when working with `bytemuck`.
//!

use proc_macro::TokenStream;
use quote::quote;
use syn::{
    Attribute, Data, DataStruct, DeriveInput, Expr, Fields, Ident, Token,
...
/// Arguments for the `#[padding_struct]` attribute macro
struct PaddingStructArgs {
    /// Optional target struct size (can be a literal or constant expression)
    size: Option<Expr>,
}

impl Parse for PaddingStructArgs {
        ...
        if !input.is_empty() {
            ...
            if ident == "size" {
                ...
                return Err(syn::Error::new_spanned(
                    ident,
                    "Unknown parameter, expected 'size'",
                ));
            }
        }

        Ok(PaddingStructArgs { size })
    }
}
...
/// # Parameters
///
/// - `size` (optional): Specifies the total size of the struct, can be:
///   - An integer literal: `#[padding_struct(size = 4096)]`
///   - A constant expression: `#[padding_struct(size = core::mem::size_of::<u64>())]`
///
/// # Requirements
///
...
/// # Examples
///
/// Basic usage without size parameter:
///
/// ```rust
...
/// struct MyStruct {
///     a: u8,
///     pub __pad1: [u8; { core::mem::offset_of!(__MyStruct, b) - core::mem::offset_of!(__MyStruct, a) - core::mem::size_of::<u8>() }],
///     b: u32,
///     pub __pad2: [u8; { core::mem::size_of::<__MyStruct>() - core::mem::offset_of!(__MyStruct, b) - core::mem::size_of::<u32>() }],
/// }
/// ```
///
/// Usage with size parameter:
///
/// ```rust
/// use padding_struct::padding_struct;
///
...
///     data: [u8; 16],
/// }
/// // The padding after the last field will make the entire struct 4096 bytes
/// ```
#[proc_macro_attribute]
pub fn padding_struct(args: TokenStream, input: TokenStream) -> TokenStream {
    let args = parse_macro_input!(args as PaddingStructArgs);
    ...
    let ref_name = Ident::new(&format!("__{}", name), name.span());
    ...
    for (i, field) in field_vec.iter().enumerate() {
        ...
        let pad_size_expr = if i == field_vec.len() - 1 {
            // Last field: padding to end of struct or specified size
            if let Some(ref size_expr) = args.size {
                quote! {
                    (#size_expr) - core::mem::offset_of!(#ref_name #ty_generics, #field_name)
