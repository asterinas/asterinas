...
pub enum PanicAttr {
    NoPanic,
    ShouldPanic,
    ExpectPanic(&'static str),
}

impl KtestItem {
    /// Creates a new [`KtestItem`].
    ///
    ...
    pub fn panic_attr(&self) -> &PanicAttr {
        &self.panic_attr
    }

    /// Gets the function pointer to the test.
    pub fn fn_(&self) -> fn() {
        self.fn_
    }
}
...
/// Gets the whitelist of the tests.
///
...
/// Gets the whitelist of the crates.
///
