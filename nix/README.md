...
toolchain comes from `rust-toolchain.toml`. The boot stack (QEMU, GRUB, OVMF)
and the nixpkgs commit are pinned to the versions the Docker image uses. Lint
and doc tools (typos, mdbook, ...) come from nixpkgs and may lag the versions
the Docker image installs with `cargo install`; `make check` inside the Docker
image is what CI runs. klint is left to the Docker image, since no build or
check target invokes it.
