...
This RFC proposes extending OSTD with hypervisor support. These OSTD facilities allow the Asterinas kernel to safely implement a complete hypervisor, including a KVM-compatible control plane, while keeping privileged virtualization mechanisms inside OSTD.

## Motivation

Asterinas needs virtualization support for existing Linux KVM-based VMMs and unmodified guest kernels.

Supporting virtualization requires extending the framekernel boundary with care. Taking x86 as an example, a malformed VMCS, a bad EPT mapping, or an incorrectly restored host register can break the soundness semantics of the host kernel. Therefore, this RFC focuses on what kind of OSTD hypervisor layer Asterinas needs: it must provide soundness by encapsulating privileged virtualization mechanisms, minimality by keeping the trusted surface small, and expressiveness by giving the Asterinas kernel enough safe operations to build a complete hypervisor above it.
...
impl<'a> CursorMut<'a> {
    ...
    // Query, navigation, map, and unmap interfaces are the same as `vm_space::CursorMut`.
    // ...
}
```
...
`GuestContext` is the per-vCPU state object. It stores the guest-visible register state, the vCPU run state, and the hardware virtualization state owned by OSTD.
...
///
/// This type represents the guest CPU's general-purpose registers.
...
pub struct GuestExitInfo {
    ...
    /// Guest physical address associated with the exit.
...
Safe and efficient VMX and VMCS lifecycle management can be implemented under the `GuestMode` and `GuestContext` API design. Following [Linux's state-tracking approach](https://elixir.bootlin.com/linux/v7.1.4/source/arch/x86/kvm/vmx/vmx.c#L586), OSTD can map these APIs to VMX operations as follows.

`GuestMode::new` creates an internal VMX lifecycle guard that is privately owned by the returned `GuestMode`. OSTD maintains a global reference count of live guards. Access to this count is protected by a lock, which serializes guard construction and destruction. When the first guard is created, OSTD executes `VMXON` on every CPU. When the final guard is dropped, OSTD performs coordinated teardown on every CPU: it first executes `VMCLEAR` for every VMCS region that is still active on that CPU, and then executes `VMXOFF`.

`GuestContext::new` allocates a VMCS region, which is privately owned by the returned `GuestContext`. OSTD uses statically allocated per-CPU data to record the current VMCS region and the set of active VMCS regions on each CPU. Each `GuestContext` also tracks the CPU on which its VMCS region was most recently loaded and whether its launch state is clear or launched.

`GuestMode::execute` disables preemption while operating on the per-CPU VMX state, thereby pinning the execution to the current CPU. The exclusive `&mut GuestContext` argument serializes operations on the corresponding VMCS region, so the same VMCS region cannot be migrated or entered concurrently from two CPUs.

Let `A` be the VMCS region owned by the supplied `GuestContext`, and let `C` be the current CPU:

1. If `A` is already active and current on `C`, OSTD proceeds directly to VM entry without executing `VMPTRLD` or `VMCLEAR`.
2. If `A` is active on `C` but is not current, OSTD executes only `VMPTRLD(A)`. It preserves the existing launch state and does not execute `VMCLEAR`.
3. If `A` is active on another CPU, OSTD sends a synchronous IPI to that CPU to execute `VMCLEAR(A)`. The old CPU then removes `A` from its active set and clears its current-VMCS record if necessary. After the IPI completes, OSTD executes `VMPTRLD(A)` on `C` and adds `A` to the active set of `C`.
4. If `A` is not active on any CPU, OSTD first executes `VMCLEAR(A)` to establish a well-defined clear state, then executes `VMPTRLD(A)` on `C` and adds `A` to the active set of `C`.

OSTD chooses `VMLAUNCH` or `VMRESUME` according to the recorded launch state. After a VM exit, `GuestMode::execute` leaves the VMCS region active and does not execute `VMCLEAR` before returning. Therefore, subsequent executions on the same CPU can reuse the current or active VMCS region and benefit from the CPU's VMCS cache.

When a `GuestContext` is dropped, OSTD first ensures that its VMCS region is no longer active. If necessary, it synchronously executes `VMCLEAR` on the CPU where the region is active before deallocating it. Similarly, when the final VMX lifecycle guard is dropped, every CPU clears all VMCS regions in its local active set before executing `VMXOFF`.

All VMX instructions and lifecycle state remain internal to OSTD. The kernel manages guest policy through the safe `GuestMode` and `GuestContext` APIs without directly accessing VMCS residency state or issuing VMX instructions.

### Building a Hypervisor With the OSTD APIs

The kernel can build a complete hypervisor by composing the OSTD objects above with kernel-owned policy and device state. A minimal pseudo-code flow looks like this:
...
fn create_and_run_guest(image: GuestImage, userspace_mem: UserRange) -> Result<()> {
    ...
    let guest_end = image.ram_gpa.checked_add(userspace_mem.len)
        .ok_or(Error::new(Errno::EOVERFLOW))?;
    let guest_range = image.ram_gpa..guest_end;
    {
    ...
    // Create a vCPU context and let the kernel set the guest boot state.
...
Although the examples above are based on Intel CPUs, the current API split should also support AMD CPUs and the ARM and RISC-V architectures.
...
`GuestContext` and `GuestExitInfo` are intentionally architecture-specific. It is both reasonable and necessary to provide one `GuestContext` API and one `GuestExitInfo` type per architecture and, where necessary, per hardware virtualization backend, since register state, system registers, exit reasons, and exit metadata are fundamentally different across x86, ARM, and RISC-V.

The hypervisor implementation model exposed by the OSTD APIs is reusable for ARM and RISC-V. Both architectures provide hardware-assisted second-stage translation. `GuestPhysMemSpace` is exactly the abstraction for that second-stage page table: it provides generic mapping, unmapping, and query operations, while the internal implementation can use different `PageTableConfig`s for Intel x86 [EPT](https://www.intel.com/content/www/us/en/developer/articles/technical/intel-sdm.html "Vol. 3C. 30.3 EPT"), AMD x86 [nested paging](https://docs.amd.com/v/u/en-US/24593_3.44_APM_Vol2 "15.25 Nested Paging"), ARM [stage-2 translation](https://developer.arm.com/documentation/102142/0100/Stage-2-translation), or [RISC-V G-stage translation](https://docs.riscv.org/reference/isa/v20260120/priv/hypervisor.html#two-stage-translation).
...
* Is the current OSTD hypervisor interface stable enough for future acceleration features such as vAPIC acceleration? If those features require exposing additional architecture-specific controls, interrupt virtualization state, or APIC backing pages, the `GuestContext`, `GuestMode`, and `GuestPhysMemSpace` boundaries may need to evolve.

## Prior Art and References

* [Asterinas OSTD soundness analysis](../ostd/soundness/README.md)
* [Linux KVM API documentation](https://www.kernel.org/doc/html/v7.1/virt/kvm/api.html)
