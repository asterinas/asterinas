...
For example, this is the page table after `mmap(offset=0x3fe000, size=0x604000)`. A new $2 \text{MiB} \times 3 + 4 \text{MiB} \times 4$ `VmMapping` is split into 3 pieces ([x] [y] and [z]) and stored across 2 levels:
...
According to the CortenMM paper, an operation starts with a cursor that locks the given range. However, non-fixed `mmap`/`mremap` does not know the range yet, so it must find a free range first. We introduce a scalable address-space allocator that partitions the entire address space per CPU to allow scalable allocation of free ranges. After allocation, non-fixed `mmap`/`mremap` uses the cursor to lock the allocated range and insert the mapping metadata.

Fixed `mmap`/`mremap` calls need to overwrite existing address space ranges. We use the allocator as the synchronization point instead of the cursor.
...
**Drawbacks:**
 ...
 - Imprecise `RLIMIT_AS`. To make `mmap` scalable, the total size of virtual address mappings are implemented with per-CPU counters. So the user program may not immediately abort if it exceeds the `RLIMIT_AS` limit.
