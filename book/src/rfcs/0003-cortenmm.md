...
Furthermore, the cursor user must be aware of the level of the cursor, rather than only the virtual address. Therefore new APIs to get and adjust the current page table level are added to the cursor.

## Sleeping page table locks

Currently the kernel cannot sleep while a cursor is holding page table locks. This is because the OSTD implementation currently uses RCU to reason about the safety of accessing page tables. When operations (e.g., updating reverse mappings, doing `madvise`, or invoking the driver) need to break atomic mode, we have to drop the locks and retry, which does not guarantee stable mappings and may lead to inconsistent behavior. We are doing such retries upon faulting the file-backed mappings.

Furthermore, after merging the page table locks and the address space locks, the page table operations are heavier than before, which also violates the "use spin locks for short critical sections" principle.

Nevertheless, the non-sleeping requirement for page table locks is not fundamental: we can change the locks to sleepable mutexes. Only the lock protocol's traversal phase is RCU-critical. After locking the root of the child page table, we ensure that the subtree is alive and the RCU critical section should end. Scheduling can happen afterward.

## Reverse mappings

Although current Asterinas lacks reverse mappings, CortenMM must not preclude reverse mapping support. File-backed reverse mappings are needed for protecting mappings as read-only when writing back pages; anonymous reverse mappings are needed for swapping. Some device drivers (e.g., GPU) also demand reverse mappings for device-backed memory pages.

### File/device-backed reverse mappings

Like Linux's `i_mmap` tree in each `inode`, Asterinas can keep all the mapping address spaces in the `Vmo` object (protected by a per-`Vmo` reverse mapping lock). Each `Vmo` maintains a `BTreeMap` that stores the virtual offset and the range in each mapping memory space.

The challenge of adopting this reverse mapping design in CortenMM is the lock ordering: operations like `mremap` need to traverse the page table to get all the reverse mapping locks; operations like `fsync` need to traverse the reverse mapping set to get all the page tables.

The solution is to retry. We keep the forward lock ordering: to acquire the reverse mapping lock, the thread must first acquire the page table lock. When doing the reverse operations, if the page table lock cannot be acquired with `try_lock`, the thread must drop the reverse mapping lock and retry the traversal of reverse mappings.

File-backed and device-backed mappings share similar solutions. If there's a driver that needs to acquire its locks, it does after the page table locks, before the reverse-mappings locks.

### Anonymous reverse mappings

Anonymous reverse mappings, unlike file-backed reverse mappings, have different problems and need different solutions. Each physical page in the anonymous mapping can be easily shared by `fork` and unshared by copy-on-write independently. Therefore, each physical page can have a distinct set of virtual memory spaces that map to it. To address this, Linux links each physical page to one `anon_vma`, and all the VMAs in the system have a bipartite graph relationship with all the `anon_vma`s. Reverse mapping is done by traversing this bipartite graph.

Instead, CortenMM's page table metadata is owned solely by a page table. Sharing it would cause complications. As a trade-off, CortenMM links each `anon_vma`-like object with all the memory spaces instead of page table metadata ranges. This approach still maintains `O(1)` page fault and `fork` overhead. The downside is that when doing reverse mapping traversal, there is a chance of iterating all the irrelevant memory spaces before finding the actual ones. This is because, unlike Linux, upon `munmap` of an entire VMA, the affected `anon_vma` cannot be unlinked from parent memory spaces (it can only be unlinked in address-space-wide unmapping like `exec`), resulting in a larger number of memory spaces to search.
...
**Unresolved Questions / follow-ups:**
 ...
 - Implementing efficient anonymous reverse mappings without sacrificing scalability is difficult. Since Asterinas does not support swapping at the moment, we leave efficient anonymous reverse mappings in CortenMM as future work.
