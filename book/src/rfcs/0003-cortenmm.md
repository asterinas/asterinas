...
**Drawbacks:**
 ...
 - The page-table locks rely on RCU, so we cannot sleep while holding them. When operations (e.g., in `madvise` or driver code) need to break the atomic mode, we may have to drop the locks and retry, which does not guarantee stable mappings and may lead to inconsistent behavior. We could synchronize on the `VmMappings` instead, but the performance overhead is unknown.
