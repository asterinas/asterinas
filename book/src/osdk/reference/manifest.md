...
with_daemons = [{ path = "path/to/daemon", args = [] }] # <21>
...
9. The boot method.
    ...
    Optional. The default value is `vmm-direct`.

    Possible values are `vmm-direct`, `grub-rescue-iso`, and `grub-qcow2`.
...
21. The `with_daemons` host processes to run for the duration of QEMU.
    ...
    If a daemon exits unsuccessfully while QEMU is running,
    OSDK terminates QEMU and reports an execution error.
    A daemon that exits successfully is reaped and does not abort the run.
    During shutdown, OSDK sends `SIGTERM` to each daemon's process group
    and sends `SIGKILL` after five seconds if it is still running.
    OSDK waits one second after starting each daemon before continuing QEMU setup.
    A managed daemon must remain in the foreground for its entire lifetime;
    it must not fork into the background after startup.
...
x86 vs. RISC-V). Schemes support any user-defined keys (see the 23rd
