...
// Get time from predefined clock
clock_gettime(clockid = <predefined_clockid>, tp);

// Get time from dynamic clock
...
```c
opt_clockid = CLOCK_REALTIME | CLOCK_MONOTONIC | CLOCK_BOOTTIME | CLOCK_PROCESS_CPUTIME_ID;

// Sleep for a relative duration using specified clock
clock_nanosleep(
    clockid = <opt_clockid>,
    flags = 0,
    t, remain
...
// Sleep until an absolute time point using specified clock
clock_nanosleep(
    clockid = <opt_clockid>,
    flags = TIMER_ABSTIME,
    t, remain
...
```c
// Obtain random bytes from urandom pool
getrandom(
    buf, buflen,
    flags = 0
);

// Obtain random bytes from high-entropy pool
getrandom(
    buf, buflen,
    flags = GRND_RANDOM
);
