...
The following command builds and runs the test binaries in `test/initramfs/src/custom` directory on Asterinas. Those testcases are designed to verify specific functionality, edge cases, or custom scenarios that may not be covered by other test suites.
