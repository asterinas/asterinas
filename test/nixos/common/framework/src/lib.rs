...
//! A declarative testing framework for NixOS-based tests.
//!
//! # Core Concepts
//!
//! - **Test Registration**: Use `#[nixos_test]` to register test functions.
//! - **Session**: A stateful PTY session with support for nested environments.
//! - **SessionDesc**: Configuration for session prompts and lifecycle commands.
//!
...
pub fn __nixos_test_main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for test_case in test_cases {
        ...
        match session.run(test_case.test_fn) {
            ...
            Err(_) => {
                eprintln!("✗ Test case 'test_{}' failed\n", test_case.name);
                failed += 1;
