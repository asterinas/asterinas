...
This directory contains integration tests of Asterinas running on NixOS and a framework for writing such tests. The framework automatically launches a QEMU virtual machine running Asterinas NixOS and interacts with it through a PTY (pseudo-terminal) session to verify system-level functionality. It provides a simple, declarative API that makes it easy to write and maintain complex interactive tests.
...
test/nixos/
├── common/
│   ├── framework/              # Core testing library
│   │   ├── Cargo.toml
│   │   └── src/
│   │       ├── lib.rs
│   │       └── session.rs
│   ├── test_macro/             # Procedural macro for test registration
│   │   ├── Cargo.toml
│   │   └── src/
│   │       └── lib.rs
...
│   │   └── src/
│   │       └── main.rs
│   └── merge_nixos_config.sh   # Helper script for merging NixOS configurations
├── tests/
...
```
This content of this file will be merged with the [default configuration file](../../distro/etc_nixos/configuration.nix) to generate the final configuration file, i.e., `configuration.nix`, for the testing Asterinas NixOS system.

## Running Tests

It is recommended to run commands from the **Asterinas root directory**.
...
```bash
# Quick test (build + run in one step)
