...
#[nixos_test]
fn grafana_server(nixos_shell: &mut Session) -> Result<(), Error> {
    // Get Grafana home path
    nixos_shell
        .run_cmd("export GRAFANA_HOME=$(dirname $(dirname $(realpath $(which grafana))))")?;

    // Start Grafana server
    nixos_shell.run_cmd("grafana server --homepath=$GRAFANA_HOME/share/grafana --config=/tmp/grafana.ini > /tmp/grafana.log 2>&1 &")?;
    nixos_shell.run_cmd("sleep 30")?;

    // Check if Grafana is responding
    nixos_shell.run_cmd_and_expect("curl -s http://10.0.2.15:3000/api/health", "ok")?;

    // Check frontend settings API
    nixos_shell.run_cmd_and_expect(
        "curl -s http://10.0.2.15:3000/api/frontend/settings",
        "statusCode",
    )?;

    // Stop Grafana server
    nixos_shell.run_cmd("pkill grafana")?;
    Ok(())
}
