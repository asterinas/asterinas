...
#define LISTEN_PORT htons(0x1241)
int option;
...
FN_TEST(nagle)
{
	...
	// 1. Disable Nagle algorithm on unbound socket
	...
	// 2. Disable Nagle algorithm on connected socket
	...
	// 3. Disable Nagle algorithm on listening socket before connection
	...
	// 4. Disable Nagle algorithm on listening socket after connection
	option = 0;
	CHECK(setsockopt(sk_listen, IPPROTO_TCP, TCP_NODELAY, &option,
			 sizeof(option)));
...
FN_TEST(keepalive)
{
	...
	// 1. Enable keepalive on unbound socket
	...
	// 2. Enable keepalive on connected socket
	...
	// 3. Enable keepalive on listening socket before connection
	...
	// 4. Disable keepalive on listening socket after connection
	option = 0;
	CHECK(setsockopt(sk_listen, SOL_SOCKET, SO_KEEPALIVE, &option,
			 sizeof(option)));
	...
	TEST_RES(getsockopt(sk_connected, IPPROTO_TCP, TCP_NODELAY, &keepalive,
			    &keepalive_len),
		 keepalive == 0);
	TEST_RES(getsockopt(sk_accepted, IPPROTO_TCP, TCP_NODELAY, &keepalive,
			    &keepalive_len),
		 keepalive == 1);
}
END_TEST()
