...
FN_TEST(shutdown_shutdown)
{
	...
	TEST_ERRNO(shutdown(sk_connect, SHUT_RDWR), ENOTCONN);
	TEST_ERRNO(shutdown(sk_accept, SHUT_RDWR), ENOTCONN);
...
#define EV_NO_ERR (POLLIN | POLLOUT | POLLHUP)
	...
	TEST_RES(recv(sk_connect, buf, 3, 0),
		 _ret == 3 && memcmp(buf, "hel", 3) == 0);
	TEST_RES(recv(sk_connect, buf, sizeof(buf), 0),
		 _ret == 3 && memcmp(buf, "lo", 3) == 0);
