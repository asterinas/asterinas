...
FN_TEST(if_nameindex)
{
	...
	int find_lo_and_eth0(struct if_nameindex * if_ni)
	{
		int found_links = 0;
	...
	TEST_RES(find_lo_and_eth0(if_ni), _ret == 2);
...
FN_TEST(get_lo_and_eth0_link)
{
	...
	int found_links = 0;
	TEST_RES(SUCC(nl_cache_foreach(link_cache, find_lo_and_eth0,
				       &found_links)),
		 found_links == 2);
...
#define BUFFER_SIZE 8192
	...
	TEST_SUCC(sendmsg(sock_fd, &msg, 0));
	TEST_RES(recv(sock_fd, buffer, BUFFER_SIZE, 0),
		 ((struct nlmsghdr *)buffer)->nlmsg_type == RTM_NEWADDR);

	// 3. Invalid required family
	req.ifa.ifa_family = 253;
	TEST_SUCC(sendmsg(sock_fd, &msg, 0));
	TEST_RES(recv(sock_fd, buffer, BUFFER_SIZE, 0),
		 ((struct nlmsghdr *)buffer)->nlmsg_type == RTM_NEWADDR);
}
