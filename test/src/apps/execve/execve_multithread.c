...
char sig_alt_stack[8192];
char child_stack[8192];

int child_process(void *_info)
{
	struct sigaction sa;
	CHECK_WITH(sigaction(SIGIO, NULL, &sa), sa.sa_handler == SIG_IGN);
	CHECK_WITH(sigaction(SIGINT, NULL, &sa), sa.sa_handler == SIG_DFL);
	sa.sa_handler = SIG_IGN;
	CHECK(sigaction(SIGINT, &sa, NULL));
	exit(EXIT_SUCCESS);
}

int child_exec_process(void *child_exit_code)
{
	int *exit_code = (int *)child_exit_code;
	write_stat(*exit_code, 0);
	struct sigaction sa;
	CHECK_WITH(sigaction(SIGIO, NULL, &sa), sa.sa_handler == SIG_IGN);
	CHECK_WITH(sigaction(SIGINT, NULL, &sa), sa.sa_handler == SIG_IGN);
	exec_child();
	exit(EXIT_FAILURE);
}

FN_TEST(clone_sighand)
{
	struct sigaction sa;
	TEST_RES(sigaction(SIGIO, NULL, &sa), sa.sa_handler == SIG_DFL);

	sa.sa_handler = SIG_IGN;
	TEST_SUCC(sigaction(SIGIO, &sa, NULL));

	stack_t stack;
	stack.ss_sp = sig_alt_stack;
	stack.ss_size = sizeof(sig_alt_stack);
	stack.ss_flags = 0;

	TEST_SUCC(sigaltstack(&stack, NULL));

	pid_t pid = TEST_SUCC(clone(child_process,
	...
	TEST_RES(wait4(pid, &status, 0, NULL),
		 _ret == pid && WIFEXITED(status) && WEXITSTATUS(status) == 0);
	TEST_RES(sigaction(SIGINT, NULL, &sa), sa.sa_handler == SIG_IGN);

	int child_exit_code = 103;
	pid = TEST_SUCC(clone(child_process, child_stack + sizeof(child_stack),
			      CLONE_VM | CLONE_SIGHAND | SIGCHLD,
			      (void *)&child_exit_code));

	TEST_RES(wait4(pid, &status, 0, NULL),
		 _ret == pid && WIFEXITED(status) && WEXITSTATUS(status) == 0);
	TEST_RES(sigaction(SIGIO, NULL, &sa), sa.sa_handler == SIG_IGN);
	TEST_RES(sigaction(SIGINT, NULL, &sa), sa.sa_handler == SIG_IGN);
}
END_TEST()