...
FN_SETUP(create_and_bind)
{
	...
	// CHECK(inet_aton(BROADCAST_ADDR, &sk_addr.sin_addr));
	// sk_addr.sin_port = htons(RECEIVE_PORT);
	// CHECK(bind(receiver, (struct sockaddr *)&sk_addr, addr_len));
}
END_SETUP()
...
{
	TEST_SUCC(sendto(sender, MESSAGE, strlen(MESSAGE), 0,
...
FN_TEST(disable_sender_broadcast)
{
	...
	TEST_ERRNO(sendto(sender, MESSAGE, strlen(MESSAGE), 0,
			  (struct sockaddr *)&broadcast_addr, addr_len),
		   EACCES);
	...
	TEST_ERRNO(connect(sender, (struct sockaddr *)&broadcast_addr,
			   addr_len),
		   EACCES);
	TEST_ERRNO(send(sender, MESSAGE, strlen(MESSAGE), 0), EDESTADDRREQ);
}
END_TEST()
...
FN_TEST(connect_then_disable_broadcast)
{
	...
	TEST_SUCC(send(sender, MESSAGE, strlen(MESSAGE), 0));
	// TEST_SUCC(recvfrom(receiver, buf, sizeof(buf), 0,
	// 		   (struct sockaddr *)&received_addr, &addr_len));
	...
	// TEST_ERRNO(send(sender, MESSAGE, strlen(MESSAGE), 0), EACCES);
}
END_TEST()
