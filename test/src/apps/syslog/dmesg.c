...
void format_timestamp(const char* timestamp_str, char* output, int human_readable) {
    if (!timestamp_str || !output) return;
...
void process_message(const char* msg, int show_time, int human_time, int decode_level, 
                     int min_level, int raw_output) {
    ...
    if (content[0] == '[') {
        ...
        if (end_bracket) {
            ...
            if (ts_len > 0 && ts_len < sizeof(timestamp) - 1) {
                ...
                if (message_text[0] == ' ') message_text++;
            }
        }
    }
    ...
    if (strlen(message_text) > 0 && message_text[strlen(message_text) - 1] != '\n') {
        printf("\n");
    }
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-c") == 0 || strcmp(argv[i], "--clear") == 0) {
            clear_buffer = 1;
        } else if (strcmp(argv[i], "-T") == 0 || strcmp(argv[i], "--notime") == 0) {
            show_time = 0;
        } else if (strcmp(argv[i], "-t") == 0 || strcmp(argv[i], "--ctime") == 0) {
            human_time = 1;
        } else if (strcmp(argv[i], "-x") == 0 || strcmp(argv[i], "--decode") == 0) {
            decode_level = 1;
            ...
            if (i + 1 < argc) {
                ...
                if (min_level < 0 || min_level > 7) {
                    fprintf(stderr, "Invalid log level: %d (must be 0-7)\n", min_level);
                    return 1;
                }
            } else {
                fprintf(stderr, "Option -l requires a level argument\n");
                return 1;
            }
    ...
    if (buffer_size == 0) {
        long size = syscall(SYS_syslog, SYSLOG_ACTION_SIZE_BUFFER, NULL, 0);
        if (size <= 0) {
    ...
    if (bytes_read > 0) {
        ...
        if (raw_output) {
            ...
            while (line != NULL) {
                process_message(line, show_time, human_time, decode_level, min_level, raw_output);
                line = strtok(NULL, "\n");
            }
        }
    }
