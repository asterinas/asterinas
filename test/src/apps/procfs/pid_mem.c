...
FN_TEST(proc_mem_remote)
{
	...
	if (child == 0) {
		...
		int fd = TEST_SUCC(open(FILE_NAME, O_RDONLY));
		void *addr =
		...
		TEST_SUCC(read(fd, filebuf, sizeof(filebuf)));
		TEST_RES(strncmp(filebuf, ORIG_STR, strlen(ORIG_STR)),
			 _ret == 0);
...
FN_TEST(proc_mem_local)
{
	...
	void *addr1 =
		CHECK_WITH(mmap(NULL, PAGE_SIZE, PROT_READ, MAP_PRIVATE, fd, 0),
			   _ret != MAP_FAILED);
	void *addr2 = CHECK_WITH(mmap(NULL, PAGE_SIZE, PROT_READ | PROT_WRITE,
