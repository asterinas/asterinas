...
FN_TEST(pseudo_fs_dev_id_unique_per_mount)
{
	...
	for (int i = 0; i < 6; i++) {
		for (int j = i + 1; j < 6; j++) {
			TEST_RES(0, !dev_id_equal(&dev_ids[i], &dev_ids[j]));
		}
	}
}
END_TEST()
