...
FN_TEST(shared_slave_propagation)
{
	...
	free(stack);
	TEST_SUCC(close(parent_to_child[0]));
	TEST_SUCC(close(parent_to_child[1]));
	TEST_SUCC(close(child_to_parent[0]));
	TEST_SUCC(close(child_to_parent[1]));
...
FN_TEST(propagation_preserves_existing_mounts)
{
	TEST_SUCC(isolate_mount_namespace());
	...
	TEST_SUCC(mount("lower", WORK_DIR "/slave/event", "tmpfs", 0, NULL));
	TEST_SUCC(mkdir(WORK_DIR "/slave/event/lower_marker", 0755));
	TEST_SUCC(mount(NULL, WORK_DIR "/slave/event", NULL, MS_SHARED, NULL));
	TEST_SUCC(mount("upper", WORK_DIR "/slave/event", "tmpfs", 0, NULL));
	TEST_SUCC(mkdir(WORK_DIR "/slave/event/upper_marker", 0755));
	TEST_ERRNO(access(WORK_DIR "/master/event/lower_marker", F_OK), ENOENT);
	TEST_ERRNO(access(WORK_DIR "/master/event/upper_marker", F_OK), ENOENT);

	TEST_SUCC(mount("event", WORK_DIR "/master/event", "tmpfs", 0, NULL));
	TEST_SUCC(mkdir(WORK_DIR "/master/event/propagated_marker", 0755));
	TEST_ERRNO(access(WORK_DIR "/slave/event/propagated_marker", F_OK),
		   ENOENT);
	TEST_SUCC(access(WORK_DIR "/slave/event/upper_marker", F_OK));
	TEST_ERRNO(access(WORK_DIR "/slave/event/lower_marker", F_OK), ENOENT);

	TEST_SUCC(umount(WORK_DIR "/slave/event"));
	TEST_SUCC(access(WORK_DIR "/slave/event/lower_marker", F_OK));
	TEST_SUCC(access(WORK_DIR "/master/event/propagated_marker", F_OK));
	TEST_SUCC(umount(WORK_DIR "/slave/event"));
	TEST_SUCC(access(WORK_DIR "/slave/event/propagated_marker", F_OK));
	TEST_SUCC(umount(WORK_DIR "/slave/event"));
	TEST_SUCC(umount(WORK_DIR "/master/event"));
