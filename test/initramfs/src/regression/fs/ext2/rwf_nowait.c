...
FN_TEST(write_buffered_nowait_einval)
{
...
FN_TEST(read_partial_cache_nowait_short_read)
{
	...
	// RWF_NOWAIT returns a short read (Linux filemap_read semantics) when
	// only part of the range is cached, instead of EAGAIN.
...
FN_TEST(write_dio_cached_pages_nowait_eagain)
{
	...
	// Any cached page in the range reports EAGAIN, even a clean one, so a
	// blocking direct write (rather than fsync) clears the cache.
