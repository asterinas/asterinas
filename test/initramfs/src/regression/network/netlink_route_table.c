...
#define ANY_GATEWAY .gateway = NULL, .gateway_size = SIZE_MAX
#define ANY_OIF UINT32_MAX
				...
				const struct route_spec *spec)
{
	...
	CHECK_WITH(sendmsg(sock_fd, &msg, 0),
		   _ret == (ssize_t)req->hdr.nlmsg_len);
	CHECK(recv_until_done_or_ack(sock_fd, req->hdr.nlmsg_seq, 1, spec,
				     &found_route, &done));
...
FN_TEST(get_route_dump_bootstrap)
{
	...
	uint32_t loopback_addr = ipv4_addr(127, 0, 0, 1);
	...
	struct route_spec loopback_addr_local = {
		IPV4_SPEC_ADDR(loopback_addr),
		.dst_len = 32,
		NO_GATEWAY,
		.oif = lo_index,
		.table = RT_TABLE_LOCAL,
	...
	req.rtmsg.rtm_scope = RT_SCOPE_UNIVERSE;
	TEST_RES(route_request_absent(sock_fd, &req, &loopback_connected),
		 _ret == 0);
	TEST_RES(route_request_success(sock_fd, &req, &loopback_addr_local),
		 _ret == 0);
	TEST_RES(route_request_success(sock_fd, &req, &loopback_local),
		 _ret == 0);
	TEST_RES(route_request_absent(sock_fd, &req, &limited_broadcast),
		 _ret == 0);
	...
	TEST_RES(route_request_success(sock_fd, &req, &loopback_local),
		 _ret == 0);
	TEST_RES(route_request_absent(sock_fd, &req, &loopback_ipv6_connected),
		 _ret == 0);
...
FN_TEST(route_dump_ignores_selectors_without_strict_checking)
{
	int sock_fd;
	uint32_t lo_index = iface_index_by_name(LOOPBACK_NAME);
	uint32_t loopback_addr = ipv4_addr(127, 0, 0, 1);
	uint32_t table = RT_TABLE_DEFAULT;
	uint32_t oif = UINT32_MAX;
	struct route_request req;
	struct route_spec loopback_addr_local = {
		IPV4_SPEC_ADDR(loopback_addr),
		.dst_len = 32,
	...
	sock_fd = TEST_SUCC(socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE));
	init_route_request(&req, RTM_GETROUTE, NLM_F_DUMP, 14);
	req.rtmsg.rtm_dst_len = 1;
	req.rtmsg.rtm_src_len = 1;
	req.rtmsg.rtm_tos = 1;
	req.rtmsg.rtm_table = RT_TABLE_DEFAULT;
	req.rtmsg.rtm_protocol = RTPROT_BOOT;
	req.rtmsg.rtm_scope = RT_SCOPE_LINK;
	req.rtmsg.rtm_type = RTN_BROADCAST;
	req.rtmsg.rtm_flags = RTM_F_CLONED;
	add_rtattr(&req.hdr, sizeof(req), RTA_TABLE, &table, sizeof(table));
	add_rtattr(&req.hdr, sizeof(req), RTA_OIF, &oif, sizeof(oif));
	TEST_RES(route_request_success(sock_fd, &req, &loopback_addr_local),
		 _ret == 0);
...
FN_TEST(route_lookup)
{
	...
	struct route_request req;
	uint32_t loopback_addr = ipv4_addr(127, 0, 0, 1);
	uint32_t default_addr = ipv4_addr(8, 8, 8, 8);
	uint32_t limited_broadcast_addr = ipv4_addr(255, 255, 255, 255);
	...
	struct route_spec lookup_route = {
		...
		.flags = RTM_F_CLONED | RTCF_LOCAL,
		.protocol = RTPROT_UNSPEC,
		.scope = RT_SCOPE_UNIVERSE,
		.type = RTN_LOCAL,
	};
	struct route_spec fibmatch_route = {
		IPV4_SPEC_ADDR(IPV4_ZERO),
		.dst_len = 0,
		ANY_GATEWAY,
		.oif = ANY_OIF,
		.table = RT_TABLE_MAIN,
	...
	struct route_spec limited_broadcast_lookup = {
		IPV4_SPEC_ADDR(limited_broadcast_addr),
		.dst_len = 32,
		NO_GATEWAY,
		.oif = ANY_OIF,
		.table = RT_TABLE_UNSPEC,
		.flags = RTM_F_CLONED | RTCF_LOCAL | RTCF_BROADCAST,
		.protocol = RTPROT_UNSPEC,
		.scope = RT_SCOPE_UNIVERSE,
		.type = RTN_BROADCAST,
	};
	...
	add_rtattr(&req.hdr, sizeof(req), RTA_DST, &loopback_addr,
	...
	TEST_RES(route_lookup_prefsrc(sock_fd, &req, &prefsrc, sizeof(prefsrc)),
		 _ret == 0 && prefsrc == loopback_addr);

	init_route_request(&req, RTM_GETROUTE, 0, 51);
	req.rtmsg.rtm_dst_len = 32;
	req.rtmsg.rtm_flags = RTM_F_LOOKUP_TABLE;
	add_rtattr(&req.hdr, sizeof(req), RTA_DST, &default_addr,
		   sizeof(default_addr));
	TEST_RES(route_lookup_table_info(sock_fd, &req, &header_table,
					 &attr_table_present, &attr_table),
		 _ret == 0 && header_table == RT_TABLE_MAIN &&
	...
	init_route_request(&req, RTM_GETROUTE, 0, 53);
	req.rtmsg.rtm_dst_len = 32;
	add_rtattr(&req.hdr, sizeof(req), RTA_DST, &limited_broadcast_addr,
		   sizeof(limited_broadcast_addr));
	TEST_RES(route_request_success(sock_fd, &req,
				       &limited_broadcast_lookup),
		 _ret == 0);
