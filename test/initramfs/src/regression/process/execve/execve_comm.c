...
FN_TEST(execveat_empty_path)
{
	...
	char child_path[256];
	get_child_path(child_path, sizeof(child_path));
	child_fd = TEST_SUCC(open(child_path, O_PATH));
#ifdef __asterinas__
	TEST_RES(snprintf(child_comm_env, sizeof(child_comm_env), "%s",
			  CHILD_COMM_ENV),
		 _ret < (int)sizeof(child_comm_env));
#else
	TEST_RES(snprintf(child_comm_env, sizeof(child_comm_env),
			  "EXPECTED_COMM=%d", child_fd),
		 _ret < (int)sizeof(child_comm_env));
#endif
	TEST_RES(snprintf(watch_path, sizeof(watch_path), "%s", child_path),
		 _ret < (int)sizeof(watch_path));
	split_dirname(watch_path, &child_dir, &child_name);
	inotify_fd = TEST_SUCC(inotify_init1(0));
	wd = TEST_SUCC(inotify_add_watch(inotify_fd, child_dir,
					 IN_OPEN | IN_CLOSE_NOWRITE));

	pid = TEST_SUCC(fork());
	if (pid == 0) {
		CHECK(execveat(child_fd, "", argv, envp, AT_EMPTY_PATH));
	}
