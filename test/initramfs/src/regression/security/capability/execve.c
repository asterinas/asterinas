...
FN_TEST(file_caps_v1_gain_effective_caps)
{
...
FN_TEST(file_caps_cleared_after_shared_writable_mmap)
{
	const struct ast_vfs_cap_data_v2 file_caps = {
		.magic_etc = AST_VFS_CAP_REVISION_2 |
			     AST_VFS_CAP_FLAGS_EFFECTIVE,
	...
	char *exec_path =
		create_exec_with_file_caps(&file_caps, sizeof(file_caps));
	int fd = TEST_SUCC(open(exec_path, O_RDWR));
	void *mapping = TEST_RES(mmap(NULL, 4096, PROT_READ | PROT_WRITE,
				      MAP_SHARED, fd, 0),
				 _ret != MAP_FAILED);
...
FN_TEST(file_caps_cleared_after_shared_mprotect_write)
{
	const struct ast_vfs_cap_data_v2 file_caps = {
		.magic_etc = AST_VFS_CAP_REVISION_2 |
			     AST_VFS_CAP_FLAGS_EFFECTIVE,
	...
	char *exec_path =
		create_exec_with_file_caps(&file_caps, sizeof(file_caps));
	int fd = TEST_SUCC(open(exec_path, O_RDWR));
	void *mapping = TEST_RES(mmap(NULL, 4096, PROT_READ, MAP_SHARED, fd, 0),
				 _ret != MAP_FAILED);

	check_file_caps_present(exec_path);
	TEST_SUCC(mprotect(mapping, 4096, PROT_READ | PROT_WRITE));
	check_file_caps_present(exec_path);
	((char *)mapping)[0] = '\x7f';
	check_file_caps_absent(exec_path);
