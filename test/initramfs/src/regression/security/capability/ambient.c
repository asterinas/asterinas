...
{
	// All capabilities should initially not be in the ambient set.
...
{
	// Lower the capability that was raised in the previous test.
...
FN_TEST(ambient_raise_ignores_bounding)
{
	...
	pid = TEST_SUCC(fork());
	if (pid == 0) {
		TEST_SUCC(add_inheritable(CAP_WAKE_ALARM));
		TEST_SUCC(prctl(PR_CAPBSET_DROP, CAP_WAKE_ALARM, 0, 0, 0));
...
FN_TEST(ambient_inherited_across_fork)
{
	...
	if (pid == 0) {
		...
		TEST_SUCC(prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_CLEAR_ALL, 0, 0,
				0));
		TEST_RES(prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_IS_SET,
			       CAP_NET_BIND_SERVICE, 0, 0),
			 _ret == 0);
...
FN_TEST(ambient_not_protected_by_keep_caps)
{
	...
	if (pid == 0) {
		...
		TEST_SUCC(prctl(PR_SET_KEEPCAPS, 1));
		TEST_SUCC(setuid(65534));
		TEST_RES(prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_IS_SET,
			       CAP_NET_BIND_SERVICE, 0, 0),
			 _ret == 0);
...
FN_TEST(ambient_raise_requires_permitted)
{
	...
	if (pid == 0) {
		...
		TEST_SUCC(add_inheritable(CAP_NET_RAW));
		...
		// the permitted set.
		TEST_ERRNO(prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_RAISE,
				 CAP_NET_RAW, 0, 0),
			   EPERM);
		_exit(__tests_failed ? 1 : 0);
	}
...
FN_TEST(ambient_procfs_reflects_state)
{
	...
	capamb = read_proc_capamb();
	TEST_RES(capamb & (1ULL << CAP_NET_BIND_SERVICE), _ret != 0);

	// Clear and verify it is gone from procfs.
	TEST_SUCC(prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_CLEAR_ALL, 0, 0, 0));
	capamb = read_proc_capamb();
	TEST_RES(capamb, _ret == 0);
