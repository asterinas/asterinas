{ lib, stdenvNoCC, callPackage, testSuite ? "ltp", workDir ? "/tmp", tests ? ""
, blocklistMode ? "auto", smp ? 1, }: rec {
  inherit testSuite;
  ...
  package = stdenvNoCC.mkDerivation {
    ...
    buildCommand = ''
      ...
      export CONFORMANCE_TESTS=${tests}
      export CONFORMANCE_BLOCKLIST_MODE=${blocklistMode}
