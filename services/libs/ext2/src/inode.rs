...
impl Ext2Inode {
    pub fn new(ino: u32, raw_inode: Dirty<RawInode>, fs: Arc<Ext2>) -> Self {
        Self {
            ino,
    ...
    pub fn file_type(&self) -> FileType {
        FileType::from_raw_mode(self.raw_inode.read().mode)
    }

    pub fn file_perm(&self) -> FilePerm {
        FilePerm::from_raw_mode(self.raw_inode.read().mode)
    }

    pub fn set_file_perm(&self, perm: FilePerm) {
        let mut raw_inode = self.raw_inode.write();
        let file_type = FileType::from_raw_mode(raw_inode.mode);
        raw_inode.mode = file_type as u16 | perm.bits();
    }

    pub fn uid(&self) -> u32 {
        let raw_inode = self.raw_inode.read();
        (raw_inode.os_dependent_2.uid_high as u32) << 16 | raw_inode.uid as u32
    }

    pub fn gid(&self) -> u32 {
        let raw_inode = self.raw_inode.read();
        (raw_inode.os_dependent_2.gid_high as u32) << 16 | raw_inode.gid as u32
    }

    pub fn file_size(&self) -> u64 {
        let raw_inode = self.raw_inode.read();
        if FileType::from_raw_mode(raw_inode.mode) == FileType::File {
            (raw_inode.size_high as u64) << 32 | raw_inode.size_low as u64
        } else {
            raw_inode.size_low as u64
        }
    }
    ...
    pub fn blocks_count(&self) -> u32 {
        self.raw_inode
            .read()
            .blocks_count
            .min((self.file_size() as usize).div_ceil(BLOCK_SIZE) as u32)
    }
...
impl Ext2Inode {
    ...
    pub fn create(
        ...
        if get_inode_ino(name, blocks).is_ok() {
        ...
        append_dir_entry(new_entry, blocks)?;
    ...
    pub fn lookup(&self, name: &str, blocks: &dyn MemStorage) -> Result<Arc<Self>> {
        if self.file_type() != FileType::Dir {
            return Err(Error::NotDir);
        }
        if name.len() > MAX_FNAME_LEN {
        ...
        let ino = get_inode_ino(name, blocks)?;
        self.fs.find_inode(ino)
    }

    pub fn write_link(&self, target: &str, blocks: &dyn MemStorage) -> Result<()> {
        ...
        if target.len() <= FAST_SYMLINK_MAX_LEN {
            ...
            blocks.write_bytes_at(0, target.as_bytes())?;
        }

        Ok(())
    }

    pub fn read_link(&self, blocks: &dyn MemStorage) -> Result<String> {
        if self.file_type() != FileType::Symlink {
            return Err(Error::IsDir);
        }
        ...
        let symlink = {
            ...
            if file_size <= FAST_SYMLINK_MAX_LEN {
                ...
                blocks.read_bytes_at(0, symlink.as_mut_slice())?;
            }
    ...
    pub fn resize(&self, len: usize) -> Result<()> {
        ...
        match file_type {
            FileType::Dir => {
                raw_inode.size_low = len as u32;
            }
            FileType::File | FileType::Symlink => {
                raw_inode.size_low = len as u32;
                raw_inode.size_high = (len >> 32) as u32;
            }
            _ => {
                todo!();
            }
    ...
    pub fn sync_metadata(&self) -> Result<()> {
        if self.raw_inode.read().is_dirty() {
            let mut raw_inode = self.raw_inode.write();
            self.fs.flush_inode(self.ino, &raw_inode)?;
            raw_inode.sync();
        }
        Ok(())
    }
    ...
    fn init_dir(&self, parent_ino: u32) -> Result<()> {
        ...
        let block = {
            ...
            let block = BioBuf::from_boxed_slice(buf.into_boxed_slice());
            self.read_block(bid, &block)?;
            block
        };

        let self_entry = DirEntry::new(self.ino, ".", FileType::Dir);
        append_dir_entry(self_entry, &block)?;
        let parent_entry = DirEntry::new(parent_ino, "..", FileType::Dir);
        append_dir_entry(parent_entry, &block)?;
