...
use self::test_utils::{PageCacheTest, VnodeTest};
...
lazy_static! {
    ...
    static ref ROOT_INODE: VnodeTest = {
        let root_inode = VnodeTest::new(EXT2FS.root_inode::<PageCacheTest>().unwrap());
...
fn test_symlink() {
    let root_inode = &ROOT_INODE;
    const TARGET_STR: &str = "/0123456789";
