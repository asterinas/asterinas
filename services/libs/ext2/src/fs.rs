...
impl Ext2 {
    ...
    pub(crate) fn find_inode(&self, ino: u32) -> Result<Arc<Ext2Inode>> {
    ...
    pub(crate) fn flush_inode(&self, ino: u32, raw_inode: &RawInode) -> Result<()> {
        let block_group = self.block_group_of_inode(ino)?;
        let inode_idx = self.inode_idx(ino);
        let inode_offset = self.inode_offset(block_group, inode_idx);
    ...
    pub fn sync_metadata(&self) -> Result<()> {
        ...
        for block_group in &self.block_groups {
            block_group.flush(self.block_device.as_ref(), &self.super_block.read())?;
        }
        Ok(())
    }

    pub fn block_group_idx_of_ino(&self, ino: u32) -> u32 {
