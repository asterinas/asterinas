...
impl Vmar_ {
    ...
    fn do_protect_inner(&self, perms: VmPerms, range: Range<usize>) -> Result<()> {
        let protect_mappings = self.get_range_mappings(range.clone());
        if protect_mappings.is_ok() {
            for vm_mapping in protect_mappings.unwrap() {
    ...
    /// Return (region base addr, child real offset).
    ...
    /// Get mapped vm_mappings at given range.
    /// TODO: improve the searching algorithm.
    fn get_range_mappings(&self, range: Range<usize>) -> Result<Vec<Arc<VmMapping>>> {
        let mut vm_mappings = Vec::new();
        for (vm_mapping_base, vm_mapping) in &self.inner.lock().vm_mappings {
            let vm_mapping_range = vm_mapping.range();
            if is_intersected(&vm_mapping_range, &range) {
                vm_mappings.push(vm_mapping.clone());
            }
        }
        if vm_mappings.is_empty() {
            return_errno_with_message!(Errno::EACCES, "No mapped vmo at this offset");
        }
        Ok(vm_mappings)
    }
