...
impl<T: Copy> Producer<T> {
    pub fn write(&self, buf: &[T]) -> Result<usize> {
        if self.is_shutdown() || self.peer_is_shutdown() {
...
impl<T> Drop for Producer<T> {
    fn drop(&mut self) {
        ...
        let mut revents = IoEvents::HUP;

        self.shutdown();

        self.0.common.lock_event();

        let rb = self.this_end().rb();
        if !rb.is_empty() {
            revents |= IoEvents::IN;
        }
