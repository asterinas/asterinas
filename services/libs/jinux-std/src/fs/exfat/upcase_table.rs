...
pub struct ExfatUpcaseTable<'a>{
    // mapping tabe
    upcase_table: [u16; UPCASE_MANDATORY_SIZE],
    fs:&'a ExfatFS
}

impl<'a> ExfatUpcaseTable <'a> {
    pub fn load_upcase_table(fs:&'a ExfatFS) -> Result<Self> {
    ...
    fn allocate_table(fs:&'a ExfatFS,dentry:&ExfatUpcaseDentry) -> Result<Self> {
        ...
        Self::vertify_checksum(&buf, dentry.checksum)?;
    ...
    fn vertify_checksum(data: &Vec<u8>, checksum: u32) -> Result<()> {
        let mut calc: u32 = 0;
        for byte in data {
