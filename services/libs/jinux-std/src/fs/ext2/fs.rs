...
use super::inode::{Ext2Inode, FilePerm, FileType, RawInode};
use super::prelude::*;
use super::super_block::{Ext2SuperBlock, SUPER_BLOCK_OFFSET};
...
impl Ext2 {
    /// Loads and returns an Ext2 fs from the block device.
    ...
    pub fn root_inode(&self) -> Result<Arc<Ext2Inode>> {
        self.lookup_inode(EXT2_ROOT_INO)
    }

    /// Finds the inode by inode number.
    pub(super) fn lookup_inode(&self, ino: u32) -> Result<Arc<Ext2Inode>> {
        ...
        // Load it from block device, then put into the cache of block group.
        let inode = {
            let inode_offset = self.inode_offset(block_group, inode_idx);
            let raw_inode = self.block_device.read_val::<RawInode>(inode_offset)?;
            Ext2Inode::new(ino, Dirty::new(raw_inode), self.self_ref.upgrade().unwrap())
        };
    ...
    /// Writes back the inode info.
    pub(super) fn flush_raw_inode(&self, ino: u32, raw_inode: &RawInode) -> Result<()> {
        let block_group = self.block_group_of_inode(ino)?;
        let inode_idx = self.inode_idx(ino);
        let inode_offset = self.inode_offset(block_group, inode_idx);
    ...
    /// Allocates a block starting from the block group `block_group_idx`.
    pub(super) fn alloc_block(&self, block_group_idx: u32) -> Result<BlockId> {
        ...
        if self.super_block.read().is_dirty() {
            ...
            for idx in 1..super_block.block_groups_count() {
                if super_block.is_backup_block_group(idx) {
                    ...
                    let offset = BlockId::new(idx * super_block.blocks_per_group).to_offset();
    ...
    #[inline]
    pub(super) fn block_group_idx_of_ino(&self, ino: u32) -> u32 {
