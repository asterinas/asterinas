...
impl BlockGroup {
    /// Loads and constructs the block group from block device and super block.
    pub fn new(
        idx: usize,
        ...
        let get_bitmap = |bid: BlockId, bit_len: usize| -> Result<BitMap> {
            ...
            BitMap::from_bytes_with_bit_len(block.as_slice(), bit_len).ok_or(Error::BadBitMap)
        };
    ...
    pub fn sync_metadata(
        ...
        block_device.write_bytes(inode_bitmap_offset, inner.inode_bitmap.as_bytes())?;

        // Write block bitmap
        let block_bitmap_offset = BlockId::new(inner.descriptor.block_bitmap).to_offset();
        block_device.write_bytes(block_bitmap_offset, inner.block_bitmap.as_bytes())?;
