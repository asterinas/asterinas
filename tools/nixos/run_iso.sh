...
ASTERINAS_IMAGE=${NIXOS_DIR}/asterinas.img
...
rm -f $ASTERINAS_IMAGE
dd if=/dev/zero of=$ASTERINAS_IMAGE bs=1M count=$NIXOS_DISK_SIZE_IN_MB
sync

QEMU_ARGS="qemu-system-x86_64 \
	-bios /root/ovmf/release/OVMF.fd \
	-cdrom ${ISO_IMAGE} -boot d \
	-drive if=none,format=raw,id=u0,file=${ASTERINAS_IMAGE} \
