...
impl Matcher {
    ...
    fn matches(syscall: &Syscall, pattern: &Pattern) -> bool {
        ...
        // 1. Argument count must match exactly
    ...
    /// - `PatternArg::None` → Always matches (wildcard)
    ...
    fn matches_arg(
        ...
        match pattern_arg {
            // Wildcard - matches any syscall argument
...
mod tests {
    ...
    fn test_integration_multiple_patterns_first_match() {
        ...
        patterns.insert(
            "test".to_string(),
            vec![
                Pattern::new("test".to_string(), vec![PatternArg::Integer]),
                Pattern::new("test".to_string(), vec![PatternArg::None]),
            ],
        );
