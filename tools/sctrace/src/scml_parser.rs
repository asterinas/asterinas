...
impl Pattern {
    ...
    ///
    /// # Wildcard Behavior:
    /// - In struct patterns (e.g., `{ field1 = value1, .. }`), the `..` at the end
    ///   means the struct can match additional unspecified fields. This sets the
    ///   `wildcard` flag in `PatternStruct` to `true`.
    /// - In syscall argument lists (e.g., `foo(a, b, ..);`), a trailing `..` means
    ///   the pattern can match extra, unspecified arguments. This sets the `wildcard`
    ///   flag in `Pattern` to `true`.
    ///
    /// # Examples
    ///
    ...
    pub fn parse(input: &str) -> IResult<&str, Pattern> {
        ...
        Ok((
            input,
            Pattern::new(name.to_string(), args, has_wildcard.is_some()),
        ))
    }

    /// Creates a new pattern with the given name and arguments.
    pub fn new(name: String, args: Vec<PatternArg>, wildcard: bool) -> Self {
        Self {
            name,
            args,
            wildcard,
...
mod tests {
    ...
    #[test]
    fn test_from_scml_pattern_wildcard_true() {
        // Pattern with trailing `..` in syscall argument list (Pattern wildcard, not struct wildcard)
        let content = "mycall(arg1, arg2, ..);";
        let result = Patterns::from_scml(content).unwrap();

        let patterns = result.get("mycall").unwrap();
        assert_eq!(patterns.len(), 1);
        let pattern = &patterns[0];

        // Should have 2 arguments, both unconstrained
        assert_eq!(pattern.args().len(), 2);
        assert!(matches!(pattern.args()[0], PatternArg::None));
        assert!(matches!(pattern.args()[1], PatternArg::None));

        // Pattern wildcard should be true
        assert!(pattern.wildcard());
    }

    #[test]
    fn test_from_scml_pattern_wildcard_with_constraints() {
        // Pattern with constraints and trailing `..` (Pattern wildcard, not struct wildcard)
        let content = "foo(a = <INTEGER>, b = O_RDONLY, ..);";
        let result = Patterns::from_scml(content).unwrap();

        let patterns = result.get("foo").unwrap();
        assert_eq!(patterns.len(), 1);
        let pattern = &patterns[0];

        // Should have 2 arguments with constraints
        assert_eq!(pattern.args().len(), 2);
        assert!(matches!(pattern.args()[0], PatternArg::Integer));
        if let PatternArg::Flags(flag_set) = &pattern.args()[1] {
            assert_eq!(flag_set.flags().len(), 1);
            assert!(matches!(&flag_set.flags()[0], PatternArg::Flag(f) if f == "O_RDONLY"));
        } else {
            panic!("Expected PatternArg::Flags");
        }
