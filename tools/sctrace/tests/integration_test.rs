...
/// Helper function to create a temporary SCML file with given content
fn create_scml_file(content: &str) -> NamedTempFile {
...
/// Helper function to create a temporary strace log file
fn create_strace_log(lines: &[&str]) -> NamedTempFile {
    let mut file = NamedTempFile::new().unwrap();
    for line in lines {
...
fn test_open_syscall() {
    ...
    "#;
    let scml_file = create_scml_file(scml_content);
    ...
    let log_file = create_strace_log(log_lines);

    let result = sctrace::check_logfile(
        scml_file.path().to_str().unwrap(),
...
fn test_multiple_struct_with_same_name() {
    ...
    assert_eq!(errors[0], format!("Unsupported syscall: {}", log_lines[2]));
}
...
#[test]
fn test_check_logfile_invalid_scml_file() {
    let log_file = create_strace_log(&["read(3, \"data\", 100) = 100"]);

    let result = sctrace::check_logfile(
        "/nonexistent/scml/file.scml",
        log_file.path().to_str().unwrap(),
    );
...
#[test]
fn test_check_logfile_invalid_log_file() {
    let scml_file = create_scml_file("read(fd, buf, count);");
...
#[test]
fn test_check_logfile_empty_log() {
    let scml_content = r#"
        read(fd, buf, count);
    "#;
    let scml_file = create_scml_file(scml_content);
    let log_file = create_strace_log(&[]);
...
fn test_check_program_simple_command() {
    ...
    // Use /bin/true which is a simple command that exits immediately
    let result = sctrace::check_program(
        scml_file.path().to_str().unwrap(),
        "/bin/true",
        (&[]).to_vec(),
    );
    ...
    match result {
        Ok(errors) => {
            assert!(errors.iter().all(|error| !error.contains("execve(")));
        }
        Err(e) => {
            panic!("check_program failed: {}", e);
        }
    }
}
