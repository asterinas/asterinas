...
impl<T: PageTableEntryTrait, M> PageTable<T, M> {
    ...
    fn do_page_walk_mut(&mut self, vaddr: Vaddr, create: bool) -> Option<&mut T> {
        ...
        debug_assert!(size_of::<T>() * (T::page_index(vaddr, level) + 1) <= PAGE_SIZE);
    ...
    ///
    /// This function will return None if it is not reach the last PTE.
...
pub fn calculate_pte_vaddr<T: PageTableEntryTrait>(
    root_pa: Paddr,
    target_va: Vaddr,
    level: usize,
) -> Vaddr {
