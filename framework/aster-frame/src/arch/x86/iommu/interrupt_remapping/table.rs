...
pub struct IntRemappingTable {
    ...
    handles: Vec<Arc<SpinLock<IrtEntryHandle>>>,
}

impl IntRemappingTable {
    pub fn alloc(&self) -> Option<Arc<SpinLock<IrtEntryHandle>>> {
        let id = self.allocator.lock_irq_disabled().alloc()?;
        Some(self.handles.get(id).unwrap().clone())
    }

    /// Create a Interrupt Remapping Table with one Frame (default).
    ...
    /// Encode as the value written into the Interrupt Remapping Table Register.
...
impl Drop for IntRemappingTable {
    fn drop(&mut self) {
        for handle in self.handles.iter_mut() {
            let mut handle = handle.lock_irq_disabled();
            handle.set_none();
        }
    }
}
