...
cpu_local! {
    ...
    /// minimizing synchronization overhead for task management. In fact, the most important thing
    /// is that our current context switch mechanism will have a difficult competition problem in
    /// the global scheduler.
    static LOCAL_SCHEDULER: SpinLock<GeneralScheduler> = SpinLock::new(GeneralScheduler::new());
}
...
pub fn set_local_scheduler(scheduler: &'static dyn Scheduler) {
    LOCAL_SCHEDULER.lock_irq_disabled().scheduler = Some(scheduler);
}

/// Sets the local scheduler for the current CPU.
pub fn fetch_task_from_local() -> Option<Arc<Task>> {
    LOCAL_SCHEDULER.lock_irq_disabled().dequeue(this_cpu())
}
...
pub fn preempt_local() -> Option<Arc<Task>> {
    LOCAL_SCHEDULER.lock_irq_disabled().preempt(this_cpu())
}
